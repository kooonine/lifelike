...
// 리뉴얼 관련 경로 설정
define('G5_LAYOUT_DIR', 'layout');
define('G5_LAYOUT_PATH', G5_PATH . '/' . G5_LAYOUT_DIR);
define('G5_LAYOUT_URL', G5_URL . '/' . G5_LAYOUT_DIR);
define('G5_VIEW_DIR', 'view');
define('G5_VIEW_PATH', G5_PATH . '/' . G5_LAYOUT_DIR . '/' . G5_VIEW_DIR);
define('G5_VIEW_URL', G5_URL . '/' . G5_LAYOUT_DIR . '/' . G5_VIEW_DIR);

define('G5_IS_IE', strpos($_SERVER['HTTP_USER_AGENT'], 'Trident') === false ? false : true);
...
for ($m = 1; $m <= 4; $m++) {
    if (date("n", strtotime("last monday") - 604800 * $m) != $this_month) {
        define('G5_WEEK_NUM', $m);
        break;
    }
}
