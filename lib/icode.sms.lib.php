...
function CheckCommonType($dest, $rsvTime) {
	...
	if (strlen($dest)<10 || strlen($dest)>11) return "휴대전화 번호가 틀렸습니다";
	$CID=substr($dest,0,3);
	//if ( eregi("[^0-9]",$CID) || ($CID!='010' && $CID!='011' && $CID!='016' && $CID!='017' && $CID!='018' && $CID!='019') ) return "휴대전화 앞자리 번호가 잘못되었습니다";
	if ( preg_match("/[^0-9]/i",$CID) || ($CID!='010' && $CID!='011' && $CID!='016' && $CID!='017' && $CID!='018' && $CID!='019') ) return "휴대전화 앞자리 번호가 잘못되었습니다";
