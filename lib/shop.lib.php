...
class item_list
{
	...
	protected $query = "";
	protected $query_unlimit = "";

	// pick list
	protected $wishlist = array();
	// 필터 추가 대응 변수 - balance@panpacific.co.kr
	protected $mix = false;
	protected $filter = "";
	protected $filter_count = array();
	...
	function set_query_unlimit($query)
	{
		$this->query_unlimit = trim($query);
	}
	function get_query_unlimit()
	{
		return $this->query_unlimit;
	}
	...
	function set_wish($wishlist)
	{
		$wl = is_array($wishlist) ? $wishlist : explode(",", $wishlist);
		$this->wishlist = $wl;
	}
	...
	function run()
	{
		...
		if ($this->query) {
			...
			if ($this->use) {
				$where[] = " it_use = '1' ";
				$where[] = " (it_period is null or it_period = '' or now() between left(it_period,10) and concat(right(it_period,10),' 23:59:59')) ";
			}
	...
	// 출력 리뉴얼 - 200907 balance@panpacific.co.kr
	// testtests 
	function out_koo()
	{
		global $g5, $config, $member, $default;
		...
		if ($this->query) {
			...
			if ($this->use) {
				$where[] = " it_use = '1' AND it_total_size = '1' ";
				$where[] = " (it_period is null or it_period = '' or now() between left(it_period,10) and concat(right(it_period,10),' 23:59:59')) ";
			}
			...
			if ($this->brand) {
				$where[] = " it_brand = '{$this->brand['br_name_en']}' ";
			}

			if ($this->order_by) {
				$view_order = " ORDER BY {$this->order_by} ";
			}

			if ($this->mix) {
				$view_order = " ORDER BY view_order ASC ";
				$mix = rand(0, 10);
			}

			$sql_select = " SELECT io.io_hoching, {$this->fields}, GROUP_CONCAT(DISTINCT fi.fi_contents) AS filter, FLOOR((a.it_discount_price / a.it_price) * 100) AS discount, FLOOR(RAND({$mix})*10000) AS view_order ";
			$sql_common = " FROM `{$g5['g5_shop_item_table']}` AS a LEFT JOIN lt_shop_item_finditem AS fi ON fi.it_id = a.it_id  LEFT JOIN lt_shop_item_option AS io on io.it_id = a.it_id";

			if ($this->event) {
				$sql_common = " FROM `{$g5['g5_shop_event_item_table']}` AS b LEFT JOIN `{$g5['g5_shop_item_table']}` AS a on (a.it_id = b.it_id) ";
				$where[] = " b.ev_id = '{$this->event}' ";
			}

			if ($this->filter) {
				$where[] = " fi.fi_contents IN ('" . str_replace(",", "','", $this->filter) . "') ";
			}
			...
			$view_id = "VIEW_LIST_" . date("Ymd") . "_" . session_id();
			$sql_view = $sql_select . $sql_common . $sql_where . $sql_group . $view_order;
			$sql = "SELECT * FROM " . $view_id . $sql_limit;

			// create가 true면 view 생성
			if ($this->create === true) {
				sql_query("CREATE OR REPLACE VIEW " . $view_id . " AS " . $sql_view);
			}
			...
			if ($this->is_page) {
				$sql2 = " SELECT count(*) AS cnt FROM " . $view_id;
				$row2 = sql_fetch($sql2);
				$this->total_count = $row2['cnt'];
			}
			...
			$this->set_query($sql);
			$this->set_query_unlimit($sql_view);
		}
	...
	// 출력 리뉴얼 - 190407 balance@panpacific.co.kr
	function out()
	{
		global $g5, $config, $member, $default;
	...
	// 출력 리뉴얼 - 20200710 sbs@panpacific.co.kr
	function out_brand()
	{
		global $g5, $config, $member, $default;
		...
		if ($this->query) {
			...
			$sql_select = " SELECT io.io_hoching, {$this->fields}, GROUP_CONCAT(DISTINCT fi.ca_name) AS filter, FLOOR((a.it_discount_price / a.it_price) * 100) AS discount, FLOOR(RAND({$mix})*10000) AS view_order ";
			$sql_common = " FROM `{$g5['g5_shop_item_table']}` AS a LEFT JOIN lt_shop_category AS fi ON fi.ca_id = a.ca_id  LEFT JOIN lt_shop_item_option AS io on io.it_id = a.it_id";
			...
			if ($this->filter) {
				$where[] = " fi.ca_name IN ('" . str_replace(",", "','", $this->filter) . "') ";
			}
...
function get_image($img, $width = 0, $height = 0, $img_id = '')
{
	...
	$full_img = 'https://lifelike.co.kr/data/item/' . $img;
...
function get_it_image($it_id, $width, $height = 0, $anchor = false, $img_id = '', $img_alt = '', $is_crop = false)
{
	...
	for ($i = 1; $i <= 10; $i++) {
		...
		// $file = 'https://lifelikecdn.co.kr/data/item/' .  $row['it_img' . $i];
...
function get_it_image_path($it_id, $width, $height = 0, $anchor = false, $img_id = '', $img_alt = '', $is_crop = false)
{
	global $g5;
	...
	if ($thumb) {
		return str_replace(G5_PATH, G5_URL, $filepath . '/' . $thumb);
	} else {
		return G5_SHOP_URL . '/img/no_image.gif';
	}
}
...
function get_it_thumbnail($img, $width, $height = 0, $id = '', $is_crop = false)
{
	...
	$file = 'https://lifelikecdn.co.kr/data/item/' . $img;
	dd($file);
	if (is_file($file))
		$size = @getimagesize($file);
...
function get_it_thumbnail_path($img, $width, $height = 0, $id = '', $is_crop = false)
{
	$str = '';
...
function get_it_stock_qty($it_id)
{
	...
	and io_id = ''
	and ct_stock_use = 0
	and ct_status in ('주문', '결제완료', '상품준비중') ";
	$row = sql_fetch($sql);
...
// 옵션의 재고 (창고재고수량 - 주문대기수량)
function get_option_stock_qty2($it_id, $io_id, $type, $io_value)
{
	global $g5;
	...
	if ($subj_count > 1) {
		...
		for ($i = 0; $i < $subj_count; $i++) {
			...
			if ($opt_count) {
				...
				$select = '<select id="it_option_' . $seq . '" class="it_option btn_select"' . $disabled . '>' . PHP_EOL;
		...
		if ($is_div === 'div') {
			...
			$str .= '<li>' . PHP_EOL;
			$str .= '<span>' . $subj[0] . '</span>' . PHP_EOL;
		} else {
			$str .= '<tr>' . PHP_EOL;
			$str .= '<th><label for="it_option_1">' . $subj[0] . '</label></th>' . PHP_EOL;
		}

		$select = '<select id="it_option_1" class="it_option btn_select">' . PHP_EOL;
		$select .= '<option value="">선택</option>' . PHP_EOL;
		for ($i = 0; $row = sql_fetch_array($result); $i++) {
			if ($row['io_price'] >= 0)
		...
		if ($is_div === 'div') {
			...
			$str .= '<strong>' . $select . '</strong>' . PHP_EOL;
			$str .= '</li>' . PHP_EOL;
		} else {
			$str .= '<td>' . $select . '</td>' . PHP_EOL;
			$str .= '</tr>' . PHP_EOL;
...
function get_item_supply($it_id, $subject, $is_div = '')
{
	...
	for ($i = 0; $i < $subj_count; $i++) {
		...
		if ($opt_count) {
			...
			$select = '<select id="it_supply_' . $seq . '" class="it_supply btn_select">' . PHP_EOL;
			$select .= '<option value="">선택</option>' . PHP_EOL;
			for ($k = 0; $k < $opt_count; $k++) {
				$opt_val = $opt[$k];
				if ($opt_val) {
...
function print_item_options($it_id, $cart_id)
{
	...
	for ($i = 0; $row = sql_fetch_array($result); $i++) {
		...
		$tmp_option = explode(' : ', get_text($row['ct_option']));

		$str .= $tmp_option[1] . ' ' . $row['ct_qty'] . '개 (' . $price_plus . display_price($row['io_price']) . ')' . PHP_EOL;
	}
...
function get_new_od_id()
{
	...
	$sql = " select max(od_id) as max_od_id from {$g5['g5_shop_order_table']} where SUBSTRING(od_id, 1, 8) = '$date' ";
	$row = sql_fetch($sql);
	$od_id = $row['max_od_id'];
	if ($od_id == 0)
		$od_id = 1;
	...
	$od_id = $date . substr("000000" . $od_id, -6);
	// 주문서 테이블 Lock 풀고
...
function item_icon($it)
{
	...
	and (
	( cp_method = '0' and cp_target LIKE '%{$it['it_id']}%' )
	OR
...
// sns 공유하기
function get_sns_share_link_new($sns, $url, $title, $sns_img, $img)
{
	global $config;
	...
	switch ($sns) {
		case 'facebook':
			$str = '<a href="https://www.facebook.com/sharer/sharer.php?u=' . urlencode($url) . '&amp;p=' . urlencode($title) . '" class="share-facebook" target="_blank"><img src="' . $img . '" srcset="/img/re/icon-facebook.png 1x, /img/re/icon-facebook@2x.png 2.5x, /img/re/icon-facebook@3x.png 4x" alt="페이스북에 공유"></a>';
			break;
		case 'twitter':
			$str = '<a href="https://twitter.com/share?url=' . urlencode($url) . '&amp;text=' . urlencode($title) . '" class="share-twitter" target="_blank"><img src="' . $img . '" srcset="/img/re/icon-twitter.png 1x, /img/re/icon-twitter@2x.png 2.5x, /img/re/icon-twitter@3x.png 4x" alt="트위터에 공유"></a>';
			break;
		case 'googleplus':
			$str = '<a href="https://plus.google.com/share?url=' . urlencode($url) . '" class="share-googleplus" target="_blank"><img src="' . $img . '" srcset="/img/re/icon-google.png 1x, /img/re/icon-google@2x.png 2.5x, /img/re/icon-google@3x.png 4x" alt="구글플러스에 공유"></a>';
			break;
		case 'kakaotalk':
			if ($config['cf_kakao_js_apikey'])
				$str = '<a href="javascript:kakaolink_send(\'' . str_replace('+', ' ', urlencode($title)) . '\', \'' . urlencode($url) . '\' , \'' . urlencode($sns_img) . '\');" class="share-kakaotalk"><img src="' . $img . '" srcset="/img/re/icon-kakao.png 1x, /img/re/icon-kakao@2x.png 2.5x, /img/re/icon-kakao@3x.png 4x" alt="카카오톡 링크보내기"></a>';
			break;
	}
...
function get_order_info($od_id)
{
	...
	from {$g5['g5_shop_cart_table']}
	where od_id = '$od_id'
	and ct_status IN ( '주문', '결제완료', '상품준비중', '배송중', '배송완료' ) ";
	$sum = sql_fetch($sql);
...
// 배송비 구함
function get_sendcost($cart_id, $selected = 1, $od_type = 'O')
{
	global $default, $g5;
	...
	$sql = "select c.ca_id3
            ...
            where a.od_id = '{$cart_id}'
            and a.ct_status IN ( '쇼핑', '주문', '결제완료', '상품준비중', '배송중', '배송완료' )
            and a.ct_send_cost = '0'
            and a.ct_select = '{$selected}'
            and a.od_type = '{$od_type}'
            group by c.ca_id3
            order by ca_id3 asc ";
	$result = sql_query($sql);
	for ($j = 0; $company = sql_fetch_array($result); $j++) {
		$ca_id3 = $company['ca_id3'];
		if ($ca_id3 == "") {
			$de_individual_costs_use = $default['de_individual_costs_use'];
			$de_send_condition = $default['de_send_condition'];
			$de_send_cost_limit = $default['de_send_cost_limit'];
			$de_send_cost_list = $default['de_send_cost_list'];
		} else {
			$cp = sql_fetch("select * from lt_member_company where company_code = '{$ca_id3}' ");

			$de_individual_costs_use = $cp['cp_individual_costs_use'];
			$de_send_condition = $cp['cp_send_condition'];
			$de_send_cost_limit = $cp['cp_send_cost_limit'];
			$de_send_cost_list = $cp['cp_send_cost_list'];
		}


		$sql = "select  SUM((b.its_final_price + a.io_price) * a.ct_qty) as price,
                        SUM((b.its_price + a.io_price) * a.ct_qty) as before_price,
                        SUM(a.ct_qty) as qty
                        ,a.od_id, a.it_id, a.it_sc_type, c.ca_id3, a.mb_id
                from lt_shop_cart as a
                ...
                and a.od_type = '{$od_type}'
                and c.ca_id3 = '{$ca_id3}'
                group by a.od_id, a.it_id, a.it_sc_type, c.ca_id3 
                order by ca_id3 asc ";

		$result = sql_query($sql);
		for ($i = 0; $sc = sql_fetch_array($result); $i++) {
			if ($sc['it_sc_type'] == '2' && $de_individual_costs_use == '1') {
				//선택설정(상품별 개별배송비 사용 필수) : 상품별로 배송비 부과
				$total_send_cost += get_item_sendcost($sc['it_id'], $sc['price'], $sc['qty'], $cart_id, $sc['before_price']);
			} else {
				//기본설정, 상품별 개별배송비 사용안함 : 전체상품금액을 합쳐서 배송비 부과
				$total_price += $sc['price'];
				$total_before_price += $sc['before_price'];
				$diff++;
			}
		}
		...
		if ($total_price >= 0 && $diff > 0) {
			if ($de_send_condition == "판매") {
				$total_price = (int) $total_before_price;
			}

			if ($total_price < $de_send_cost_limit) {
				$send_cost += $de_send_cost_list;
			}
		}
	}
	...
	$mb_id = sql_fetch("SELECT mb_id FROM lt_shop_cart WHERE ct_id='{$cart_id}'");
	if (!empty($mb_id['mb_id'])) {
		$sql_mb_oc = "SELECT COUNT(*) AS CNT FROM lt_shop_order WHERE mb_id='{$mb_id['mb_id']}'";
		$mb_oc = sql_fetch($sql_mb_oc);
		if ((int) $mb_oc['CNT'] == 0) {
			return 0;
		}
	}
...
// 상품별 배송비
function get_item_sendcost($it_id, $price, $qty, $cart_id, $before_price = 0)
{
	global $g5, $default;
	if ($before_price == 0) $before_price = $price;

	$sql = " select it_id, it_sc_type, it_sc_method, it_sc_price, it_sc_minimum, it_sc_qty, it_send_condition, it_individual_costs_use
	from {$g5['g5_shop_item_table']}
	where it_id = '$it_id' ";
	$it = sql_fetch($sql);
	if (!$it['it_id']) {
	...
	//할인전, 정상판매가격 기준(권장)
	if ($it['it_send_condition'] == "판매") {
		$price = $before_price;
	}

	if ($it['it_sc_type'] == 1) { // 무료배송
		$sendcost = 0;
	} else if ($it['it_sc_type'] == 2) { // 상품별로 배송비 부과
		...
		$sendcost = $it['it_sc_price'];
	} else if ($it['it_sc_type'] == 4) { // 수량별 부과
		if (!$it['it_sc_qty'])
			$it['it_sc_qty'] = 1;
		...
		$sendcost = 0; //기본설정, 전체상품금액을 합쳐서 배송비 부과
	}
...
function get_cancel_sendcost($od_id, $ct_id_arr)
{
	global $g5, $default;
	...
	if ($ct_id_arr != "") {
		...
		$sql = "  select  SUM((b.its_final_price + a.io_price) * a.ct_qty) as price,
		                  SUM((b.its_price + a.io_price) * a.ct_qty) as before_price,
		                  SUM(a.ct_qty) as qty,
		                  a.it_id, a.it_sc_type, c.it_send_condition, c.it_sc_minimum, c.it_sc_price
		from  lt_shop_cart as a
		      left join lt_shop_item_sub as b on a.it_id = b.it_id and a.its_no = b.its_no
		      left join lt_shop_item c on ( a.it_id = c.it_id )
		where a.od_id = '{$od_id}'
		and a.ct_id in (" . $ct_id_arr . ")
		and a.ct_status IN ( '쇼핑', '주문', '결제완료', '상품준비중', '배송중', '배송완료' )
		group by a.it_id, a.it_sc_type, c.it_send_condition, c.it_sc_minimum, c.it_sc_price ";
		$result = sql_query($sql);

		for ($i = 0; $row = sql_fetch_array($result); $i++) {
			if ($row['it_sc_type'] == '2' && $default['de_individual_costs_use'] == '1') {
				//선택설정(상품별 개별배송비 사용 필수) : 상품별로 배송비 부과
				$price = $row['price'];
				if ($row['it_send_condition'] == "판매") {
					$price = $row['before_price'];
				}

				if ($price >= $row['it_sc_minimum']) {
					$sendcost = 0;
				} else {
					$sendcost = $row['it_sc_price'];
				}
				$total_send_cost += $sendcost;
			} else {
				//기본설정, 상품별 개별배송비 사용안함 : 전체상품금액을 합쳐서 배송비 부과
				$total_price += $row['price'];
				$total_before_price += $row['before_price'];
				$diff++;
			}
		}
		...
		if ($total_price >= 0 && $diff > 0) {
			...
			if ($default['de_send_condition'] == "판매") {
				$total_price = $total_before_price;
			}

			if ($total_price < $default['de_send_cost_limit']) {
				$send_cost = $default['de_send_cost_list'];
			}
		}
	}
...
function get_return_sendcost($od_id, $ct_id_arr)
{
	global $g5, $default;
	...
	$sql = "  select  SUM((b.its_final_price + a.io_price) * a.ct_qty) as price,
	                  SUM((b.its_price + a.io_price) * a.ct_qty) as before_price,
	                  SUM(a.ct_qty) as qty,
	                  a.it_id, a.it_sc_type, c.it_send_condition, c.it_sc_minimum, c.it_sc_price, c.it_return_costs
	from  lt_shop_cart as a
	      left join lt_shop_item_sub as b on a.it_id = b.it_id and a.its_no = b.its_no
	      left join lt_shop_item c on ( a.it_id = c.it_id )
	where a.od_id = '{$od_id}' ";

	if ($ct_id_arr != "") $sql .= " and a.ct_id not in (" . $ct_id_arr . ") ";

	$sql .= " and a.ct_status IN ( '쇼핑', '주문', '결제완료', '상품준비중', '배송중', '배송완료' )
	           group by a.it_id, a.it_sc_type, c.it_send_condition, c.it_sc_minimum, c.it_sc_price, c.it_return_costs ";
	...
	for ($i = 0; $row = sql_fetch_array($result); $i++) {
		if ($row['it_sc_type'] == '2' && $default['de_individual_costs_use'] == '1') {
			//선택설정(상품별 개별배송비 사용 필수) : 상품별로 배송비 부과
			$sendcost = (int) $row['it_sc_price'] + (int) $row['it_return_costs'];
			$total_send_cost += $sendcost;
		} else {
			//기본설정, 상품별 개별배송비 사용안함 : 전체상품금액을 합쳐서 배송비 부과
			$diff++;
	...
	$send_cost = 0;
	if ($total_price >= 0 && $diff > 0) {
		$send_cost = $default['de_send_cost_list'] + $default['de_return_costs'];
	}
...
function pg_charge_calc($od_settle_case, $price)
{
	$pg_charge = 0;
	if ($od_settle_case == "계좌이체") {
		$pg_charge = $price * 0.015;
		if ($pg_charge > 200) $pg_charge = 200;
	} else if ($od_settle_case == "가상계좌") {
		$pg_charge = 250;
	} else if ($od_settle_case == "휴대전화") {
		$pg_charge = $price * 0.028;
	} else if ($od_settle_case == "신용카드") {
		$pg_charge = $price * 0.025;
	} else if ($od_settle_case == "간편결제") {
		$pg_charge = $price * 0.025;
	}

	return floor($pg_charge);
}
/**
 *
(일반헤지)
1년 이내 해지시: (월 리스료÷30일) X (의무사용일수-실사용일수) X 30%
1년 이상~2년 이내 해지시: (월 이용료÷30일) X (의무사용일수-실사용일수) X 20%
2년 이상~3년 이내 해지시: (월 이용료÷30일) X (의무사용일수-실사용일수) X 10%
( )(파손해지)
파손/분실료 기준: (상품판매가X90%)-{(상품판매가÷36개월)X사용개월수
 */
function rental_contractout_calc($od, $type)
{
	$penalty = 0;

	$rt_rental_price = (int) $od['rt_rental_price']; //리스 금액
	$rental_startdate = $od['rt_rental_startdate']; //리스시작일
	$rental_enddate = date("Y-m-d", strtotime($od['rt_rental_startdate'] . "+" . $od['rt_month'] . " month")); //리스종료일
	$tt = strtotime($rental_enddate) - strtotime($rental_startdate);
	$rental_day = ceil($tt / (60 * 60 * 24)) - 1; //의무사용일수

	$tt2 = strtotime(G5_TIME_YMD) - strtotime($rental_startdate);
	$use_day = ceil($tt2 / (60 * 60 * 24)) + 1; //실사용일수

	$residual_value = ceil($rt_rental_price / 30); //월 이용료÷30일


	$tot_price = (int) $od['od_cart_price']; //상품판매가

	if ($type == "분실해지" || $type == "파손해지" || $type == "분실/파손해지") {
		//파손/분실료 기준: (상품판매가X90%)-{(상품판매가÷36개월)X사용개월수
		$penalty = ceil(($tot_price * 0.9) - ($tot_price / (int) $od['rt_month'] * ceil($use_day / 30)));
	} else {
		//($type == "일반해지")
		//1년 이내 해지시: (월 이용료÷30일) X (의무사용일수-실사용일수) X 30%
		//1년 이상~2년 이내 해지시: (월 이용료÷30일) X (의무사용일수-실사용일수) X 20%
		//2년 이상~3년 이내 해지시: (월 이용료÷30일) X (의무사용일수-실사용일수) X 10%
		//$penalty = rental_contractout_calc($od);
		if ($use_day < 365) $penalty = ceil($residual_value * ($rental_day - $use_day) * 0.3);
		elseif ($use_day < 730) $penalty = ceil($residual_value * ($rental_day - $use_day) * 0.2);
		elseif ($use_day < 1095) $penalty = ceil($residual_value * ($rental_day - $use_day) * 0.1);
	}
	return $penalty;
}
/*
function rental_contractout_calc($od)
{
	global $g5, $default;
	...
	등록비(판매가 10%)
	+ 위약금(할인이전 총금액 * 남은개월수 * 20%(1년 이상 사용시 10%))
	+ 배송비
	- 선납지불금액(리스료 - 리스료 * 사용일수/30)
	$penalty = 0;
	//리스 금액
	$rt_rental_price = (int)$od['rt_rental_price'];

	//등록비
	$tot_price = $rt_rental_price * (int)$od['rt_month'];
	$penalty += $tot_price * 0.1;

	$rt_minus_month = (int)$od['rt_month'] - (int)$od['rt_payment_count'];

	$rt_minus_percent = 0.3; //1년 이내 해지시: (월 이용료÷30일) X (의무사용일수-실사용일수) X 30%
	if((int)$od['rt_payment_count'] >= 12) $rt_minus_percent = 0.2; //1년 이상~2년 이내 해지시: (월 이용료÷30일) X (의무사용일수-실사용일수) X 20%
	if((int)$od['rt_payment_count'] >= 24) $rt_minus_percent = 0.1; //2년 이상~3년 이내 해지시: (월 이용료÷30일) X (의무사용일수-실사용일수) X 10%

	$penalty += $rt_rental_price * $rt_minus_month * $rt_minus_percent;
	$penalty += (int)$default['de_return_costs'];
...
// 다운로드한 쿠폰인지
function get_btn_act($od, $ct_status, $uid)
{
	global $g5;
	...
	switch ($ct_status) {
		...
		case '수거박스배송':
			if ($od['od_boxsend_invoice']) {
				$url = G5_URL . "/common/tracking.php?invc_co={$od['od_boxsend_delivery_company']}&invc_no={$od['od_boxsend_invoice']}";
				$btn_act .= '<a href="' . $url . '" target="_blank" class="btn_invoice"><button class="btn gray_line small"><span>배송조회</span></button></a>';
			}
			break;
		case '수거중':
...
function get_samjin_sendcost($od_id, $current_status)
{
	global $default, $g5;
	...
	$sql = "select  SUM((b.its_final_price + a.io_price) * a.ct_qty) as price,
                    SUM((b.its_price + a.io_price) * a.ct_qty) as before_price,
                    SUM(a.ct_qty) as qty
                    ,a.od_id, a.it_id, a.it_sc_type, a.it_sc_price
            from lt_shop_cart as a
              inner join lt_shop_item_sub as b on a.it_id = b.it_id and a.its_no = b.its_no
              inner join lt_shop_item as c on a.it_id = c.it_id
            where a.od_id = '{$od_id}'
            and a.ct_status = '{$current_status}'
            and a.io_sapcode_color_gz != ''
            group by a.od_id, a.it_id, a.it_sc_type, a.it_sc_price ";
	...
	for ($i = 0; $sc = sql_fetch_array($result); $i++) {
		...
		if ($sc['it_sc_type'] == '2' && $de_individual_costs_use == '1') {
			//선택설정(상품별 개별배송비 사용 필수) : 상품별로 배송비 부과
			$total_send_cost += (int) $sc['it_sc_price'];
		} else {
			//기본설정, 상품별 개별배송비 사용안함 : 전체상품금액을 합쳐서 배송비 부과
			$diff++;
		}
