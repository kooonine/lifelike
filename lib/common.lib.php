...
function get_paging($write_pages, $cur_page, $total_page, $url, $add = "")
{
	...
	// <span class="paging-prev">&lt;</span><span class="active">1</span><span>2</span><span>3</span><span>4</span><span>5</span><span class="paging-next">&gt;</span>
	$str = '';

	// if ($cur_page >= 1) {
	// 	$str .= '<a class="paging-first" href="' . $url . '1' . $add . '">&lt;&lt;<span class="blind">처음</span></a>' . PHP_EOL;
	...
	if ($total_page > 1) {
		$str .= '<a class="paging-first" href="' . $url . '1' . $add . '"><img style="width :12px;height:8px" src="/img/re/paging_first@2x.png" srcset="/img/re/paging_first.png 0.5x,/img/re/paging_first@2x.png 2.5x,/img/re/paging_first@3x.png 3x"><span class="blind">처음</span></a>' . PHP_EOL;
		$str .= '<a class="paging-prev" href="' . $url . ($start_page - 1) . $add . '"><img style="width :8px;height:6px" src="/img/re/paging_prev@2x.png" srcset="/img/re/paging_prev.png 0.5x,/img/re/paging_prev@2x.png 2.5x,/img/re/paging_prev@4x.png 3x"><span class="blind">이전</span></a>' . PHP_EOL;
	}
	
	if ($end_page >= $total_page) $end_page = $total_page;

	// if ($start_page > 1) $str .= '<a class="paging-prev" href="' . $url . ($start_page - 1) . $add . '">&lt;<span class="blind">이전</span></a>' . PHP_EOL;
	...
	//if ($total_page > $end_page) $str .= '<a class="paging-next" href="' . $url . ($end_page + 1) . $add . '">&gt;<span class="blind">다음</span></a>' . PHP_EOL;
	if ($total_page > 1) $str .= '<a class="paging-next" href="' . $url . ($end_page + 1) . $add . '"><img style="width :8px;height:6px" src="/img/re/paging_next@2x.png" srcset="/img/re/paging_next.png 0.5x,/img/re/paging_next@2x.png 2.5x,/img/re/paging_next@4x.png 3x"><span class="blind">다음</span></a>' . PHP_EOL;
	if ($cur_page < $total_page) {
		$str .= '<a class="paging-end" href="' . $url . $total_page . $add . '"><img style="width :12px;height:8px" src="/img/re/paging_end@2x.png" srcset="/img/re/paging_end.png 0.5x,/img/re/paging_end@2x.png 2.5x,/img/re/paging_end@4x.png 3x"><span class="blind">맨끝</span></a>' . PHP_EOL;
	}

	if ($str)
		return "<div class='list-paging'>{$str}</div>";
	else
		return "";
}
...
function html_purifier($html)
{
	...
	array_push($domains, 'lifelikecdn.co.kr/');
...
// 회원 정보를 얻는다.
function get_member_by_email($mb_email, $fields = '*')
{
	global $g5;

	$mb_email = strtolower(str_replace(' ', '', $mb_email));
...
function is_admin($mb_id)
{
	...
	$sql = " select ad_type from lt_admin where mb_id = '{$mb_id}' ";
	$row = sql_fetch($sql);

	if ($row['ad_type'] == 'super') return 'super';
	elseif ($row['ad_type'] == 'admin') return 'admin';
	elseif ($row['ad_type'] == 'brand') return 'brand';
	return '';
}
...
// 포인트 부여
function insert_point($mb_id, $point, $content = '', $rel_table = '', $rel_id = '', $rel_action = '', $expire = 0, $admin_id = '')
{
	global $config;
	global $g5;
	global $is_admin;
	...
	$mb = sql_fetch(" select mb_id,mb_hp,mb_tier from {$g5['member_table']} where mb_id = '$mb_id' ");
	if (!$mb['mb_id']) {
		return 0;
	}
	// // 이벤트 
	...
	// 	if($odDate['od_receipt_time'] < '2021-09-19' && $odDate['od_receipt_time'] > '2021-09-06') {
	// 		$point = $point * 2;
	// 	}
	// }
	...
	// 주문start--
	// if ($rel_table== '@order' && strpos($content, '적립') !== false)
	// {
	...
	if ($admin_id != '') $sql .= ", po_approve_id = '$admin_id' ";
	sql_query($sql);
...
function get_sideview($mb_id, $name = '', $email = '', $homepage = '')
{
	...
	if ($email)
		$str2 .= "<a href=\"" . G5_ADMIN_URL . "/operation/configform_sendEmail.php?mb_id=" . $mb_id . "&amp;act_button=EMAIL\">메일보내기</a>\n";
	//if($homepage)
	//    $str2 .= "<a href=\"".$homepage."\" target=\"_blank\">홈페이지</a>\n";

	if ($mb_id)
		$str2 .= "<a href=\"" . G5_ADMIN_URL . "/member_form.php?mb_id=" . $mb_id . "\" onclick=\"win_profile(this.href); return false;\">회원관리</a>\n";
	...
	if ($is_admin == "super" && $mb_id) {
		...
		$str2 .= "<a href=\"" . G5_ADMIN_URL . "/operation/configform_saveMoney_management.php?sfl=mb_id&amp;stx=" . $mb_id . "\" target=\"_blank\">적립금내역</a>\n";
	}
	$str2 .= "</span>\n";
	$str .= $str2;
...
// 날짜를 select 박스 형식으로 얻는다
function month_select($date, $name = '')
{
	global $g5;

	$s = '';
	if (substr($date, 0, 4) == "0000") {
		$date = substr(G5_TIME_YMD, 0, 7);
	}
...
function get_hidden($field, $value)
{
	if (is_int($value)) {
		return ((int) $field === $value) ? ' hidden' : '';
	}
...
function is_mobile()
{
	$G5_EXCEPT_MOBILE_AGENT = 'iPad';
	$excepted = preg_match('/' . $G5_EXCEPT_MOBILE_AGENT . '/i', $_SERVER['HTTP_USER_AGENT']);
	$mobile = preg_match('/' . G5_MOBILE_AGENT . '/i', $_SERVER['HTTP_USER_AGENT']);
	return empty($excepted) ? $mobile : false;
}
...
function get_shop_uniqid()
{
	...
	while (1) {
		...
		$sql = " select ifnull(max(uq_id)+1,'" . $datekey . "000001') new_uq_id from {$g5['uniqid_table']} where uq_id like '{$datekey}%' and length(uq_id) = 14 ";
		$row = sql_fetch($sql);
		$key = $row["new_uq_id"];
...
// 휴대전화번호의 숫자만 취한 후 중간에 하이픈(-)을 넣는다.
...
function member_delete($mb_id)
{
	...
	//회원 탈퇴일 지정
	sql_query("update {$g5['member_table']} set mb_leave_date = '" . G5_TIME_YMD . "' , mb_3 = '" . date('Ymd', G5_SERVER_TIME) . " 관리자 삭제' where mb_id = '{$mb_id}' ");

	//삭제 대상 회원 자료 백업
	sql_query("insert into lt_member_delete select * from lt_member where mb_id = '{$mb_id}' ");

	// 회원자료는 정보만 없앤 후 아이디는 보관하여 다른 사람이 사용하지 못하도록 함 : 061025
	$sql = " update {$g5['member_table']} set
	mb_password = '', mb_level = 1, mb_email = '', mb_homepage = '', mb_tel = '', mb_hp = '', mb_zip1 = '', mb_zip2 = '', mb_addr1 = '', mb_addr2 = '', mb_birth = '', mb_sex = '', mb_signature = ''
...
function get_member_profile_img($mb_id = '', $width = '', $height = '', $alt = 'profile_image', $title = '')
{
	...
	if (!$src) {
		if (!empty($no_profile_cache)) {
			...
			$no_profile_cache = $src = isset($tmp[1]) ? $tmp[1] : G5_IMG_URL . '/default.jpg';
		}
	}
...
// MS DB 연결
function mssql_sql_connect($host, $user, $pass, $db = SAMJIN_MSSQL_DB)
{
	// SAMJIN_MSSQL_DB = TA_TEST_MALL_LINK2
	global $g5;
	...
	if (OSWINDOWS) {
		...
		putenv("FREETDSCONF=" . G5_DATA_PATH . "/freetds.conf");
		$link = @mssql_connect($host, $user, $pass);
		mssql_sql_select_db($db, $link);
	}
...
// DB 선택
function mssql_sql_select_db($db, $connect)
{
	global $g5;
...
function array_iconv($row)
{
	if ($row) {
		foreach ($row as $key => $item) {
			$current_enc = mb_detect_encoding($item, "auto", true);
			if ($current_enc == 'EUC-KR') {
				$p[$key] = iconv("EUC-KR", "UTF-8", $item);
			} else if ($current_enc == 'UTF-8' || $current_enc == 'ASCII') {
				$p[$key] = $item;
			} else {
				$p[$key] =  mb_convert_encoding($item, "UTF-8", "EUC-KR, ASCII, UTF-8");
			}
		}
		// print_r2($p);
		return $p;
...
// mysqli_query 와 mysqli_error 를 한꺼번에 처리
// mysql connect resource 지정 - 명랑폐인님 제안
function mssql_sql_query($sql, $error = G5_DISPLAY_SQL_ERROR, $link = null)
{
	global $g5;

	if (!$link)
		$link = $g5['connect_samjindb'];
	...
	if (OSWINDOWS) {
		if ($error) {
			$result = @sqlsrv_query($link, $sql) or die("<p>$sql<p>" . " : " .  sqlsrv_errors($link) . "<p>error file : {$_SERVER['SCRIPT_NAME']}");
		} else {
			$result = @sqlsrv_query($link, $sql);
		}
	} else {
		if ($error) {
			$result = @mssql_query($sql, $link) or die("<p>$sql<p> : <p>error file : {$_SERVER['SCRIPT_NAME']}");
		} else {
...
// 쿼리를 실행한 후 결과값에서 한행을 얻는다.
function mssql_sql_fetch($sql, $error = G5_DISPLAY_SQL_ERROR, $link = null)
{
	global $g5;
	...
	$result = mssql_sql_query($sql, $error, $link);
	//$row = @sql_fetch_array($result) or die("<p>$sql<p>" . mysqli_errno() . " : " .  mysqli_error() . "<p>error file : $_SERVER['SCRIPT_NAME']");
...
// 결과값에서 한행 연관배열(이름으로)로 얻는다.
function mssql_sql_fetch_array($result)
{
	if (OSWINDOWS) {
...
function mssql_sql_num_rows($result)
{
	if (OSWINDOWS) {
...
function mssql_sql_init($sp_name, $params)
{
	global $g5;
...
//php5.4 이하버전의 JSON_UNESCAPED_UNICODE 미지원으로 인한 json_encode한글 처리 문제
function json_encode_raw($input, $options = null)
{
...
function msg_autosend($sf_cate, $sf_title, $mb_id, $arr_change_data = null)
{

	global $g5, $config;

	if ($arr_change_data == null) $arr_change_data = array();

	// 리스계약
	if (!empty($arr_change_data['rt_id'])) {
		$od = sql_fetch("select a.*, date_add(rt_rental_startdate, interval rt_month month) as rt_rental_enddate from {$g5['g5_shop_rental_table']} as a where a.rt_id = '{$arr_change_data['rt_id']}' ");

		$tot_price = $od['rt_cart_price'] + $od['rt_send_cost'] - $od['rt_cancel_price'];

		$arr_change_data['주문번호'] = $od['rt_id'];
		$arr_change_data['주문일자'] = $od['rt_time'];
		$arr_change_data['총주문금액'] = number_format($tot_price) . "원";
		$arr_change_data['결제금액'] = number_format($od['rt_receipt_price']) . "원";
		$arr_change_data['쿠폰금액'] = number_format((int) $od['rt_cart_coupon'] + (int) $od['rt_cart_coupon'] + (int) $od['rt_coupon']) . "원";
		$arr_change_data['적립금사용금액'] = number_format($od['rt_receipt_point']) . "원";
		$arr_change_data['결제수단'] = $od['rt_settle_case'];
		$arr_change_data['최종결제금액'] = number_format($od['rt_receipt_price']) . "원";

		if ($od['rt_receipt_price'] == 0) {
			//취소
			$arr_change_data['환불금액'] = number_format($od['rt_cancel_price']) . "원";

			$arr_change_data['결제금액'] = number_format($od['rt_cancel_price']) . "원";
			$arr_change_data['최종결제금액'] = number_format($od['rt_cancel_price']) . "원";
		} else {
			//환불
			$arr_change_data['환불금액'] = number_format($od['rt_refund_price']) . "원";
		}

		$arr_change_data['수령인'] = $od['rt_b_name'];
		$arr_change_data['연락처'] = $od['rt_b_hp'];
		$arr_change_data['배송지'] = $od['rt_b_zip1'] . $od['rt_b_zip2'] . ' ' . $od['rt_b_addr1'] . ' ' . $od['rt_b_addr2'];
		$arr_change_data['배송메모'] = $od['rt_memo'];

		$arr_change_data['배송택배사'] = $od['rt_delivery_company'];
		$arr_change_data['배송송장번호'] = $od['rt_invoice'];
		$arr_change_data['배송일자'] = $od['rt_invoice_time'];
		$arr_change_data['링크-배송조회'] = G5_URL . "/common/tracking.php?invc_co=" . $od['rt_delivery_company'] . "&invc_no=" . $od['rt_invoice'];

		$arr_change_data['링크-주문내역'] = G5_SHOP_URL . "/orderinquiryview.php?rt_id=" . $od['rt_id'];

		if ($od['rt_type'] == "R") {
			$arr_change_data['계약번호'] = $od['rt_id'];
			$arr_change_data['계약기간'] = $od['rt_month'] . "개월";
			$arr_change_data['리스시작일'] = $od['rt_rental_startdate'];
			$arr_change_data['리스만료일'] = $od['rt_rental_enddate'];
			$arr_change_data['리스금액'] = $arr_change_data['렌탈금액'] = number_format($od['rt_rental_price']) . "원";
			$arr_change_data['납부횟수'] = $od['rt_payment_count'] . "회";

			$arr_change_data['위약금'] = number_format($od['rt_penalty']) . "원";
		}

		if ($od['od_type'] == "L" || $od['od_type'] == "K" || $od['od_type'] == "S") {
			$ct = sql_fetch("select a.* from lt_shop_cart as a where a.od_id = '{$arr_change_data['od_id']}' ");

			$arr_change_data['제품이미지'] = get_it_image($ct['it_id'], 100, 100);
			$arr_change_data['제품명'] = $ct['it_name'];
			$arr_change_data['옵션명'] = $ct['ct_option'];
			$arr_change_data['고객기재사항'] = $od['cust_memo'];

			if ($od['od_type'] == "L") {
				$arr_change_data['세탁금액'] = ($ct['ct_free_laundry_use']) ? "무료" : number_format($ct['ct_laundry_price']) . "원";
			} else if ($od['od_type'] == "K") {
				$arr_change_data['세탁금액'] = ($ct['ct_free_laundry_use']) ? "무료" : number_format($ct['ct_laundrykeep_lprice']) . "원";
				$arr_change_data['보관금액'] = number_format(((int) $ct['ct_laundrykeep_kprice']) * (int) $ct['ct_keep_month']) . "원";
				$arr_change_data['보관기간'] = $ct['ct_keep_month'] . "개월";

				$arr_change_data['보관시작일'] = $ct['ct_keep_start'];
				$arr_change_data['보관종료일'] = $ct['ct_keep_end'];
			}
		}
	}

	// 일반상품
	if (isset($arr_change_data['od_id']) && $arr_change_data['od_id']) {
		$od = sql_fetch("select a.*, date_add(rt_rental_startdate, interval rt_month month) as rt_rental_enddate from lt_shop_order as a where a.od_id = '{$arr_change_data['od_id']}' ");
		$cart = sql_fetch("SELECT it_name, ct_invoice, ct_delivery_company,ct_invoice_time, COUNT(*) AS CNT FROM lt_shop_cart WHERE od_id='" . $od['od_id'] . "' GROUP BY od_id");

		$tot_price = $od['od_cart_price'] + $od['od_send_cost'] - $od['od_cancel_price'];

		$arr_change_data['주문내역'] = $cart['it_name'];
		if ($cart['CNT'] > 1) $arr_change_data['주문내역'] .= "외 " . ($cart['CNT'] - 1) . "건";
		$arr_change_data['주문번호'] = $od['od_id'];
		$arr_change_data['주문일자'] = $od['od_time'];
		$arr_change_data['총주문금액'] = number_format($tot_price) . "원";
		$arr_change_data['결제금액'] = number_format($od['od_receipt_price']) . "원";
		$arr_change_data['쿠폰금액'] = number_format((int) $od['od_cart_coupon'] + (int) $od['od_cart_coupon'] + (int) $od['od_coupon']) . "원";
		$arr_change_data['적립금사용금액'] = number_format($od['od_receipt_point']) . "원";
		$arr_change_data['결제수단'] = $od['od_settle_case'];
		$arr_change_data['최종결제금액'] = number_format($od['od_receipt_price']) . "원";

		if ($od['od_receipt_price'] == 0) {
			//취소
			$arr_change_data['환불금액'] = number_format($od['od_cancel_price']) . "원";

			$arr_change_data['결제금액'] = number_format($od['od_cancel_price']) . "원";
			$arr_change_data['최종결제금액'] = number_format($od['od_cancel_price']) . "원";
		} else {
			//환불
			$arr_change_data['환불금액'] = number_format($od['od_refund_price']) . "원";
		}

		$arr_change_data['수령인'] = $od['od_b_name'];
		$arr_change_data['연락처'] = $od['od_b_hp'];
		$arr_change_data['배송지'] = $od['od_b_zip1'] . $od['od_b_zip2'] . ' ' . $od['od_b_addr1'] . ' ' . $od['od_b_addr2'];
		$arr_change_data['배송메모'] = $od['od_memo'];

		$arr_change_data['배송택배사'] = $od['od_delivery_company'];
		$arr_change_data['배송송장번호'] = $od['od_invoice'];
		$arr_change_data['배송일자'] = $od['od_invoice_time'];

		if (!$arr_change_data['배송택배사'] || $arr_change_data['배송택배사']==0) $arr_change_data['배송택배사'] = $cart['ct_delivery_company'];
		if (!$arr_change_data['배송송장번호'] || $arr_change_data['배송송장번호']==0) $arr_change_data['배송송장번호'] = $cart['ct_invoice'];
		if (!$arr_change_data['배송일자'] || $arr_change_data['배송일자']==0) $arr_change_data['배송일자'] = $cart['ct_invoice_time'];

		$arr_change_data['링크-배송조회'] = G5_URL . "/common/tracking.php?invc_co=" . $od['od_delivery_company'] . "&invc_no=" . $od['od_invoice'];

		$arr_change_data['링크-주문내역'] = G5_SHOP_URL . "/orderinquiryview.php?od_id=" . $od['od_id'];

		if ($od['od_type'] == "R") {
			$arr_change_data['계약번호'] = $od['od_id'];
			$arr_change_data['계약기간'] = $od['rt_month'] . "개월";
			$arr_change_data['리스시작일'] = $od['rt_rental_startdate'];
			$arr_change_data['리스만료일'] = $od['rt_rental_enddate'];
			$arr_change_data['리스금액'] = $arr_change_data['렌탈금액'] = number_format($od['rt_rental_price']) . "원";
			...
			$arr_change_data['위약금'] = number_format($od['od_penalty']) . "원";
		}
	...
	if ($arr_change_data['od_list']) {
		$list = $arr_change_data['od_list'];

		$product_content1 = "";
		$product_content2 = '<table align="center" cellpadding="0" cellspacing="0" style="margin:0;padding:0;width:700px;border-collapse:collapse;"><colgroup><col width="25%"></colgroup><tbody>';
		$product_content2 .= '<tr><td colspan="3" style="padding:0 0 10px;font-family:Noto Sans KR, dotum, sans-serif;line-height:1;font-size:14px;font-weight:bold;"><div style="padding:0 0 15px;font-family:Noto Sans KR, dotum, sans-serif;line-height:1;font-size:14px;font-weight:bold;border-bottom:2px solid #ccc;">제품정보</div></td></tr>';

		$product_content1_cancel = "";
		$product_content2_cancel = $product_content2;

		$product_content1_return1 = "";
		$product_content2_return1 = $product_content2;

		$product_content1_return2 = "";
		$product_content2_return2 = $product_content2;

		for ($i = 0; $i < count($list); $i++) {
			$product_content_sms = $list[$i]['it_name'] . "/" . $list[$i]['ct_option'] . PHP_EOL;

			$product_content_mail = '<tr>';
			$product_content_mail .= '<td rowspan="3" style="margin:0;padding:10px 10px;font-family:Noto Sans KR, dotum, sans-serif;line-height:1;font-size:13px;border:1px solid #ddd;">' . $list[$i]['it_simg'] . '</td>';
			$product_content_mail .= '<td style="margin:0;padding:10px 10px;font-family:Noto Sans KR, dotum, sans-serif;line-height:1;font-size:13px;border:1px solid #ddd;"><span style="font-weight:bold;">' . $list[$i]['it_name'] . '</span></td>';
			$product_content_mail .= '<td rowspan="3" style="margin:0;padding:10px 10px;font-family:Noto Sans KR, dotum, sans-serif;line-height:1;font-size:13px;border:1px solid #ddd;"><span style="font-weight:bold;">' . number_format($list[$i]['stotal_price']) . '원</span></td>';
			$product_content_mail .= '</tr>';
			$product_content_mail .= '<tr>';
			$product_content_mail .= '<td style="margin:0;padding:10px 10px;font-family:Noto Sans KR, dotum, sans-serif;line-height:1;font-size:13px;border:1px solid #ddd;"><span style="font-weight:bold;">' . $list[$i]['ct_option'] . '</span></td>';
			$product_content_mail .= '</tr>';
			$product_content_mail .= '<tr>';
			$product_content_mail .= '<td style="margin:0;padding:10px 10px;font-family:Noto Sans KR, dotum, sans-serif;line-height:1;font-size:13px;border:1px solid #ddd;"><span style="font-weight:bold;">수량:' . $list[$i]['qty'] . '개</span></td>';
			$product_content_mail .= '</tr>';

			if ($list[$i]['ct_status'] == "주문취소") {
				$product_content1_cancel .= $product_content_sms;
				$product_content2_cancel .= $product_content_mail;
			} else if ($list[$i]['ct_status'] == "반품요청") {
				$product_content1_return1 .= $product_content_sms;
				$product_content2_return1 .= $product_content_mail;
			} else if ($list[$i]['ct_status'] == "반품완료") {
				$product_content1_return2 .= $product_content_sms;
				$product_content2_return2 .= $product_content_mail;
			}
			$product_content1 .= $product_content_sms;
			$product_content2 .= $product_content_mail;
		}

		$product_content2 .= "</tbody></table>";
		$arr_change_data['제품정보'] = $product_content1;
		$arr_change_data['제품정보상세'] = $product_content2;

		$arr_change_data['제품정보-주문취소'] = $product_content1_cancel;
		$arr_change_data['제품정보상세-주문취소'] = $product_content2_cancel;

		$arr_change_data['제품정보-반품요청'] = $product_content1_return1;
		$arr_change_data['제품정보상세-반품요청'] = $product_content2_return1;

		$arr_change_data['제품정보-반품완료'] = $product_content1_return2;
		// $arr_change_data['반품상품'] = $product_content1_return2;
		$arr_change_data['제품정보상세-반품완료'] = $product_content2_return2;
	}

	$mb = get_member($mb_id);
	$arr_change_data["고객명"] = $mb['mb_name'];
	$arr_change_data["아이디"] = $mb['mb_id'];
	$arr_change_data["고객명(아이디)"] = $mb['mb_name'] . "(" . $mb['mb_id'] . ")";

	if (!isset($arr_change_data["0000.00.00"]))	$arr_change_data["0000.00.00"] = G5_TIME_YMD;
	$arr_change_data["0000.00"] = substr(G5_TIME_YMD, 0, 7);
	$arr_change_data["MALL_NAME"] = $config['cf_admin_email_name'];

	$arr_change_data['회사명'] = $default['de_admin_company_name'];
	$arr_change_data["라이프라이크바로가기"] = "<a href='" . G5_URL . "'>라이프라이크 바로가기</a>";
	$arr_change_data["라이프라이크로그인하기"] = "<a href='" . G5_URL . "/bbs/login.php'>라이프라이크 로그인하기</a>";
	$arr_change_data["SITEURL"] = G5_URL;

	if ($mb['mb_hp'] != "") {
		sms_autosend($sf_cate, $sf_title, '', $mb['mb_name'], $mb['mb_hp'], $arr_change_data);
	}

	if ($mb['mb_email'] != "") {
		mailer_autosend($sf_cate . '-' . $sf_title, $mb['mb_email'], $arr_change_data);
	}
}
...
if (!function_exists('column_char')) {
	...
	{
		if ($i >= 26) return chr(65 + floor($i / 26) - 1) . chr(65 + ($i % 26));
		else return chr(65 + $i);
	}
}
