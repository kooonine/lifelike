    ...
    class SMS5 extends SMS {
        ...
        function CheckCommonTypeDest($strDest, $nCount) {
            for ($i=0; $i<$nCount; $i++) {
                ...
                if (strlen($hp_number)<10 || strlen($hp_number)>11) return "휴대전화 번호가 틀렸습니다";

                $CID=substr($hp_number,0,3);
                if ( preg_match("/[^0-9]/",$CID) || ($CID!='010' && $CID!='011' && $CID!='016' && $CID!='017' && $CID!='018' && $CID!='019') ) return "휴대전화 앞자리 번호가 잘못되었습니다";
            }
        }
