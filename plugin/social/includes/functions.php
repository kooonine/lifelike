...
function social_extends_get_keys($provider)
{
    ...
    if (empty($r)) {
        // Apple
        $testUrl = 'https://lifelike.co.kr/auth/apple_login.php';
        $r['Apple'] = array(
            "enabled" => option_array_checked('apple', $config['cf_social_servicelist']) ? true : false,
            "keys" => array(
                "id" => 'com.litandard.lifelike.singin',
                ...
                "scope" => get_social_callbackurl('apple'),
            ),
...
function social_login_success_after($mb, $link = '', $mode = '', $tmp_create_info = array())
{
    ...
    if ((isset($mb['mb_id']) && !empty($mb['mb_id']) && $provider && $user_profile = social_session_exists_check()) || $provider =='apple') {
