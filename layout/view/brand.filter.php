...
<div class="modal fade" id="modal-filter" tabindex="-1" role="dialog" aria-labelledby="btn-modal-filter" aria-hidden="true" style="max-width: unset; min-width: unset;">
    <div class="modal-dialog" role="document">
        <div id="modal-filter-content" class="modal-content" style="height: auto;">
...
<style>
    ...
    #modal-filter-content .filter-btn-group{margin : 0 14px;  justify-content: space-around; margin-bottom : 40px;}
...
<script>
    ...
    function formFilterSubmit(reset) {
        ...
        //     if (filterSet[fi].value.length > 0 && filterSet[fi].value != "on") filterChecked.push(filterSet[fi].value);
        // })
    ...
    $("span.product-list-filter-selected").on("click", function() {
        let filterChecked = filter.split(',');
        const removeFilter = $(this).text();

        $(filterChecked).each(function(fi) {
            if (filterChecked[fi] == removeFilter) filterChecked.splice(fi, 1);
        })
    ...
    $(".btn-filter-youpick").on("click", function() {
        return location.href = "/shop/brand.php?br_id=pick";
    });
