...
if (get_cookie('ck_visit_ip') != $_SERVER['REMOTE_ADDR'])
{
    ...
    $tmp_row = sql_fetch(" select count(vi_id) as new_vi_id from {$g5['visit_table']} where vi_ip = '{$remote_addr}'");
    $vi_new = ($tmp_row['new_vi_id'])?0:1;
    
    if(version_compare(phpversion(), '5.3.0', '>=') && defined('G5_BROWSCAP_USE') && G5_BROWSCAP_USE) {
        include_once(G5_BBS_PATH.'/visit_browscap.inc.php');
    }
    $sql = " insert {$g5['visit_table']} ( vi_id, vi_ip, vi_date, vi_time, vi_referer, vi_agent, vi_browser, vi_os, vi_device, vi_new ) 
                values ( '{$vi_id}', '{$remote_addr}', '".G5_TIME_YMD."', '".G5_TIME_HIS."', '{$referer}', '{$user_agent}', '{$vi_browser}', '{$vi_os}', '{$vi_device}', '{$vi_new}'  ) ";
