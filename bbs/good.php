...
if($_POST['js'] == "on") {
	...
	if (!get_session($ss_name) && !$comment) {
	...
	if ($good == 'good' || $good == 'nogood'){
		if (!$board['bo_use_good'] && $good == 'good') {
			$error = '이 게시판은 좋아요 기능을 사용하지 않습니다.';
			print_result($error, $flag, $count);
		}

		$sql = " select bg_flag from {$g5['board_good_table']} where bo_table = '{$bo_table}' and wr_id = '{$wr_id}' and mb_id = '{$member['mb_id']}' and bg_flag = '{$good}' ";
		$row = sql_fetch($sql);
		if ($row['bg_flag']){
			// 기존에 존재하여 카움트 차감
			sql_query(" update {$g5['write_prefix']}{$bo_table} set wr_{$good} = wr_{$good} - 1 where wr_id = '{$wr_id}' ");
			// 내역 생성
			sql_query(" delete from {$g5['board_good_table']} where bo_table = '{$bo_table}' and wr_id = '{$wr_id}' and mb_id = '{$member['mb_id']}' and bg_flag = '{$good}' ");
			...
			if(isset($bg_comment) && $bg_comment != ""){
			    $sql .= " ,bg_comment= '{$bg_comment}' ";
			}
