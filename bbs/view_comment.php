...
if($comment_order == 'wr_good desc'){
    $comment_order = 'wr_good desc, ';
    $comment_order_sql = 'p_wr_good desc,a.wr_comment desc, ';
} else if($comment_order != ''){
    $comment_order .= ', ';
    $comment_order_sql = $comment_order;
} else{
    $comment_order = "wr_comment desc, ";
    $comment_order_sql = "wr_comment desc, ";
}

$limit_count = $_GET['limit_count'];
if($limit_count == ''){
    $limit_count = $board['bo_reply_rows'];
}
// 코멘트 출력

$sql = " select count(*) as count from $write_table where wr_parent = '$wr_id' and wr_is_comment = 1 ";
$row2 = sql_fetch($sql);
$total_count = $row['count'];

//$sql = " select * from {$write_table} where wr_parent = '{$wr_id}' and wr_is_comment = 1 order by wr_comment desc, wr_comment_reply ";
$sql = " select a.*, b.wr_good as p_wr_good from $write_table as a left join $write_table as b on a.wr_parent = b.wr_parent and a.wr_comment = b.wr_comment and b.wr_comment_reply = '' and b.wr_is_comment = 1
         where a.wr_parent = '$wr_id' and a.wr_is_comment = 1 order by {$comment_order_sql} a.wr_comment_reply limit 0, {$limit_count}";
