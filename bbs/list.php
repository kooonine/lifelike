...
$bo_search_sql = " ";
if(isset($wr_6) && $wr_6){
	$wr_6_split = explode(',', $wr_6);

	for($i=0;$i<count($wr_6_split);$i++){
		if($wr_6_split[$i] != ''){
			$bo_search_sql .= " and wr_6 like '%{$wr_6_split[$i]}%'";
		}
	}
}
...
if ($is_search_bbs) {
	$sql = " select distinct wr_parent from {$write_table} where {$sql_search} {$bo_search_sql} {$sql_order} limit {$from_record}, $page_rows ";
} else {
	$sql = " select * from {$write_table} where wr_is_comment = 0 {$bo_search_sql}";
	if(!empty($notice_array))
