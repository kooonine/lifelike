...
// 쇼핑몰 설정값 배열변수
if ($member['mb_id']) {
    $defPoint = " select *, (SELECT mr_point_percent FROM lt_member_rating LEFT JOIN lt_member ON lt_member_rating.mr_rating = lt_member.mb_tier WHERE lt_member.mb_id = '{$member['mb_id']}' LIMIT 1) AS percentRating from {$g5['g5_shop_default_table']} ";
} else {
    $defPoint = " select * from {$g5['g5_shop_default_table']} ";
}
// $default = sql_fetch(" select * from {$g5['g5_shop_default_table']} ");
$default = sql_fetch($defPoint);
if ($default['percentRating'] && $default['de_point_percent'] != $default['percentRating']) $default['de_point_percent'] = $default['percentRating'];
// sjan qorhvk !!2qo whwrkxsp !!!!
// 
...
'입금'  : 신용카드, 계좌이체, 휴대전화결제가 된 상태, 무통장, 가상계좌는 주문후 입금한 상태를 말합니다.
'배송'  : 배송이 되면 취소가 불가합니다.
