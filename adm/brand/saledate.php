...
for ($i=0; $row=sql_fetch_array($list_result); $i++)
{
    ...
    $save['last_receipt_price'] += $row['od_receipt_price'] - $row['od_refund_price'];
...
if(isset($act) && $act == "exceldownload"){
    ...
    $headers = array('NO','주문일','결제건수','(총)주문금액','(총)배송비','쿠폰 할인','적립금사용'
        ,'결제 총 합계','할인합계','주문취소','환불금액','미수금');
    
    $bodys = array('NO','od_date','ordercount','orderprice','send_cost','ordercoupon','receiptpoint'
