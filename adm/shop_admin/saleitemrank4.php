...
if ($sc_ct_time != "") {
    ...
    $where_sql .= " and ct_time between '" . trim($it_times[0]) . " 00:00:00' and '" . trim($it_times[1]) . " 23:59:59' ";
}

if ($sca) {
    $where_sql .= " and b.ca_id like '$sca%' ";
}

if ($stx != "") {
    $where_sql .= " and (b.it_name like '%$stx%' or b.it_id like '%$stx%' )";
    if ($save_stx != $stx)
        $page = 1;
}

$sub_sql .= $where_sql;
$sub_sql .= " group by a.it_id";
$sql  = "select a.*
                    ...
                    <tbody>
                        ...
                        for ($i = 0; $row = sql_fetch_array($result); $i++) {
                            ...
                            <tr class="<?php echo $bg; ?>">
                                ...
                                <td class="td_num"><span class="openMemberList" data-itid="<?php echo $row['it_id'] ?>"><?php echo number_format($row['user_cnt']); ?></span></td>
...
<script>
    $(function() {
        ...
        $("span.openMemberList").on("click", function() {
            const itemId = $(this).data("itid");

            $.get('ajax.memberlist.php?it_id=' + itemId + '&q=<?php echo $enc->encrypt($where_sql) ?>', function(data) {
    ...
    function openMemberPopup(mb_id) {
        const url = "<?php echo G5_ADMIN_URL ?>/member_form.php?w=u&mb_id=" + mb_id;
