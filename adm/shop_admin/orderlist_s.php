...
$headers = array('NO', '주문일시', '주문번호', '주문상태', '클레임상태', '주문자아이디', '주문자명', '제품코드', '제품명', '옵션항목', 'RFID', '결제수단', '결제금액', '수거운송장번호', '고객운송장번호', '메모');
$bodys = array('NO', 'od_time', 'disp_od_id', 'od_status', 'ct_status_claim', 'mb_id', 'od_name', 'it_id', 'it_name', 'ct_option', 'rf_serial', 'od_settle_case', 'od_receipt_price', 'str_od_pickup_invoice', 'str_od_invoice', 'od_memo');
			...
			<tr>
				...
				<td colspan="2">
					<div class="col-lg-12 col-md-6 col-sm-12 col-xs-12">
						...
						<input type="radio" name="od_status" value="수선신청" id="od_status_odr" <?= get_checked($od_status, '수선신청'); ?>>
						<label for="od_status_odr">수선신청</label>
						...
						<label for="od_status_rdy2">수선수거완료</label>
						<input type="radio" name="od_status" value="제품확인" id="od_status_c" <?= get_checked($od_status, '제품확인'); ?>>
						<label for="od_status_c">제품확인</label>
						<input type="radio" name="od_status" value="수선중" id="od_status_l" <?= get_checked($od_status, '수선중'); ?>>
						<label for="od_status_l">수선중</label>
						<input type="radio" name="od_status" value="배송중" id="od_status_dvr" <?= get_checked($od_status, '배송중'); ?>>
						<label for="od_status_dvr">배송중</label>
						<input type="radio" name="od_status" value="수선완료" id="od_status_done" <?= get_checked($od_status, '수선완료'); ?>>
						<label for="od_status_done">수선완료</label>
					</div>
				</td>
			</tr>
		</table>
...
<form name="forderlist" id="forderlist" method="post" autocomplete="off">
	...
	<div class="local_cmd01 local_cmd">
		<div style="float: left">
			<input type="button" value="수선취소(CS)" class="btn btn_02" onclick="forderlist_submit('수선취소');">
			<input type="button" value="수선처리(CS)" class="btn btn_02" onclick="forderlist_submit('수선처리');">
		</div>
		<div style="float: right">
			...
			<input type="button" value="TEST:수거중" class="btn btn_01" onclick="forderlist_submit('수거중');">
			<input type="button" value="TEST:수거완료" class="btn btn_01" onclick="forderlist_submit('수거완료');">
			...
			<input type="button" value="수선주문확인" class="btn btn_02" onclick="forderlist_submit('주문확인');">
		...
		<table id="sodr_list">
			...
			<thead>
				<tr>
					...
					<th scope="col" rowspan="2">주문일시<br /><a href="<?= title_sort("od_id", 1) . "&amp;$qstr1"; ?>">주문번호</a></th>
					<th scope="col" rowspan="2">주문상태</th>
					<th scope="col" rowspan="2">제품<br />이미지</th>
					<th scope="col" rowspan="2">제품명</th>
					<th scope="col" rowspan="2">주문자명(ID)</th>
					<th scope="col" rowspan="2">결제금액<br />(결제수단)</th>
					<th scope="col" colspan="2">운송장정보</th>
					<th scope="col" rowspan="2">관리</th>
				</tr>
				<tr>
					<th scope="col">수거배송</th>
					<th scope="col">고객배송</th>
				</tr>
			</thead>
			<tbody>
				<?
				...
				?>
					<tr>
						...
						<td class="td_itopt_tl">
							<label>제품코드 : </label>
							<?= $opt['it_id'] ?>
							<?= ($opt['its_order_no']) ? "<br/>제품옵션 : (" . $opt['its_order_no'] . ")" : "" ?>
						...
						<td rowspan="<?= $rowspan ?>">
							...
							<?= $row['od_receipt_price'] ? '<br/>(' . $s_receipt_way . ')' : ''; ?>
						</td>
						...
						<td headers="delino">
							...
							<? if ($row['od_pickup_invoice']) { ?>
								<a href='<?php echo G5_URL ?>/common/tracking.php?invc_co=<?= $row['od_pickup_delivery_company'] ?>&invc_no=<?= $row['od_pickup_invoice'] ?>&view_popup=1' target='_blank' class="od_invoice">
									<?= $row['od_pickup_delivery_company'] ? $row['od_pickup_delivery_company'] : '-' ?><br />
									<?= $row['od_pickup_invoice'] ? $row['od_pickup_invoice'] : '-' ?><br />
								</a>
							<? } ?>
						</td>
						...
						<td headers="delino">
							...
							<? if ($row['od_invoice']) { ?>
								<a href='<?php echo G5_URL ?>/common/tracking.php?invc_co=<?= $row[''] ?>&invc_no=<?= $row['od_invoice'] ?>&view_popup=1' target='_blank' class="od_invoice">
...
<script>
	$(function() {
		...
		$(".orderitem").on("click", function() {
			...
			var url = "./orderproc.s.php?w=v&od_id=" + od_id;
			window.open(url, "orderform", "left=230,top=100,width=1000,height=800,scrollbars=yes,resizable=yes");
	...
	function forderlist_submit(change_status) {
		...
		switch (change_status) {
			case "수선취소":
				...
				if (current_status != "수선신청") {
					alert("'수선신청' 상태의 주문만 '수선취소'가 가능합니다.");
					return false;
				}

				var url = "./orderpartcancel.php?od_id=" + $("input[name='od_id[" + k + "]']").val();
				window.open(url, "orderpartcancel", "left=230,top=100,width=1000,height=800,scrollbars=yes,resizable=yes");
			...
			case "수선처리":
				...
				if (current_status != "수거중" && current_status != "수거완료" && current_status != "제품확인" && current_status != "수선중" && current_status != "수선완료") {
					alert("'수거중, 수거완료, 제품확인, 수선중, 수선완료' 상태의 주문만 '수선처리'가 가능합니다.");
					return false;
				}

				var url = "./orderproc.s.php?od_id=" + $("input[name='od_id[" + k + "]']").val();
			...
			case "주문확인":
				...
				for (var i = 0; i < $chk.size(); i++) {
					...
					if (current_status != "수선신청") {
						alert("'수선신청' 상태의 주문만 '수선주문확인'이 가능합니다.");
						return false;
					}
				}
			...
			case "수거중":
				...
				for (var i = 0; i < $chk.size(); i++) {
					...
					if (current_status != "수선신청" && current_status != "수거중") {
						alert("'수선신청','수거중' 상태의 주문만 '수거중'으로 변경 가능합니다.");
						return false;
					}
				}
			...
			case "배송중":
				...
				for (var i = 0; i < $chk.size(); i++) {
					...
					if (current_status != "수선중" && current_status != "수선완료") {
						alert("'수선중','수선완료' 상태의 주문만 '배송중'으로 변경 가능합니다.");
						return false;
					}
				}
