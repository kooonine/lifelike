...
// $rows=4;
// 테이블의 전체 레코드수만 얻음
$cnt_sql = "SELECT COUNT(*) AS cnt FROM  samjin_order_sale_registration_return {$sql_search}";
...
$sql = "SELECT * FROM samjin_order_sale_registration_return {$sql_search}  limit $from_record, $rows ";
...
$oform_headers = array('구분','거래처코드','거래처명','삼진코드','삼진품목명','색상','사이즈','주문수량','할인','마진','단가','분할가','쇼핑몰주문번호','사방넷주문번호');
$oform_bodys = array('order_gb','mall_code','mall_name','samjin_code','samjin_name','samjin_color','samjin_size','order_cnt','order_sale','order_majin','order_price','order_division_price','mall_order_no','sabang_ord_no');
...
<table id="reportTb" class="display" style="width:100%">
        <thead>
            <tr>
                <th><label for="chkall" class="sound_only">선택 전체</label>
                    <input type="checkbox" name="chkall" value="1" id="chkall" class="chk_all" onclick="allCheck2(this.form)">
                </th>
                <th>구분</th>
                <th>거래처코드</th>
                <th>거래처명</th>
        ...
        <tbody id="revenue-status">
            ...
            <tr>
                ...
                <td><?=$row_ord['order_division_price']?></td>
...
<?php echo get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, "{$_SERVER['SCRIPT_NAME']}?$qstr&amp;page="); ?>
    ...
    <script>
        ...
        function down_excel(){
            ...
            excel_sql = "select * from samjin_order_sale_registration_return where sno in ( "+selects +" ) ";
        ...
        function upload_excel(){
            ...
            var $form = $('<form></form>');     
            $form.attr('action', './upload_samjin_sale_registration_return.php');
            $form.attr('method', 'post');
