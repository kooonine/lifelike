...
$rt_laundry = get_search_string($rt_laundry);
...
$sql_common .= " LEFT JOIN {$g5['g5_shop_rental_order_table']} AS ro ON a.od_id=ro.od_id ";
$sql_common .= " LEFT JOIN {$g5['g5_shop_rental_table']} AS r ON ro.rt_id=r.rt_id ";
...
$excel_sql = "select a.*, b.it_id, b.it_name, b.ct_option, b.ct_qty, (b.ct_rental_price + b.io_price)  as opt_price, b.ct_status, b.ct_status_claim, b.ct_item_rental_month
                     ...
                     ,concat(a.od_type,'-',left(a.od_id,8),'-',right(a.od_id,6),'-',c.od_sub_id) as disp_od_sub_id
                     ,c.its_order_no,c.rf_serial,c.ct_free_laundry_use,c.ct_free_laundry*3 as ct_free_laundry,c.ct_laundry_use
                from lt_shop_order as a
                     left join lt_shop_cart as b
                        on a.od_id = b.od_id and b.io_type = '0'
                     left join lt_shop_order_item as c
                        on a.od_id = b.od_id and b.ct_id = c.ct_id
                $sql_search
...
$headers = array('NO', '리스상태', '주문일시', '주문번호', '주문자아이디', '주문자명', '상품주문번호', '상품코드', 'ORDERNO', 'RFID', '상품명', '옵션항목', '납부횟수', '전체', '납부상태', '무료세탁사용', '무료세탁가능횟수', '유료세탁사용');
$bodys = array('NO', 'od_status', 'od_time', 'disp_od_id', 'mb_id', 'od_name', 'disp_od_sub_id', 'it_id', 'its_order_no', 'rf_serial', 'it_name', 'ct_option', 'rt_payment_count', 'rt_month', 'rt_payment_status', 'ct_free_laundry_use', 'ct_free_laundry', 'ct_laundry_use');
            ...
            <tr>
                ...
                <td colspan="2">
                    <div class="col-lg-12 col-md-6 col-sm-12 col-xs-12">
                        <input type="radio" name="rt_payment_status" value="" id="rt_payment_status_all" <?php echo get_checked($rt_payment_status, '');     ?>>
                        <label for="rt_payment_status_all">전체</label>

                        <input type="radio" name="rt_payment_status" value="정상납부" id="rt_payment_status_ok" <?php echo get_checked($rt_payment_status, '정상납부'); ?>>
                        <label for="rt_payment_status_ok">정상납부 회원</label>
                        <input type="radio" name="rt_payment_status" value="미납" id="rt_payment_status_no" <?php echo get_checked($rt_payment_status, '미납'); ?>>
                        <label for="rt_payment_status_no">미납 회원</label>
                    </div>
                </td>
            </tr>

            <tr>
                <th scope="row">세탁서비스</th>
                <td colspan="2">
                    <div class="col-lg-12 col-md-6 col-sm-12 col-xs-12">
                        <input type="radio" name="rt_laundry" value="" id="rt_laundry_all" <?php echo get_checked($rt_laundry, '');     ?>>
                        <label for="rt_laundry_all">전체</label>

                        <input type="radio" name="rt_laundry" value="paid" id="rt_laundry1" <?php echo get_checked($rt_laundry, 'paid'); ?>>
                        <label for="rt_laundry1">세탁서비스 이용 회원(유료)</label>
                        <input type="radio" name="rt_laundry" value="free" id="rt_laundry2" <?php echo get_checked($rt_laundry, 'free'); ?>>
                        <label for="rt_laundry2">세탁서비스 이용 회원(무료)</label>
                        <input type="radio" name="rt_laundry" value="nouse" id="rt_laundry3" <?php echo get_checked($rt_laundry, 'nouse'); ?>>
                        <label for="rt_laundry3">세탁서비스 미이용 회원</label>
                    </div>
                </td>
            </tr>
...
<form name="forderlist" id="forderlist" method="post" autocomplete="off">
    ...
    <div class="tbl_head01 tbl_wrap">
        <table id="sodr_list">
            <caption>리스 내역 목록</caption>
            <thead>
                <tr>
                    <th scope="col" rowspan="2">No</th>
                    <th scope="col" rowspan="2">리스상태</th>
                    ...
                    <th scope="col" colspan="2">납부횟수정보</th>
                    <th scope="col" rowspan="2">납부상태</th>
                    <th scope="col" colspan="2">무료세탁서비스</th>

                    <th scope="col" rowspan="2">유료세탁서비스</th>

                    <th scope="col" rowspan="2">계약서</th>
                </tr>
                <tr>
                    <th scope="col">현재</th>
                    <th scope="col">전체</th>
            ...
            <tbody>
                ...
                for ($i = 0; $row = sql_fetch_array($result); $i++) {
                    ...
                    $odsql = " select it_id, od_sub_id, it_name, its_sap_code, its_order_no, rf_serial, 
                            ct_id, it_id, ct_price, ct_option, ct_status, io_type, io_price, ct_rental_price, ct_item_rental_month,
                            ct_free_laundry_use,ct_free_laundry,ct_free_laundry_delivery_price,ct_laundry_use,ct_laundry_price,
                            ct_laundry_delivery_price,ct_laundrykeep_use,ct_laundrykeep_lprice,ct_laundrykeep_kprice,
                            ct_laundrykeep_delivery_price,ct_repair_use,ct_repair_price,ct_repair_delivery_price
            from  lt_shop_order_item
            where od_id = '" . $row['od_id'] . "'
            ";
                    if ($rt_laundry) {
                        if ($rt_laundry == 'paid') $odsql .= " AND ct_laundry_use > '0' ";
                        if ($rt_laundry == 'free') $odsql .= " AND ct_free_laundry_use > '0' ";
                        if ($rt_laundry == 'nouse') {
                            $odsql .= " AND ct_free_laundry_use = '0' ";
                            $odsql .= " AND ct_laundry_use = '0' ";
                        }
                    }

                    $odsql .= " order by ct_id asc, od_sub_id";
                    ...
                    for ($k = 0; $opt = sql_fetch_array($od); $k++) {
                        ...
                        $opt_rental_price = $opt['ct_rental_price'] + $opt['io_price'];
                        ...
                        <tr>
                            <?php if ($k == 0) { ?>
                                ...
                                <td headers="odrstat" class="odrstat" rowspan="<?php echo $rowspan ?>">
                                    <label class="sound_only">리스상태</label>
                            ...
                            <td class="td_itopt_tl">
                                ...
                                <a href="./itemform.php?w=u&amp;it_id=<?php echo $opt['it_id']; ?>"><?php echo stripslashes($opt['it_name']); ?></a>
                                <br /> <?php echo $opt['ct_option']; ?>
                            </td>

                            <?php if ($k == 0) { ?>
                                <td class="td_chk" rowspan="<?php echo $rowspan ?>">
                                    <?php echo $row['rt_payment_count'] ?>회
                                </td>
                                <td class="td_chk" rowspan="<?php echo $rowspan ?>">
                                    <?php echo $row['rt_month'] ?>회
                                </td>

                                <td class="odrstat" rowspan="<?php echo $rowspan ?>">
                                    <?php echo $row['rt_payment_status'] ?>
                                    <?php echo ($row['rt_nonpayment_date'] == '') ? '' : '(' . $row['rt_nonpayment_date'] . ')' ?>
                                </td>
                            <?php } ?>

                            <td class="td_chk">
                                <?php echo $opt['ct_free_laundry_use'] ?>
                            </td>
                            <td class="td_chk">
                                <?php echo $opt['ct_free_laundry'] * 3 ?>
                            </td>

                            <td class="td_name">
                                <?php echo $opt['ct_laundry_use'] ?>
                            </td>

                            <?php if ($k == 0) { ?>
                                <td class="td_chk" rowspan="<?php echo $rowspan ?>">
                                    <label class="sound_only">계약서</label>
                                    <a href="<?php G5_SHOP_URL ?>/shop/orderinquiryview.rental.php?od_id=<?php echo $row['od_id'] ?>" target="_blank"><input type="button" value="리스계약서" class="btn btn_01"></a>
                                </td>
                            <?php } ?>
                        </tr>
                <?php
