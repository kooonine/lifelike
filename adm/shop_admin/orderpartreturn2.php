			...
			";
                         ...
                         ";
							...
							if ($cancel_cnt < $tot_rows) {
								...
								$partSql = "SELECT * FROM lt_shop_cart WHERE od_id = '$od_id' AND ct_status_claim = '반품' AND ct_status !='반품완료' GROUP BY ct_return_link";
								...
								// //부분 반품
								// if (count($ct_id_arr) > 0) {
								// 	$cancel_od_send_cost = get_cancel_sendcost($od_id, implode(",", $ct_id_arr));
								// } else {
								// 	$cancel_od_send_cost = 0;
								// }

								// $tot_cancel_price = $od['od_receipt_price'];
								// // $tot_cancel_price = $od['od_receipt_price'] + (int) $od['od_send_cost'];
							...
							<div class="tbl_frm01 tbl_wrap">
								<table>
									...
									<tbody>
										...
										<tr>
											<!-- <td><input type="text" class="frm_input text-right" id="od_refund_price" name="od_refund_price" value="<?php echo ($od['od_receipt_price'] - $od['od_refund_price'] - $tot_cancel_price) ?>"></td> -->
											...
											<td>
												<h4><span id="tot_cancel_price"><?= number_format($tot_cancel_price); ?> 원</span></h4>
												<input type="hidden" name="tax_mny" id="tax_mny" value="<?php echo $tot_cancel_price ?>">
											</td>
										</tr>
									</tbody>
								</table>
							...
							<div class="x_content">
								<div class="form-group">
									<div class="col-md-12 col-sm-12 col-xs-12 text-right">
										...
										<button type="submit" class="btn btn-success" id="btnConfirm">반품완료(환불처리)</button>
									</div>
								</div>
							</div>
...
<script>
	$(function() {
		// $("#od_refund_price").val(5000);
		$("#od_refund_price").autoNumeric('init', {
			mDec: '0'
		});
		$("#od_refund_price").keypress(function() {
			var tot_cancel_price = parseInt(<?php echo $od['od_receipt_price'] ?>) - parseInt($("#od_refund_price").autoNumeric('get'));
			if (tot_cancel_price >= 0) {
				$("#tot_cancel_price").html(number_format(tot_cancel_price) + " 원");
				$("#tax_mny").val(tot_cancel_price);
			} else {
				$("#od_refund_price").autoNumeric('set', <?php echo ($od['od_receipt_price'] - $tot_cancel_price) ?>);
				$("#tot_cancel_price").html("<?= number_format($tot_cancel_price); ?> 원");
				$("#tax_mny").val('<?= $tot_cancel_price; ?>');
			}
		});
