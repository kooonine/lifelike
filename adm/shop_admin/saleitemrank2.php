...
$sql  = "select c1.ca_name as ca_name1, c2.ca_name as ca_name2, c3.ca_name as ca_name3
                  ,b.ca_id
                	,SUM((ct_price + io_price) * ct_qty) as ct_price
                	,SUM(IF(od_receipt_price > 0,ct_qty, 0)) as ct_qty_sum
                	,SUM(IF(od_receipt_price > 0,((ct_price + io_price) * ct_qty), 0)) as ct_price_sum
                ...
                from 	lt_shop_cart a, lt_shop_item b,lt_shop_order c
                      ,lt_shop_category c1,lt_shop_category c2,lt_shop_category c3
                where 	a.it_id = b.it_id
                and		a.od_id = c.od_id
                and 	a.od_type in ('O','R','B')
                and   c.od_receipt_price > 0 
                and   c1.ca_id = left(b.ca_id,2)
                and   c2.ca_id = left(b.ca_id,4)
                and   c3.ca_id = b.ca_id ";
...
<div class="x_panel">
    <div class="x_title">
    	<h4><span class="fa fa-check-square"></span> 판매상품순위내역<small></small></h4>
    	<div class="clearfix"></div>
	</div>
	...
	<div class="tbl_head01 tbl_wrap">
        ...
        <div class="pull-right">
            ...
            <select id="sc_sort1" onchange="sstsod_change(this);">
                <option value="ct_status_1" <?php echo get_selected($sort1, 'ct_status_1') ; ?>>판매수량순</option>
            	<option value="ct_price_sum" <?php echo get_selected($sort1, 'ct_price_sum') ; ?>>판매금액순</option>
            </select>
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        $headers = array('순위','대분류','중분류', '소분류','결제수량', '취소수량', '반품수량', '판매수량','판매합계');
        $bodys = array('NO','ca_name1','ca_name2', 'ca_name3','ct_status_1', 'ct_status_2', 'ct_status_3', 'ct_qty_sum', 'ct_price_sum');
        $enc = new str_encrypt();
        ...
        for ($i=0; $row=sql_fetch_array($result); $i++)
        {
            ...
            $bg = 'bg'.($i%2);
            $it_names[] = $row['ca_name3'];
            $tot0[] = $row['ct_qty_sum'];
            $total_qty += (int)$row['ct_qty_sum'];
            
            $tot1[] = $row['ct_price_sum'];
            $total_price += (int)$row['ct_price_sum'];
            
            ?>
            <tr class="<?php echo $bg; ?>">
                <td class="td_num"><?php echo $num; ?></td>
                <td class="td_left"><?php echo $row['ca_name1']; ?></a></td>
                <td class="td_left"><?php echo $row['ca_name2']; ?></a></td>
                <td class="td_left"><?php echo $row['ca_name3']; ?></a></td>
                ...
                tooltips: {
                    callbacks: {
                        label: function(tooltipItem, data) {
                            ...
                            label += number_format(data.datasets[tooltipItem.datasetIndex].data[tooltipItem.index])+"원";
