...
$excel_sql = "select a.*, b.it_id, b.it_name, b.ct_option, b.ct_qty, (b.ct_price + b.io_price)  as opt_price, b.ct_status, b.ct_status_claim, b.rf_serial, b.ct_keep_month
...
$headers = array('NO', '주문일시', '주문번호', '세탁구분', '보관기간', '주문상태', '클레임상태', '주문자아이디', '주문자명', '제품코드', '제품명', '옵션항목', 'RFID', '결제수단', '결제금액', '박스운송장번호', '수거운송장번호', '고객운송장번호', '메모');
$bodys = array('NO', 'od_time', 'disp_od_id', 'ct_free_laundry', 'ct_keep_month', 'od_status', 'ct_status_claim', 'mb_id', 'od_name', 'it_id', 'it_name', 'ct_option', 'rf_serial', 'od_settle_case', 'od_receipt_price', 'str_od_boxsend_invoice', 'str_od_pickup_invoice', 'str_od_invoice', 'od_memo');
			...
			<tr>
				...
				<td colspan="2">
					<div class="col-lg-12 col-md-6 col-sm-12 col-xs-12">
						...
						<input type="radio" name="od_status" value="보관신청" id="od_status_odr" <?= get_checked($od_status, '보관신청'); ?>>
						<label for="od_status_odr">보관요청</label>
						...
						<input type="radio" name="od_status" value="수거중" id="od_status_dvr" <?= get_checked($od_status, '수거중'); ?>>
						<label for="od_status_dvr">수거중</label>
						<input type="radio" name="od_status" value="수거완료" id="od_status_rdy2" <?= get_checked($od_status, '수거완료'); ?>>
						<label for="od_status_rdy2">보관수거완료</label>
						...
						<input type="radio" name="od_status" value="보관중" id="od_status_dvr" <?= get_checked($od_status, '보관중'); ?>>
						<label for="od_status_dvr">보관중</label>
						<br />

						<input type="radio" name="od_status" value="보관완료" id="od_status_done1" <?= get_checked($od_status, '보관완료'); ?>>
						<label for="od_status_done1">보관완료</label>
...
<form name="forderlist" id="forderlist" method="post" autocomplete="off">
	...
	<div class="local_cmd01 local_cmd">
		<div style="float: left">
			<input type="button" value="보관취소(CS)" class="btn btn_02" onclick="forderlist_submit('보관취소');">
			<input type="button" value="보관처리(CS)" class="btn btn_02" onclick="forderlist_submit('보관처리');">
		</div>
		<div style="float: right">
			...
			<input type="button" value="보관주문확인" class="btn btn_02" onclick="forderlist_submit('주문확인');">
		...
		<table id="sodr_list">
			...
			<thead>
				<tr>
					...
					<th scope="col">세탁구분<br />(보관기간)</th>
					<th scope="col">주문상태<br />(클레임상태)</th>
					<th scope="col">제품<br />이미지</th>
					<th scope="col">제품명</th>
				...
				?>
					<tr>
						...
						<td rowspan="<?= $rowspan ?>">
							...
							<?= ($opt['ct_free_laundry_use']) ? "무료" : "유료"; ?><br />
							<label class="sound_only">보관기간</label>
							(<?= $opt['ct_keep_month'] ?>개월)
						</td>
						...
						<td class="td_itopt_tl">
							...
							<?= ($opt['its_order_no']) ? "<br/>옵션명 : " . $opt['its_order_no'] : "" ?>
						...
						<td rowspan="<?= $rowspan ?>">
							...
							<?= number_format($row['od_receipt_price']); ?> 원<br />
							<? if ($row['od_receipt_price']) { ?>
	...
	function forderlist_submit(change_status) {
		...
		switch (change_status) {
			case "보관취소":
				...
				if (current_status != "보관신청") {
					alert("'보관신청' 상태의 주문만 '보관취소'가 가능합니다.");
					return false;
				}
			...
			case "보관처리":
				...
				if (current_status != "세탁중" && current_status != "세탁완료") {
					alert("'세탁중, 세탁완료' 상태의 주문만 '보관처리'가 가능합니다.");
					return false;
				}
			...
			case "주문확인":
				...
				for (var i = 0; i < $chk.size(); i++) {
					...
					if (current_status != "보관신청") {
						alert("'보관신청' 상태의 주문만 '세탁주문확인'이 가능합니다.");
						return false;
					}
				}
			...
			case "배송중":
				...
				for (var i = 0; i < $chk.size(); i++) {
					...
					if (current_status != "보관중") {
						alert("'보관중','보관완료' 상태의 주문만 '배송중'으로 변경 가능합니다.");
						return false;
					}
				}
