...
//$where[] = " od_id in (select od_id from lt_shop_cart, lt_shop_item where lt_shop_cart.it_id = lt_shop_item.it_id and lt_shop_item.ca_id3 = '{$cp['company_code']}') ";
$where[] = " a.company_code = (select company_code from lt_member_company where mb_id = '{$member['mb_id']}' )";
...
if (isset($is_mb) && $is_mb != '') {
	if($is_mb == '1') $where[] = " (mb_id is not null and mb_id != '') ";
	else if($is_mb == '0') $where[] = " (mb_id is null or mb_id = '') ";
} else {
	$is_mb = '';
}
...
$headers = array('NO', '주문일시','주문번호', '주문통합상태','주문자아이디','주문자명', '제품코드', '제품명', '옵션항목', '결제수단','수량','제품금액','총주문금액','취소금액','운송장번호','주문상태','클레임상태','메모');
...
<form name="forderlist" id="forderlist" method="post" autocomplete="off">
	...
	<div class="local_cmd01 local_cmd">
		<div style="float: right">
			...
			<input type="button" value="발송처리" class="btn btn_02" onclick="forderlist_submit('발송처리');">
		...
		<table id="sodr_list">
			...
			<thead>
				<tr>
					...
					<th scope="col"><a href="<?=title_sort("od_id", 1)."&amp;$qstr1"; ?>">주문번호</a></th>
					<!--<th scope="col">회원구분</th>-->
					<th scope="col">주문자명(ID)</th>
					<th scope="col">이미지</th>
					<!--<th scope="col">제품주문번호</th>-->
			...
			<tbody>
				...
				for ($i=0; $row=sql_fetch_array($result); $i++){
					...
					$odsql = " select it_id, it_name, its_sap_code, its_order_no, ct_id, it_id, ct_price, ct_qty, ct_option, ct_status, cp_price, ct_send_cost, io_type, io_price, rf_serial,ct_status_claim, (select group_concat(od_sub_id) from lt_shop_order_item where lt_shop_order_item.ct_id = lt_shop_cart.ct_id ) as od_sub_id from lt_shop_cart where od_id = '".$row['od_id']."' and io_type = '0' order by io_type asc, ct_id asc";
					$od = sql_query($odsql);
					...
					for($k=0; $opt=sql_fetch_array($od); $k++) {
						...
						<tr>
							<? if($k == 0) { ?>
								...
								<td headers="th_ordnum" rowspan="<?=$rowspan ?>">
								...
								<td headers="th_odrer" class="td_name" rowspan="<?=$rowspan ?>">
									<label class="sound_only">주문자명</label>
									<?=$row['od_name']; ?><br/>(<?=$row['mb_id']; ?>)
									<br/><?=get_text($row['od_tel']); ?>
								</td>
							<? } ?>
							<td headers="td_odrnum2" class="td_odrnum2">
								<a href="./itemform.brand.php?w=u&amp;it_id=<?=$opt['it_id']; ?>"><?=$image; ?></a>
							</td>
							<td class="td_itopt_tl">
								<label class="sound_only">제품코드</label>
								제품코드 : <?=$opt['it_id'] ?>
								<?=($opt['its_order_no'])?"<br/>(".$opt['its_order_no'].")":"" ?><br/>
								<label class="sound_only">제품명</label>
								<a href="./itemform.brand.php?w=u&amp;it_id=<?=$opt['it_id']; ?>"><?=stripslashes($opt['it_name']); ?></a><br/>
								옵션 : <?=$opt['ct_option']; ?>
							</td>
							<? if($k == 0) { ?>
								<td headers="odrpay" class="odrpay" rowspan="<?=$rowspan ?>">
							...
							<? if($k == 0) { ?>
								<td class="td_num_right" rowspan="<?=$rowspan ?>">
									<label class="sound_only">총결제금액</label>
									<?=number_format($row['od_receipt_price']); ?>
									<?=((int)$row['od_refund_price']>0)?'<br/><b class="red">('.number_format((-1)*(int)$row['od_refund_price']).')</b>':''; ?>
								</td>
							<? } ?>
							<td headers="delino" class="delino">
								...
								if($row['od_invoice']) {
									...
									echo (is_null_time($row['od_invoice_time']) ? '-' : substr($row['od_invoice_time'],0,16)).'<br/>';
	...
	function forderlist_submit(change_status)
	{
		...
		switch (change_status)
		{
			...
			if (current_status != "결제완료") {
				alert("'결제완료' 상태의 주문만 '주문취소'가 가능합니다.");
				return false;
			}
			...
			case "발송처리" :
			 ...
			 //window.open("./orderdelivery.php", "win_excel", opt);
			 if($chk.size() != 1){
			 	alert("CS처리는 다중 선택할 수 없습니다.");
			 	return false;
			 }
			 ...
			 if (current_status != "상품준비중") {
			 	alert("'상품준비중' 상태의 주문만 '발송처리' 가능합니다.");
			 	return false;
			 }

			 var url = "./orderpartdelivery.php?od_type=<?=$od_type; ?>&od_id="+$("input[name='od_id["+k+"]']").val();
			 window.open(url, "orderpartdelivery", "left=230,top=100,width=1000,height=650,scrollbars=yes,resizable=yes");
			 break;
			 case "배송완료" :
