...
$sql  = " select *,
            ...
            (select sh_memo from lt_shop_order_history where od_id = a.od_id and ct_status_claim = '철회' limit 1) as sh_memo
           $sql_common
...
$excel_sql = "select a.*, b.it_id, b.it_name, b.ct_option, b.ct_qty, (b.ct_price + b.io_price)  as opt_price, b.ct_status, b.ct_status_claim
    ...
    from lt_shop_order as a
         left join lt_shop_cart as b
            on a.od_id = b.od_id and b.io_type = '0' and b.ct_status_claim in ( '철회' )
         left join lt_shop_order_history as c
            on a.od_id = c.od_id and c.ct_status_claim = '철회'
            and sh_id in (select max(sh_id) from lt_shop_order_history where ct_status_claim = '철회' group by od_id)
    $sql_search
...
$headers = array('NO', '주문일시','주문번호', '주문상태','주문자아이디','주문자명', '상품코드', '상품명', '옵션항목', '수량','배송추적','요청일','사유');
$bodys = array('NO',  'od_time','disp_od_id', 'od_status','mb_id','od_name', 'it_id', 'it_name', 'ct_option', 'ct_qty','str_od_invoice','od_status_claim_date','sh_memo');
...
<input type="hidden" name="claimtype" value="<?php echo $claimtype; ?>">
    ...
    <tr>
		...
		<div class="col-lg-12 col-md-6 col-sm-12 col-xs-12">
            ...
            <input type="radio" name="od_status" value="철회요청" id="od_status_income" <?php echo get_checked($od_status, '철회요청'); ?>>
            <label for="od_status_income">철회요청</label>
            <input type="radio" name="od_status" value="철회수거중" id="od_status_rdy" <?php echo get_checked($od_status, '철회수거중'); ?>>
            <label for="od_status_rdy">철회수거중</label>
            <input type="radio" name="od_status" value="수거완료" id="od_status_dvr" <?php echo get_checked($od_status, '수거완료'); ?>>
            <label for="od_status_dvr">철회수거완료</label>
            <input type="radio" name="od_status" value="철회완료" id="od_status_done" <?php echo get_checked($od_status, '철회완료'); ?>>
            <label for="od_status_done">철회완료</label>
		</div>
		</td>
	</tr>
...
<div class="local_cmd01 local_cmd">
	...
	<input type="button" value="철회승인(택배사 수거요청)" class="btn btn_02" onclick="forderlist_submit('철회승인');">
	<input type="button" value="수거완료처리" class="btn btn_02" onclick="forderlist_submit('수거완료');">
	<input type="button" value="철회거부처리" class="btn btn_02" onclick="forderlist_submit('철회거부');">
	<input type="button" value="철회취소처리(CS)" class="btn btn_02" onclick="forderlist_submit('철회취소');">
	<input type="button" value="철회완료" class="btn btn_02" onclick="forderlist_submit('철회완료');">
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
        ...
        <td headers="delino" class="delino" rowspan="<?php echo $rowspan ?>">
            ...
            <?php 
            if($row['od_invoice'] && $row['od_status'] != '철회요청') {
                echo "<a href='https://www.doortodoor.co.kr/parcel/doortodoor.do?fsp_action=PARC_ACT_002&fsp_cmd=retrieveInvNoACT&invc_no='".$row['od_invoice']." target='_blank'>";
        ...
        <td class="td_name" rowspan="<?php echo $rowspan ?>">
        	<label class="sound_only">사유</label>
        	<?php echo $row['sh_memo'] ?>
        </td>
        <?php } ?>
...
function forderlist_submit(change_status)
{
	...
	switch (change_status)
    {
         case "철회승인" :
             ...
             if (current_status != "철회요청") {
                 alert("'철회요청' 상태의 주문만 '철회승인'이 가능합니다.");
                 return false;
             }
             var url = "./orderpartreturn_r1.php?change_status="+change_status+"&od_id="+$("input[name='od_id["+k+"]']").val();
         ...
         case "철회거부" :
             ...
             if (current_status != "철회요청") {
                 alert("'철회요청' 상태의 주문만 '철회거부'가 가능합니다.");
                 return false;
             }
             var url = "./orderpartreturn_r1.php?change_status="+change_status+"&od_id="+$("input[name='od_id["+k+"]']").val();
             window.open(url, "orderpartreturn", "left=230,top=100,width=1000,height=650,scrollbars=yes,resizable=yes");
         ...
         case "철회취소" :
             ...
             if (current_status != "철회요청") {
                 alert("'철회요청' 상태의 주문만 '철회취소'가 가능합니다.");
                 return false;
             }
             var url = "./orderpartreturn_r1.php?change_status="+change_status+"&od_id="+$("input[name='od_id["+k+"]']").val();
             window.open(url, "orderpartreturn", "left=230,top=100,width=1000,height=650,scrollbars=yes,resizable=yes");
         ...
         case "수거완료" :
        	...
        	for (var i=0; i<$chk.size(); i++)
			{
                 ...
                 if (current_status != "철회수거중") {
                     alert("'철회수거중' 상태의 주문만 '수거완료'가 가능합니다.");
                     return false;
                 }
			}
         ...
         case "철회완료" :
             ...
             if (current_status != "수거완료") {
                 alert("'수거완료' 상태의 주문만 '철회완료'가 가능합니다.");
                 return false;
             }
             var url = "./orderpartreturn_r2.php?change_status="+change_status+"&od_id="+$("input[name='od_id["+k+"]']").val();
             window.open(url, "orderpartreturn", "left=230,top=100,width=1000,height=650,scrollbars=yes,resizable=yes");
