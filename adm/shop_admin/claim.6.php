...
$html[] = "<input type='hidden' name='claim_type' value='6'>";

$html[] = "<div>계약 상품목록</div>
            <table>
			<tbody>
				<tr>
                    <th scope='row'>해지유형선택</th>
...
for ($i = 0; $order = sql_fetch_array($db_order); $i++) {
    ...
    if ($i == 0) {
        $html[] = "<tr>
        <td rowspan={$db_order->num_rows}><select id='claim-reaason' onchange='change_reason(this)'><option name='claim_reason' value='0'>일반해지</option><option name='claim_reason' value='1'>분실/파손</option><option name='claim_reason' value='2'>관리자 입력</option></select></td>";
    } else {
        $html[] = "<tr>";
    }
    $html[] = "<td>{$disp_od_id}
...
$html[] = "<div>계약 해지내용</div><table><tbody>";
$html[] = "<tr><th style='width: 109px;'>해지유형</th><td id='claim-reason'>일반해지</td></tr>";
...
$return['title'] = "해지요청(CS)";
$return['desc'] = "계약번호 : " . get_disp_id($rt_id, 'T');
$return['footer'] = "<button type='button' class='btn btn-default' data-dismiss='modal'>닫기</button><button type='submit' class='btn btn-success' id='btn-claim-cancel-rental'>해지요청</button>";
