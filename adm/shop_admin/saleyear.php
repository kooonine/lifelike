...
if (!$fr_year) {
    ...
    date_add($fr_date, date_interval_create_from_date_string('-1 years'));
    $fr_year = date_format($fr_date,"Y");

    $to_year = substr(G5_TIME_YMD,0,4);
}
...
if(isset($act) && $act == "exceldownload"){
    ...
    $headers = array('NO','주문연도','리스 주문수','제품 주문수','세탁 주문수','세탁보관 주문수','수선 주문수','주문수 총 합계','(총)주문금액','(총)박스비','(총)배송비','쿠폰 할인','적립금사용'
        ,'리스 결제합계','제품 결제합계','세탁 결제합계','세탁보관 결제합계','수선 결제합계','결제 총 합계','할인합계','주문취소','환불금액','미수금','일반위약금','파손위약금','분실위약금');
    
    $bodys = array('NO','od_date','R_count','O_count','L_count','K_count','S_count','ordercount','orderprice','zbox_price','send_cost','ordercoupon','receiptpoint'
        ,'R','O','L','K','S','receipt_price','discountprice','ordercancel','refund_price','misu','','','');
...
function print_line($save)
{
    ...
    ?>
<tr>
    <td class="td_alignc"><?php echo $save['od_date']; ?></td>

    <td class="td_num"><?php echo number_format($save['R_count']); ?></td>
    <td class="td_num"><?php echo number_format($save['O_count']); ?></td>
    <td class="td_num"><?php echo number_format($save['L_count']); ?></td>
    <td class="td_num"><?php echo number_format($save['K_count']); ?></td>
    <td class="td_num"><?php echo number_format($save['S_count']); ?></td>
    <td class="td_num"><?php echo number_format($save['ordercount']); ?></td>

    <td class="td_numsum"><?php echo number_format($save['orderprice']); ?></td>
    <td class="td_numincome"><?php echo number_format($save['zbox_price']); ?></td>
    ...
    <td class="td_numincome"><?php echo number_format($save['R']); ?></td>
    <td class="td_numincome"><?php echo number_format($save['O']); ?></td>
    <td class="td_numincome"><?php echo number_format($save['L']); ?></td>
    <td class="td_numincome"><?php echo number_format($save['K']); ?></td>
    <td class="td_numincome"><?php echo number_format($save['S']); ?></td>
...
<div class="tbl_head01 tbl_wrap">
    ...
    <thead>
    <tr>
        <th scope="col" rowspan="2">주문연도</th>

        <th scope="col" colspan="6">주문수</th>

        <th scope="col" rowspan="2">(총)주문금액</th>
        <th scope="col" rowspan="2">(총)박스비</th>
        <th scope="col" rowspan="2">(총)배송비</th>

        <th scope="col" rowspan="2">쿠폰 할인</th>
        <th scope="col" rowspan="2">적립금사용</th>

        <th scope="col" colspan="6">결제합계</th>
        <th scope="col" rowspan="2">할인합계</th>

        <th scope="col" rowspan="2">주문취소</th>
        <th scope="col" rowspan="2">환불금액</th>
        <th scope="col" rowspan="2">미수금</th>

        <th scope="col" colspan="3">위약금</th>
    </tr>
    <tr>
        <th scope="col">리스</th>
        <th scope="col">제품</th>
        <th scope="col">세탁</th>
        <th scope="col">세탁보관</th>
        <th scope="col">수선</th>
        <th scope="col">합계</th>
