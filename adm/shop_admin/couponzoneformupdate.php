...
if ($_POST['cp_point_coupon_check'] == 1) {
    $_POST['cp_method'] = 31;
    $_POST['cz_period'] = 15;
    $_POST['cz_download_user_limit'] = 1;

} else {

    if (($_POST['cp_method'] == 0 || $_POST['cp_method'] == 4) && !$_POST['cp_target'])
    ...
    if ($_POST['cp_method'] == 0 || $_POST['cp_method'] == 4) {
        $sql = " select count(*) as cnt from {$g5['g5_shop_item_table']} where it_id IN ($cp_target) and it_nocoupon = '0' ";
        $row = sql_fetch($sql);
        if (!$row['cnt'])
...
$sql_common = " cz_subject  = '{$_POST['cz_subject']}',
                cz_desc     = '{$_POST['cz_desc']}',
                cz_type     = '{$_POST['cz_type']}',
                cz_start    = '{$_POST['cz_start']}',
                cz_end      = '{$_POST['cz_end']}',
                cz_period   = '{$_POST['cz_period']}',
                cz_point    = '{$_POST['cz_point']}',
                cz_weekday  = '" . implode(',', $_POST_ORG['cz_weekday']) . "',
                cz_week     = '" . implode(',', $_POST_ORG['cz_week']) . "',
                cp_brand    = '{$_POST['cp_brand']}',
                ...
                cz_download_limit = '{$_POST['cz_download_limit']}' ,
                cz_download_user_limit = '{$_POST['cz_download_user_limit']}',
                cp_point_coupon_amount = '{$_POST['cp_point_coupon_amount']}',
                cp_point_coupon_check = '{$_POST['cp_point_coupon_check']}' ";
...
if ($w == '') {
    ...
    if ($_POST['cp_point_coupon_check'] != 1) {
        $updateSql = " UPDATE lt_shop_coupon SET cp_target = '{$_POST['cp_target']}' WHERE cz_id = '$cz_id' AND od_id = 0 ";
        sql_query($updateSql);
    }
}
