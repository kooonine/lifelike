...
<div class="row">
	<div class="col-md-12 col-sm-12 col-xs-12">
		<div class="x_panel">
			<div class="" role="tabpanel" data-example-id="togglable-tabs">
				<ul id="myTab" class="nav nav-tabs bar_tabs" role="tablist">
                    <?for ($i = 0 ; $cp_Row = sql_fetch_array($cp_res); $i++) { ?>
                        <li role="presentation" class="<?= ($cp_code == $cp_Row['cp_code'] ? "active" : "") ?>"><a href="./b2b_order.php?cp_code=<?=$cp_Row['cp_code']?>"><?=$cp_Row['cp_name']?></a></li>
                    <?}?>
			...
			<?
			include_once('./b2b_order_detail.php');
			?>
		</div>
	</div>
</div>
