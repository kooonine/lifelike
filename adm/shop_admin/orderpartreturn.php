...
<div class="row">
  <div class="col-md-12 col-sm-12 col-xs-12">
	...
	<div class="x_content">
        ...
        <input type="hidden" id="od_id" name="od_id" value="<?php echo $od_id; ?>">
        <input type="hidden" id="return_price" name="return_price" value="">
        <input type="hidden" id="return_price_send" name="return_price_send" value="">
    	...
    	<div class="tbl_head01 tbl_wrap">
            ...
            for($i=0; $row=sql_fetch_array($result); $i++) {
                ...
                for($k=0; $opt=sql_fetch_array($res); $k++) {
                    ...
                    if($ct_price['stotal'] < 30000) {
                        $ct_price['stotal'] += 3000;
                    }
                ?>
                <tr>  	
                    <td class="td_chk" >
                    ...
                    <td class="td_cntsmall">
                     <? echo $opt['ct_qty'] ?>
                     <input type="hidden" name="ct_qty[<?php echo $cnt ?>]" value="<?php echo $opt['ct_qty'] ?>" id="ct_qty_<?php echo $cnt ?>">
        ...
        <div class="tbl_frm01 tbl_wrap">
            ...
            <tr>
                ...
                <th scope="row"><label >반품차액 환불처리</label></th>
            </tr>
            <tr>
                <td><input type="number" name="od_send_cost2" id="od_send_cost2" value="<?php echo $de_return_costs; ?>" class="frm_input" size="6"></td>
                <td><span id="cancel_price" data-value="<?php echo ($od['od_receipt_price'])?>"><?php echo number_format($od['od_receipt_price']); ?> 원</span></td>
                <td>
                    <h4><span id="tot_cancel_price" name ="tot_cancel_price"></span></h4>
                    <!-- <h4><span id="tot_cancel_price"><?php echo number_format((int)$od['od_receipt_price']-(int)$de_return_costs); ?> 원</span></h4> -->
                </td>
            </tr>
            </tbody>
            </table>
    	...
    	<div class="x_content">
    		<div class="form-group">
    			<div class="col-md-12 col-sm-12 col-xs-12 text-right">
            		...
            		<button type="submit" class="btn btn-success" id="btnConfirm">반품승인</button>
            	</div>
            </div>
        </div>
...
function cancel_price_chk(){
    ...
    var cancelpriceValue = document.querySelector('#cancel_price').dataset.value;    
	$("#cancel_price").html(number_format(cancelpriceValue) + " 원");		
	// $("#cancel_price").html(number_format(cancel_price) + " 원");		
    
	tot_cancel_price = cancelpriceValue;

	if(cancel_send_costs > 0){
		//배송비 유료 환불조건
		tot_cancel_price = cancelpriceValue - cancel_send_costs;
	} 
    let odId = $("#od_id").val();
    let data = {
        action: 'return',
    ...
    $.get("/shop/ajax.order.cancel.php", data, function(response) {        
        if (response.result) {
            $("#tot_cancel_price").text(number_format(response.data.cancel.pieceRefund-cancel_send_costs) + "원");
            $("#return_price").val(response.data.cancel.pieceRefund);  
            $("#return_price_send").val(cancel_send_costs);  
        } else {
            return;
        }
    }, "JSON");
