...
$types = array(
	1 => array('current' => '계약등록', 'next' => '상품준비중'),
	2 => array('current' => '상품준비중', 'next' => '배송중'),
	3 => array('current' => '배송중', 'next' => '배송완료'),
	4 => array('current' => '계약등록', 'next' => '계약취소'),
	5 => array('current' => '상품준비중', 'next' => '주문철회'),
	6 => array('current' => '리스중', 'next' => '해지요청'),
);
...
foreach ($_POST['cb_claim'] as $i => $od_id) {
	...
	$change_status  = $types[$type]['next'];
	$od_type = $od['od_type'];

	// 배송정보가 있으면 배송정보 업데이트 - 200107 balance@panpacific.co.kr
	$od_id_key = (int) $od_id;
	if (!empty($_POST['od_invoice'][$od_id_key])) {
		$delivery['invoice_time'] = G5_TIME_YMDHIS;
		$delivery['invoice'] = $_POST['od_invoice'][$od_id_key];
		$delivery['delivery_company'] = $_POST['od_delivery_company'][$od_id_key];
	...
	switch ($current_status) {
		...
		case '계약등록':
			if ($change_status != '상품준비중') continue;
			change_status($od_id, '계약등록', '상품준비중');
		...
		case '수선중':
			...
			break;
		case '리스중':
			if ($change_status != '해지요청') continue;
			...
			$sql_rental = "SELECT rt_id FROM {$g5['g5_shop_rental_order_table']} WHERE od_id='{$od_id}'";
			$rt = sql_fetch($sql_rental);

			$sql = " update {$g5['g5_shop_rental_table']} set rt_status = '{$change_status}' where rt_id = '{$rt['rt_id']}' and rt_status = '{$current_status}' ";
			sql_query($sql, true);
