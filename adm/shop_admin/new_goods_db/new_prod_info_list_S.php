...
$left_on_sql = "SELECT tc.* , infos.normal , infos.project FROM (
    ...
    (SELECT (CASE WHEN  ps_code_gubun IN ('MA','MW') THEN '오프라인'  WHEN ps_code_gubun IN ('MO','MD','MS','MX') THEN '온라인'  END ) AS on_off,
     aa.* FROM lt_prod_schedule AS aa WHERE ps_display = 'Y' AND ps_code_year >= 21 AND ps_code_item_type = 'C'   GROUP BY ps_item_nm ) AS lps
...
$left_off_sql = "SELECT tc.* , infos.normal , infos.project FROM (
    ...
    FROM 
    (SELECT (CASE WHEN  ps_code_gubun IN ('MA','MW') THEN '오프라인'  WHEN ps_code_gubun IN ('MO','MD','MS','MX') THEN '온라인'  END ) AS on_off,
     aa.* FROM lt_prod_schedule AS aa WHERE ps_display = 'Y' AND ps_code_year >= 21 AND ps_code_item_type = 'C' AND ps_shooting_yn ='Y'  GROUP BY ps_item_nm ) AS lps
    
    WHERE lps.ps_code_year IS NOT NULL AND lps.ps_code_season <> '' AND lps.ps_display = 'Y' AND lps.on_off = '오프라인' AND lps.ps_code_brand = 'S'
    GROUP BY lps.ps_code_year , lps.ps_code_season , lps.on_off 
    ORDER BY lps.on_off DESC , lps.ps_code_year DESC , lps.ps_code_season ASC
    ) AS infos
    RIGHT JOIN (
...
if(!empty($item_nm) || (!empty($year) && !empty($season))){
    $sql_where = "WHERE (1) AND ps_code_item_type = 'C'  AND ps_code_brand ='S'   ";
...
<div id="left_cate" >
    ...
    <form id="new_prod_info"  method="post" action="./new_prod_info_list.php?od_type=S" >
    ...
    <div id="on-content" style="<?if($onoff == 'ON'):?>display:block; <?endif?>">
        ...
        ?>
            <?if($left_on_row['on_off'] == '온라인'):?>
                ...
                <div><a href="./new_prod_info_list.php?od_type=S&year=<?=$left_on_row['ps_code_year']?>&season=A&onoff=ON">AA 정상 <?=$left_on_row['normal'] ? $left_on_row['normal'] : '0'?>건 기획 <?=$left_on_row['project']? $left_on_row['project'] : '0'?>건</a></div>
                <?endif?>
                <?if($left_on_row['ps_code_season'] == 'S'):?><div><a href="./new_prod_info_list.php?od_type=S&year=<?=$left_on_row['ps_code_year']?>&season=S&onoff=ON">SS 정상 <?=$left_on_row['normal'] ? $left_on_row['normal'] : '0'?>건 기획 <?=$left_on_row['project']? $left_on_row['project'] : '0'?>건</a></div><?endif?>
                <?if($left_on_row['ps_code_season'] == 'H'):?><div><a href="./new_prod_info_list.php?od_type=S&year=<?=$left_on_row['ps_code_year']?>&season=H&onoff=ON">HS 정상 <?=$left_on_row['normal'] ? $left_on_row['normal'] : '0'?>건 기획 <?=$left_on_row['project']? $left_on_row['project'] : '0'?>건</a></div><?endif?>
                <?if($left_on_row['ps_code_season'] == 'F'):?><div><a href="./new_prod_info_list.php?od_type=S&year=<?=$left_on_row['ps_code_year']?>&season=F&onoff=ON">FW 정상 <?=$left_on_row['normal'] ? $left_on_row['normal'] : '0'?>건 기획 <?=$left_on_row['project']? $left_on_row['project'] : '0'?>건</a></div><?endif?>
            <?endif?>
    ...
    <div id="off-content" style="<?if($onoff == 'OFF'):?>display:block; <?endif?>">
        ...
        ?>
            <?if($left_off_row['on_off'] == '오프라인'):?>
                ...
                <div><a href="./new_prod_info_list.php?od_type=S&year=<?=$left_off_row['ps_code_year']?>&season=A&onoff=OFF">AA 정상 <?=$left_off_row['normal']? $left_off_row['normal'] : '0' ?>건 기획 <?=$left_off_row['project'] ? $left_off_row['project'] : '0'?>건</a></div>
                <?endif?>
                <?if($left_off_row['ps_code_season'] == 'S'):?><div><a href="./new_prod_info_list.php?od_type=S&year=<?=$left_off_row['ps_code_year']?>&season=S&onoff=OFF">SS 정상 <?=$left_off_row['normal']? $left_off_row['normal'] : '0' ?>건 기획 <?=$left_off_row['project'] ? $left_off_row['project'] : '0'?>건</a></div><?endif?>
                <?if($left_off_row['ps_code_season'] == 'H'):?><div><a href="./new_prod_info_list.php?od_type=S&year=<?=$left_off_row['ps_code_year']?>&season=H&onoff=OFF">HS 정상 <?=$left_off_row['normal']? $left_off_row['normal'] : '0' ?>건 기획 <?=$left_off_row['project'] ? $left_off_row['project'] : '0'?>건</a></div><?endif?>
                <?if($left_off_row['ps_code_season'] == 'F'):?><div><a href="./new_prod_info_list.php?od_type=S&year=<?=$left_off_row['ps_code_year']?>&season=F&onoff=OFF">FW 정상 <?=$left_off_row['normal']? $left_off_row['normal'] : '0' ?>건 기획 <?=$left_off_row['project'] ? $left_off_row['project'] : '0'?>건</a></div><?endif?>
            <?endif?>
        <?}?>
    </div>
</div>
