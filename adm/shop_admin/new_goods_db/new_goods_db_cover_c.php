			...
			<tr>
				...
				<div class="col-lg-12 col-md-6 col-sm-12 col-xs-12">
					...
					<select name="cm_season_jo" id="cm_season_jo" required oninvalid="this.setCustomValidity('시즌를 선택해주세요')" oninput="setCustomValidity('')">
						...
						<option value="AA" <?= get_selected($cm_season_jo, 'AA'); ?>>AA</option>
					</select>
...
<form name="forderlist" id="forderlist" method="post" autocomplete="off">
	<div class="local_cmd01 local_cmd">
		<div style="float: left">
			...
			<input type="button" value="삭제" class="btn btn_02" onclick="delCoverDate()">
		</div>
	</div>
				...
				<tr class="data_row row_<?=$row['cm_id']?> noExport">
					...
					<td>
						<select name='cm_scent[]' id="cm_scent_<?= $row['cm_id'] ?>">
							<option class="noExport" value="" <?= get_selected($row['cm_scent'], ''); ?>>선택</option>
							<option class="noExport" value="향균" <?= get_selected($row['cm_scent'], '향균'); ?>>향균</option>
							<option class="noExport" value="비향균" <?= get_selected($row['cm_scent'], '비향균'); ?>>비향균</option>
						</select>
						<!-- <p id="cm_scents_<?= $row['cm_id'] ?>" style="display : none;" > <?=$row['cm_scent']?> </p> -->
					</td>
					<td>
						<?= $row['merge_nabgi']?>
					</td>
...
<script>
	...
	function delCoverDate(){
		...
		var check = confirm("일정에서 삭제하시겠습니까?"); 
        if(check){
			$.ajax({
				cache : false,
				url : "./ajax.new_goods_db_cover_c_delete.php", // 요기에
				type : 'POST', 
				// dataType : 'json',
				data : {selects_id : selects},
				success : function(data) {
					// console.log(data);
					if(data == 200){alert("삭제되었습니다.");}
	...
	function down_excel_c(){
		...
		$("input[name='chk[]']:checked").each(function (index) { 
			...
			$("#cm_scents_"+this.value).css("display","block");
		});
