...
$sql_search = " where (1) AND item_type = 'C'  ";
...
// 테이블의 전체 레코드수만 얻음
$cnt_sql = "SELECT COUNT(*) AS cnt FROM  new_goods_db_cover {$sql_search}";
...
$new_goods_db_cover = "select * from new_goods_db_cover  {$sql_search} order by no desc limit $from_record, $rows ";
$ovs_sql = sql_query($new_goods_db_cover);
$ovs_db_data = sql_fetch($new_goods_db_cover);
...
<form name="new_goods_form" id="new_goods_form" onsubmit="" method="post">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            <th scope="row">판매량기간설정1</th>
            <td colspan="2">
            <div class="col-lg-3 col-md-2 col-sm-3 col-xs-2">
                <input type='text' class="form-control" id="sale_date1" name="sale_date1" value="" autocomplete="off"/>
                <i class="glyphicon glyphicon-calendar fa fa-calendar" style="position: absolute;bottom: 10px;right: 24px;top: auto;cursor: pointer;"></i>
            </div>
            </td>
        </tr>
        <tr>
            <th scope="row">판매량기간설정2</th>
            <td colspan="2">
            <div class="col-lg-3 col-md-2 col-sm-3 col-xs-2">
                <input type='text' class="form-control" id="sale_date2" name="sale_date2" value="" autocomplete="off"/>
                <i class="glyphicon glyphicon-calendar fa fa-calendar" style="position: absolute;bottom: 10px;right: 24px;top: auto;cursor: pointer;"></i>
            </div>
            </td>
        </tr>
...
<form name="fwrite" id="fwrite" action="./update_new_goods_db_cover.php" method="post" enctype="multipart/form-data" autocomplete="off">
    <div>
    ...
    <table id="reportTb" name="reportTb" class="display" style="width:100%">
        ...
        <thead>
            <tr>
                ...
                <th colspan = "3">판매량 기간설정 1</th>
                <th colspan = "3" style="position:relative;" onclick="all_sale(event)">판매량 기간설정 2 </th>
        ...
        <tbody id="revenue-status">
            ...
            <tr class="data_row row_<?=$i?> noExport" data-item-idx=<?=$i?>>
                ...
                $sqlSamjin = "SELECT TOP 1 ORDER_NO, SAP_CODE, ITEM , WONGA FROM S_MALL_ORDERS WHERE SAP_CODE = '{$ovs_row['samjin_code']}' AND COLOR = '{$ovs_row['color']}' ";
                ...
                for ($k = 0; $samrow = mssql_sql_fetch_array($rsSamjin); $k++) {  
                    $cov_samjin_name = $samrow['ITEM'];
                    $cov_samjin_code = $samrow['ORDER_NO'];
                    $cov_sap_code = $samrow['SAP_CODE'];
                    $cov_samjin_wonga = $samrow['WONGA'];
                }
                
                if(!empty($sale_date1)){
                    
                    $f_date = explode('~',$sale_date1);
                    $rep_sale_sum1 = NM_REP_SALE_SUM(trim($f_date[0]),trim($f_date[1]),2,$cov_samjin_code,$cov_samjin_code,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL);
                    if(count($rep_sale_sum1) > 0){
                        for($a=0 ;  $a< count($rep_sale_sum1); $a++){
                            if($ovs_row['color'] == trim($rep_sale_sum1[$a]['COLOR'])){
                                
                                if(trim($rep_sale_sum1[$a]['HOCHING'])=='Q'){
                                    $rep_sale_sum1_q += $rep_sale_sum1[$a]['QTY'];
                                }else if(trim($rep_sale_sum1[$a]['HOCHING'])=='K'){
                                    $rep_sale_sum1_k += $rep_sale_sum1[$a]['QTY'];
                                }else{
                                    $rep_sale_sum1_s += $rep_sale_sum1[$a]['QTY'];
                                }
                            }
                        }
                    }
                ...
                if(!empty($sale_date2)){
                    $t_date = explode('~',$sale_date2);
                    $rep_sale_sum2 = NM_REP_SALE_SUM(trim($t_date[0]),trim($t_date[1]),2,$cov_samjin_code,$cov_samjin_code,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL);
                    if(count($rep_sale_sum2) > 0){
                        for($b=0 ;  $b< count($rep_sale_sum2); $b++){
                            if($ovs_row['color'] == trim($rep_sale_sum2[$b]['COLOR'])){
                                if(trim($rep_sale_sum2[$b]['HOCHING'])=='Q'){
                                    $rep_sale_sum2_q += $rep_sale_sum2[$b]['QTY'];
                                }else if(trim($rep_sale_sum2[$b]['HOCHING'])=='K'){
                                    $rep_sale_sum2_k += $rep_sale_sum2[$b]['QTY'];
                                }else{
                                    $rep_sale_sum2_s += $rep_sale_sum2[$b]['QTY'];
                                }
                            }
                        }
                    }
                ...
                ?>
                <td><?=$rep_sale_sum1_s?></td>
                <td><?=$rep_sale_sum1_q?></td>
                <td><?=$rep_sale_sum1_k?></td>
                <td><?=$rep_sale_sum2_s?></td>
                <td><?=$rep_sale_sum2_q?></td>
                <td><?=$rep_sale_sum3_k?></td>
...
<script>
    ...
    $('#sale_date1').val("<?php echo $sale_date1 ?>");
    $('#sale_date2').daterangepicker({
        "autoApply": true,
        "opens": "right",
        locale: {
            "format": "YYYY-MM-DD",
    ...
    $('#sale_date2').val("<?php echo $sale_date2 ?>");
    ...
    function all_down_excel(){
        ...
        excel_sql = "select * from new_goods_db_cover order by no desc";
        ...
        var $form = $('<form></form>');     
        $form.attr('action', '<?=G5_ADMIN_URL?>/ajax.excel_download.new_goods_db_cover.php');
