...
$sql  = " select *,
            ...
            (select sh_memo from lt_shop_order_history where od_id = a.od_id and ct_status_claim = '반품' limit 1) as sh_memo
           $sql_common
...
$excel_sql = "select a.*, b.it_id, b.it_name, b.ct_option, b.ct_qty, (b.ct_price + b.io_price)  as opt_price, b.ct_status, b.ct_status_claim
                ...
                from lt_shop_order as a
                     left join lt_shop_cart as b
                        on a.od_id = b.od_id and b.io_type = '0' and b.ct_status_claim in ( '반품' )
                     left join lt_shop_order_history as c
                        on a.od_id = c.od_id and c.ct_status_claim = '반품'
                        and sh_id in (select max(sh_id) from lt_shop_order_history where ct_status_claim = '반품' group by od_id)
                $sql_search
...
$headers = array('NO', '회원구분', '주문일시', '주문번호', '주문상태', '주문자아이디', '주문자명', '상품코드', '상품명', '옵션항목', '수량', '배송추적', '요청일', '사유');
$bodys = array('NO', 'ismember', 'od_time', 'disp_od_id', 'ct_status', 'mb_id', 'od_name', 'it_id', 'it_name', 'ct_option', 'ct_qty', 'str_od_invoice', 'od_status_claim_date', 'sh_memo');
...
$qstr = "$qstr1&amp;sort1=$sort1&amp;sort2=$sort2&amp;page=$page&amp;claimtype=$claimtype";
            ...
            <tr>
                ...
                <td colspan="2">
                    <div class="col-lg-12 col-md-6 col-sm-12 col-xs-12">
                        ...
                        <input type="radio" name="od_status" value="반품요청" id="od_status_income" <?php echo get_checked($od_status, '반품요청'); ?>>
                        <label for="od_status_income">반품요청</label>
                        <input type="radio" name="od_status" value="반품수거중" id="od_status_rdy" <?php echo get_checked($od_status, '반품수거중'); ?>>
                        <label for="od_status_rdy">반품수거중</label>
                        <input type="radio" name="od_status" value="수거완료" id="od_status_dvr" <?php echo get_checked($od_status, '수거완료'); ?>>
                        <label for="od_status_dvr">반품수거완료</label>
                        <input type="radio" name="od_status" value="반품완료" id="od_status_done" <?php echo get_checked($od_status, '반품완료'); ?>>
                        <label for="od_status_done">반품완료</label>
                    </div>
                </td>
            </tr>
...
<form name="forderlist" id="forderlist" method="post" autocomplete="off">
    ...
    <div class="local_cmd01 local_cmd">
        <div style="float: right">
            <input type="button" value="반품승인(택배사 수거요청)" class="btn btn_02" onclick="forderlist_submit('반품승인');">
            <input type="button" value="수거완료처리" class="btn btn_02" onclick="forderlist_submit('수거완료');">
            <input type="button" value="반품거부처리" class="btn btn_02" onclick="forderlist_submit('반품거부');">
            <input type="button" value="반품철회처리(CS)" class="btn btn_02" onclick="forderlist_submit('반품철회');">
            <input type="button" value="반품완료" class="btn btn_02" onclick="forderlist_submit('반품완료');">
            ...
            <tbody>
                ...
                for ($i = 0; $row = sql_fetch_array($result); $i++) {
                    ...
                    $odsql = " select it_id, od_sub_id, it_name, its_sap_code, its_order_no,
                        ...
                        ,ct_delivery_company,ct_invoice
            from {$g5['g5_shop_cart_table']} 
            where od_id = '" . $row['od_id'] . "'
                  and io_type = '0'
                and ct_status_claim in ( '반품' )
                        ...
                        <tr>
                            <?php if ($k == 0) { ?>
                                ...
                                <td headers="odrstat" class="odrstat" rowspan="<?php echo $rowspan ?>">
                                    ...
                                    <input type="hidden" name="current_status_ct[<?php echo $i ?>]" value="<?php echo $opt['ct_status'] ?>">
                                </td>
                                <td rowspan="<?php echo $rowspan ?>">
    ...
    function forderlist_submit(change_status) {
        ...
        switch (change_status) {
            case "반품승인":
                ...
                var k = $($chk[0]).val();
                var current_status = $("input[name='current_status_ct[" + k + "]']").val();

                if (current_status != "반품요청") {
                    alert("'반품요청' 상태의 주문만 '반품승인'이 가능합니다.");
                    return false;
                }
                var url = "./orderpartreturn1.php?change_status=" + change_status + "&od_id=" + $("input[name='od_id[" + k + "]']").val();
            ...
            case "반품거부":
                ...
                if (current_status != "반품요청") {
                    alert("'반품요청' 상태의 주문만 '반품거부'가 가능합니다.");
                    return false;
                }
                var url = "./orderpartreturn1.php?change_status=" + change_status + "&od_id=" + $("input[name='od_id[" + k + "]']").val();
                window.open(url, "orderpartreturn", "left=230,top=100,width=1000,height=650,scrollbars=yes,resizable=yes");
            ...
            case "반품철회":
                ...
                if (current_status != "반품요청") {
                    alert("'반품요청' 상태의 주문만 '반품철회'가 가능합니다.");
                    return false;
                }
                var url = "./orderpartreturn1.php?change_status=" + change_status + "&od_id=" + $("input[name='od_id[" + k + "]']").val();
                window.open(url, "orderpartreturn", "left=230,top=100,width=1000,height=650,scrollbars=yes,resizable=yes");
            ...
            case "수거완료":
                ...
                for (var i = 0; i < $chk.size(); i++) {
                    ...
                    if (current_status != "반품수거중") {
                        alert("'반품수거중' 상태의 주문만 '수거완료'가 가능합니다.");
                        return false;
                    }
                }
            ...
            case "반품완료":
                ...
                if (current_status != "수거완료") {
                    alert("'수거완료' 상태의 주문만 '반품완료'가 가능합니다.");
                    return false;
                }
                var url = "./orderpartreturn2.php?change_status=" + change_status + "&od_id=" + $("input[name='od_id[" + k + "]']").val();
                window.open(url, "orderpartreturn", "left=230,top=100,width=1000,height=650,scrollbars=yes,resizable=yes");
