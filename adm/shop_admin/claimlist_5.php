...
if ($od_contractout) {
    $where[] = " od_contractout = '$od_contractout' ";
} else {
    $od_contractout = "";
}
...
if ($sc_it_name) {
    $where[] = " od_id in (select od_id from {$g5['g5_shop_cart_table']} where it_name like '%$sc_it_name%') ";
}
...
$sql  = " select *,
            ...
            (select sh_memo from lt_shop_order_history where od_id = lt_shop_order.od_id and ct_status_claim = '해지' limit 1) as sh_memo
           $sql_common
...
$excel_sql = "select a.*, b.it_id, b.it_name, b.ct_option, b.ct_qty, (b.ct_price + b.io_price)  as opt_price, b.ct_status, b.ct_status_claim
    ...
    from lt_shop_order as a
         left join lt_shop_cart as b
            on a.od_id = b.od_id and b.io_type = '0' and b.ct_status_claim in ( '해지' )
         left join lt_shop_order_history as c
            on a.od_id = c.od_id and c.ct_status_claim = '해지'
            and sh_id in (select max(sh_id) from lt_shop_order_history where ct_status_claim = '해지' group by od_id)
    $sql_search
...
$headers = array('NO', '주문일시','주문번호', '주문상태','주문자아이디','주문자명', '상품코드', '상품명', '옵션항목', '수량','해지구분','해지상태','해지위약금','배송추적','요청일');
$bodys = array('NO', 'od_time','disp_od_id', 'od_status','mb_id','od_name', 'it_id', 'it_name', 'ct_option', 'ct_qty', 'od_contractout', 'ct_status', 'od_penalty','str_od_invoice','od_status_claim_date');
...
<input type="hidden" name="claimtype" value="<?php echo $claimtype; ?>">
    ...
    <tr>
        <th scope="row">해지구분</th>
		<td colspan="2">
		<div class="col-lg-12 col-md-6 col-sm-12 col-xs-12">
            <input type="radio" name="od_contractout" value="" id="od_contractout0"    <?php echo get_checked($od_contractout, '');     ?>>
            <label for="od_contractout0">전체</label>
            <input type="radio" name="od_contractout" value="일반해지" id="od_contractout1" <?php echo get_checked($od_contractout, '일반해지'); ?>>
            <label for="od_contractout1">일반해지</label>
            <input type="radio" name="od_contractout" value="분실/파손해지" id="od_contractout2" <?php echo get_checked($od_contractout, '분실/파손해지'); ?>>
            <label for="od_contractout2">분실/파손해지</label>
		</div>
		</td>
	</tr>
    <tr>
        <th scope="row">해지상태</th>
		<td colspan="2">
		<div class="col-lg-12 col-md-6 col-sm-12 col-xs-12">
            <input type="radio" name="od_status" value="" id="od_status_all"    <?php echo get_checked($od_status, '');     ?>>
            <label for="od_status_all">전체</label>
            <input type="radio" name="od_status" value="해지요청" id="od_status_income" <?php echo get_checked($od_status, '해지요청'); ?>>
            <label for="od_status_income">해지요청</label>
            <input type="radio" name="od_status" value="해지결제요청" id="od_status_income2" <?php echo get_checked($od_status, '해지결제요청'); ?>>
            <label for="od_status_income2">해지결제요청</label>
            <input type="radio" name="od_status" value="해지수거중" id="od_status_rdy" <?php echo get_checked($od_status, '해지수거중'); ?>>
            <label for="od_status_rdy">해지수거중</label>
            <input type="radio" name="od_status" value="수거완료" id="od_status_dvr" <?php echo get_checked($od_status, '수거완료'); ?>>
            <label for="od_status_dvr">해지수거완료</label>
            <input type="radio" name="od_status" value="해지완료" id="od_status_done" <?php echo get_checked($od_status, '해지완료'); ?>>
            <label for="od_status_done">해지완료</label>
		</div>
		</td>
	</tr>
...
<input type="hidden" name="search_od_contractout" value="<?php echo $od_contractout; ?>">
...
<div class="local_cmd01 local_cmd">
	...
	<input type="button" value="해지관리" class="btn btn_02" onclick="forderlist_submit('해지관리');">
	<input type="button" value="수거완료처리" class="btn btn_02" onclick="forderlist_submit('수거완료');">
	<input type="button" value="해지거부처리" class="btn btn_02" onclick="forderlist_submit('해지거부');">
	<input type="button" value="해지취소처리(CS)" class="btn btn_02" onclick="forderlist_submit('해지취소');">
	<input type="button" value="해지완료" class="btn btn_02" onclick="forderlist_submit('해지완료');">
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
    	...
    	<th scope="col">해지상태</th>
    	<th scope="col">해지위약금</th>
    	
    	<th scope="col">배송추적</th>
    	<th scope="col">해지요청일</th>
    ...
    <tr>
        ...
        <td headers="td_odrnum2" class="td_odrnum2">
        	...
        	<?php
        	$od_sub_ids = $opt['od_sub_id'];
        	echo $disp_od_id.'<br/>-'.$od_sub_ids;
        	?>
        </td>
        <td headers="td_odrnum2" class="td_odrnum2">
        ...
        <td headers="odrstat" class="odrstat" rowspan="<?php echo $rowspan ?>">
        	<label class="sound_only">해지구분</label>
            <?php echo $row['od_contractout']; ?>
        </td>
        <td headers="odrstat" class="odrstat" rowspan="<?php echo $rowspan ?>">
        	<label class="sound_only">해지상태</label>
            <?php echo $opt['ct_status']; ?>
        </td>
        <td headers="odrstat" class="odrstat" rowspan="<?php echo $rowspan ?>">
        	<label class="sound_only">위약금</label>
            <?php echo number_format($row['od_penalty']); ?>
        </td>
        
        <td headers="delino" class="delino" rowspan="<?php echo $rowspan ?>">
        	<label class="sound_only">운송장번호</label>
            <?php 
            if($row['od_pickup_invoice'] && $row['od_status'] != '해지요청') {
                echo "<a href='https://www.doortodoor.co.kr/parcel/doortodoor.do?fsp_action=PARC_ACT_002&fsp_cmd=retrieveInvNoACT&invc_no='".$row['od_pickup_invoice']." target='_blank'>";
                echo ($row['od_pickup_invoice'] ? $row['od_pickup_invoice'] : '-').'<br/>';
                echo ($row['od_pickup_delivery_company'] ? $row['od_pickup_delivery_company'] : '-').'<br/>';
                echo (is_null_time($row['od_pickup_invoice_time']) ? '-' : substr($row['od_pickup_invoice_time'],0,16));
                echo "</a>";
            } else {
                echo "-";
            }
        ...
        <td class="td_name" rowspan="<?php echo $rowspan ?>">
...
function forderlist_submit(change_status)
{
	...
	switch (change_status)
    {
         case "해지관리" :
             ...
             if (current_status != "해지요청" && current_status != "해지결제요청") {
                 alert("'해지요청','해지결제요청' 상태의 주문만 '해지관리'가 가능합니다.");
                 return false;
             }
             var url = "./orderproc.out.php?change_status="+change_status+"&od_id="+$("input[name='od_id["+k+"]']").val();
             window.open(url, "orderprocout", "left=230,top=100,width=1000,height=650,scrollbars=yes,resizable=yes");
         ...
         case "해지거부" :
             ...
             if (current_status != "해지요청") {
                 alert("'해지요청' 상태의 주문만 '해지거부'가 가능합니다.");
                 return false;
             }
             var url = "./orderproc.out.php?change_status="+change_status+"&od_id="+$("input[name='od_id["+k+"]']").val();
         ...
         case "해지취소" :
             ...
             if (current_status != "해지요청") {
                 alert("'해지요청' 상태의 주문만 '해지취소처리(CS)'가 가능합니다.");
                 return false;
             }
             var url = "./orderproc.out.php?change_status="+change_status+"&od_id="+$("input[name='od_id["+k+"]']").val();
             window.open(url, "orderpartreturn", "left=230,top=100,width=1000,height=650,scrollbars=yes,resizable=yes");
         ...
         case "수거완료" :
        	...
        	for (var i=0; i<$chk.size(); i++)
			{
                 ...
                 if (current_status != "해지수거중") {
                     alert("'해지수거중' 상태의 주문만 '수거완료'가 가능합니다.");
                     return false;
                 }
			}

        	$("#post_od_status").val("수거완료");
        	$("#forderlist").attr("action","./claimlistupdate.php").submit();
         ...
         case "해지완료" :
         	...
         	for (var i=0; i<$chk.size(); i++)
 			{
                  ...
                  if (current_status != "수거완료") {
                      alert("'수거완료' 상태의 주문만 '해지완료'가 가능합니다.");
                      return false;
                  }
 			}

         	$("#post_od_status").val("해지완료");
         	$("#forderlist").attr("action","./claimlistupdate.php").submit();
