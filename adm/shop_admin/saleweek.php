...
if(isset($act) && $act == "exceldownload"){
    ...
    $headers = array('NO','주차','시작일','종료일','리스 주문수','제품 주문수','세탁 주문수','세탁보관 주문수','수선 주문수','주문수 총 합계','(총)주문금액','(총)박스비','(총)배송비','쿠폰 할인','적립금사용'
        ,'리스 결제합계','제품 결제합계','세탁 결제합계','세탁보관 결제합계','수선 결제합계','결제 총 합계','할인합계','주문취소','환불금액','미수금','일반위약금','파손위약금','분실위약금');
    
    $bodys = array('NO','od_week','od_s_date','od_e_date','R_count','O_count','L_count','K_count','S_count','ordercount','orderprice','zbox_price','send_cost','ordercoupon','receiptpoint'
