...
if ($_POST['act'] == "it_use2") {
    for ($i=0; $i<count($_POST['chk']); $i++) {
        ...
        $sql = "update {$g5['g5_shop_item_table']}
                   set it_update_time = '".G5_TIME_YMDHIS."'
                 where it_id   = '{$_POST['it_id'][$k]}' ";
        sql_query($sql);
        $sql = "update lt_shop_item_option
                set io_stock_qty = 0 
                where it_id   = '{$_POST['it_id'][$k]}' ";
        sql_query($sql);
    }
} else if ($_POST['act'] == "it_use0") {
    for ($i=0; $i<count($_POST['chk']); $i++) {
...
if($is_admin == "brand")
{
    goto_url("./itemlist.brand.php?sca=$sca&amp;sst=$sst&amp;sod=$sod&amp;sfl=$sfl&amp;stx=$stx&amp;page=$page");
} else {
