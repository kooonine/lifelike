...
// 주문서 정보
//------------------------------------------------------------------------------
$sql = " SELECT * from {$g5['g5_shop_rental_table']} where rt_id = '$rt_id' ";
$rental = sql_fetch($sql);


$sql_visit = "SELECT * FROM lt_visit WHERE vi_ip='{$rental['rt_ip']}' AND vi_new=true ORDER BY vi_id DESC";
$visit = sql_fetch($sql_visit);

if (!$rental['rt_id']) {
    alert("해당 주문번호로 주문서가 존재하지 않습니다.");
}
$sql = " SELECT * from {$g5['g5_shop_order_table']} where od_id = 20190916000009";
$od = sql_fetch($sql);

$is_company = ($rental['company_code'] && $rental['company_code'] != "") ? true : false;
if ($rental['company_code'] == "") {
...
$rental['mb_id'] = $rental['mb_id'] ? $rental['mb_id'] : "비회원";
...
// 주문번호에 - 추가
$disp_rt_id = 'T-' . substr($rental['rt_id'], 0, 8) . '-' . substr($rental['rt_id'], 8);
...
<div class="row">
    <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
            ...
            <div class="local_desc02 local_desc">
                <p>
                    <label class="control-label col-md-3">주문경로/유입경로 : <strong><?php echo ($rental['od_mobile']) ? "MOBILE" : "PC" ?></strong> </label>
                    <label class="control-label col-md-9">주문번호 : <strong><?php echo $disp_rt_id ?></strong> </label>
                    <div class="clearfix"></div>

                    <label class="control-label col-md-3">현재 주문상태 : <strong><?php echo $rental['rt_status'] ?></strong> </label>
                    <label class="control-label col-md-9">주문일자 : <strong><?php echo substr($rental['rt_time'], 0, 16); ?> (<?php echo get_yoil($rental['rt_time']); ?>)</strong> </label>
                    <br />
            </div>
                    ...
                    <div class="tbl_head01 tbl_wrap">
                        <table>
                            ...
                            <tbody>
                                ...
                                // 상품목록
                                $sql = "SELECT a.od_id, a.ct_id, a.it_id, a.it_name, a.cp_price, a.ct_send_cost, a.it_sc_type, a.od_type, a.io_sapcode_color_gz as option_code
		                                ...
		                                FROM {$g5['g5_shop_rental_order_table']} as r
		                                	...
		                                	INNER JOIN {$g5['g5_shop_order_item_table']} as d on a.ct_id = d.ct_id
		                                WHERE
		                                	r.rt_id = '{$rental['rt_id']}'
	...
	";
                                ...
                                for ($i = 0; $row = sql_fetch_array($result); $i++) {
                                    ...
                                    $rfid_modify = empty($row['od_sub_id']) ? false : true;
                                    $opt_rental_price = $row['ct_rental_price'] + $row['io_price'];
                                    $item_options = get_item_option($row['it_id']);
                                    $disp_od_id = $row['od_type'] . '-' . substr($row['od_id'], 0, 8) . '-' . substr($row['od_id'], 8);
                                    ?>
                                    <tr>
                                        <td><a href="./orderform.php?od_id=<?php echo $row['od_id'] ?>"><?php echo $disp_od_id ?></a></td>
                                        <td class=" td_itname" style="text-align: center !important;"><?php echo $image; ?></td>
                                        <td class="td_itname">
                                            <?php echo stripslashes($row['it_name']); ?><br />
                                            <?php echo $item_options[$row['option_code']]['sapcode'] ?><br />
                                            <?php
                                                echo $item_options[$row['option_code']]['item'] . ' - ' . get_text($row['ct_option']);
                                                if ($row['io_price'] > 0) echo "(옵션금액 + " . number_format($row['io_price']) . "원)";
                                                ?>
                                        </td>
                                        <td><?php echo $row['ct_qty']; ?></td>
                                        <td class="td_num_right"><?php echo number_format($opt_rental_price * $row['ct_item_rental_month']); ?> 원</td>
                                        <td class="td_num_right"><?php echo number_format($opt_rental_price); ?> 원<br />(<?php echo $row['ct_item_rental_month'] ?>개월)</td>
                                        <?php if ($i == 0) { ?>
                                            <td rowspan="<?php echo $rowspan; ?>" class="td_num_right"><?php echo number_format($sum['price'] * $row['ct_item_rental_month']); ?> 원
                                                <br />(<?php echo number_format($sum['price']); ?> 원)
                                            </td>
                                        <?php } ?>
            ...
            $amount['order'] = $rental['rt_cart_price'] + $rental['rt_send_cost'] + $rental['rt_send_cost2'];

            // 입금액 = 결제금액 + 포인트
            $amount['receipt'] = $rental['rt_receipt_price'] - $rental['rt_refund_price'];

            // 쿠폰금액
            $amount['coupon'] = $rental['rt_cart_coupon'] + $rental['rt_coupon'] + $rental['rt_send_coupon'];

            // 취소금액
            $amount['cancel'] = $rental['rt_cancel_price'];

            // 미수금 = 주문금액 - 취소금액 - 입금금액 - 쿠폰금액
            //$amount['미수'] = $amount['order'] - $amount['receipt'] - $amount['coupon'];

            // 결제방법
            $s_receipt_way = $rental['rt_settle_case'];

            if ($rental['rt_settle_case'] == '간편결제') {
                switch ($rental['rt_pg']) {
                    case 'lg':
                    ...
                    case 'kcp':
                        ...
                        break;
                    default:
                        $s_receipt_way = $row['rt_settle_case'];
                        break;
                }
            }

            if ($rental['rt_receipt_point'] > 0)
                $s_receipt_way .= "+포인트";
            ?>
                        ...
                        <table>
                            <tbody>
                                <tr>
                                    ...
                                    <td colspan="3"><?php echo $disp_rt_id; ?></td>
                                </tr>
                                <tr>
                                    <th scope="row"><label>주문상태</label></th>
                                    <td><strong><?php echo $rental['rt_status'] ?></strong></td>
                                    <th scope="row"><label>클레임상태</label></th>
                                    <td><strong><?php echo $rental['rt_status_claim'] ?></strong></td>
                                </tr>
                                <tr>
                                    <th scope="row"><label>주문자명(ID)</label></th>
                                    <td colspan="3"><?php echo get_sideview($rental['mb_id'], get_text($rental['rt_name']), $rental['rt_email'], ''); ?>(<?php echo $rental['mb_id']; ?>)</td>
                                </tr>
                                <tr>
                                    <th scope="row"><label>계약금액</label></th>
                                    <td><?php echo display_price($amount['order']); ?></td>
                                    <th scope="row"><label>배송비</label></th>
                                    <td><?php echo display_price($rental['rt_send_cost']); ?></td>
                                </tr>
                                <tr>
                        ...
                        <table>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><?php echo number_format($rental['rt_rental_price'] * $rental['rt_month']); ?> 원</td>
                                </tr>
                                <tr>
                                    <th scope="row" class="sodr_sppay">월 이용료</th>
                                    <td><?php echo number_format($rental['rt_rental_price']); ?> 원</td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    ...
                                    <td><?php echo $rental['rt_billday']; ?>일</td>
                                </tr>
                                <tr>
                                    <th scope="row" class="sodr_sppay">납부 횟수</th>
                                    <td><?php echo $rental['rt_payment_count']; ?> 회</td>
                                </tr>
                                <tr>
                                    <th scope="row" class="sodr_sppay">납부 시작일</th>
                                    <td><?php echo $rental['rt_rental_startdate']; ?></td>
                                </tr>
                                ...
                                <tr>
                                    ...
                                    <td><?php echo display_price($rental['rt_refund_price']); ?></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                            ...
                            <table>
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td>
                                            <?php echo $rental['rt_delivery_company']; ?> <?php echo get_delivery_inquiry($rental['rt_delivery_company'], $rental['rt_invoice'], 'btn btn_01'); ?>
                                            <?php echo $rental['rt_invoice']; ?>
                                        </td>
                                    </tr>
                                    <tr>
                                        <th scope="row">발송완료일</th>
                                        <td><?php echo is_null_time($rental['rt_invoice_time']) ? "" : $rental['rt_invoice_time']; ?></td>
                                    </tr>
                                ...
                                <table>
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td><input type="text" name="od_name" value="<?php echo get_text($rental['rt_name']); ?>" id="od_name" required class="frm_input required"></td>
                                        </tr>
                                        <tr hidden>
                                            <th scope="row"><label for="od_tel"><span class="sound_only">주문하신 분 </span>연락처</label></th>
                                            <td><input type="text" name="od_tel" value="<?php echo get_text($rental['rt_tel']); ?>" id="od_tel" required class="frm_input required"></td>
                                        </tr>
                                        <tr>
                                            <th scope="row"><label for="od_hp"><span class="sound_only">주문하신 분 </span>핸드폰</label></th>
                                            <td><input type="text" name="od_hp" value="<?php echo get_text($rental['rt_hp']); ?>" id="od_hp" class="frm_input"></td>
                                        </tr>
                                        <tr>
                                            <th scope="row"><span class="sound_only">주문하시는 분 </span>주소</th>
                                            <td>
                                                <label for="od_zip" class="sound_only">우편번호</label>
                                                <input type="text" name="od_zip" value="<?php echo $rental['rt_zip1'] . $rental['rt_zip2']; ?>" id="od_zip" required class="frm_input required" size="5">
                                                ...
                                                <input type="text" name="od_addr1" value="<?php echo get_text($rental['rt_addr1']); ?>" id="od_addr1" required class="frm_input required" size="35">
                                                <label for="od_addr1">기본주소</label><br>
                                                <input type="text" name="od_addr2" value="<?php echo get_text($rental['rt_addr2']); ?>" id="od_addr2" class="frm_input" size="35">
                                                <label for="od_addr2">상세주소</label>
                                                <br>
                                                <input type="text" name="od_addr3" value="<?php echo get_text($rental['rt_addr3']); ?>" id="od_addr3" class="frm_input" size="35">
                                                <label for="od_addr3">참고항목</label>
                                                <input type="hidden" name="od_addr_jibeon" value="<?php echo get_text($rental['rt_addr_jibeon']); ?>"><br>
                                            </td>
                                        </tr>
                                        <tr>
                                            <th scope="row"><label for="od_email"><span class="sound_only">주문하신 분 </span>E-mail</label></th>
                                            <td><input type="text" name="od_email" value="<?php echo $rental['rt_email']; ?>" id="od_email" required class="frm_input email required" size="30"></td>
                                        </tr>
                                        <tr>
                                            <th scope="row"><span class="sound_only">주문하신 분 </span>IP Address</th>
                                            <td><?php echo $rental['rt_ip']; ?></td>
                                        </tr>
                                        <tr>
                                ...
                                <table>
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td><input type="text" name="od_b_name" value="<?php echo get_text($rental['rt_b_name']); ?>" id="od_b_name" required class="frm_input required"></td>
                                        </tr>
                                        <tr hidden>
                                            <th scope="row"><label for="od_b_tel"><span class="sound_only">받으시는 분 </span>연락처</label></th>
                                            <td><input type="text" name="od_b_tel" value="<?php echo get_text($rental['rt_b_tel']); ?>" id="od_b_tel" required class="frm_input required"></td>
                                        </tr>
                                        <tr>
                                            <th scope="row"><label for="od_b_hp"><span class="sound_only">받으시는 분 </span>핸드폰</label></th>
                                            <td><input type="text" name="od_b_hp" value="<?php echo get_text($rental['rt_b_hp']); ?>" id="od_b_hp" class="frm_input required"></td>
                                        </tr>
                                        <tr>
                                            <th scope="row"><span class="sound_only">받으시는 분 </span>주소</th>
                                            <td>
                                                <label for="od_b_zip" class="sound_only">우편번호</label>
                                                <input type="text" name="od_b_zip" value="<?php echo $rental['rt_b_zip1'] . $rental['rt_b_zip2']; ?>" id="od_b_zip" required class="frm_input required" size="5">
                                                ...
                                                <input type="text" name="od_b_addr1" value="<?php echo get_text($rental['rt_b_addr1']); ?>" id="od_b_addr1" required class="frm_input required" size="35">
                                                <label for="od_b_addr1">기본주소</label>
                                                <input type="text" name="od_b_addr2" value="<?php echo get_text($rental['rt_b_addr2']); ?>" id="od_b_addr2" class="frm_input" size="35">
                                                <label for="od_b_addr2">상세주소</label>
                                                <input type="text" name="od_b_addr3" value="<?php echo get_text($rental['rt_b_addr3']); ?>" id="od_b_addr3" class="frm_input" size="35">
                                                <label for="od_b_addr3">참고항목</label>
                                                <input type="hidden" name="od_b_addr_jibeon" value="<?php echo get_text($rental['rt_b_addr_jibeon']); ?>"><br>
                                            </td>
                                        </tr>
                                        ...
                                        <?php if ($default['de_hope_date_use']) { ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <input type="text" name="od_hope_date" value="<?php echo $rental['rt_hope_date']; ?>" id="od_hopedate" required class="frm_input required" maxlength="10" minlength="10"> (<?php echo get_yoil($rental['rt_hope_date']); ?>)
                                                </td>
                                            </tr>
                                        <?php } ?>
                                        ...
                                        <tr>
                                            ...
                                            <td><?php if ($rental['rt_memo']) echo get_text($rental['rt_memo'], 1);
                                                else echo "없음"; ?></td>
                                        </tr>
                                    </tbody>
                                </table>
