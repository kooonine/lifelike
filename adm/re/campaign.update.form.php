...
$sql_cz = "SELECT * FROM lt_shop_coupon_zone WHERE cp_promotion_check = 1  ORDER BY cz_id DESC";
...
if ($w == '') {
    ...
    $title_msg = '수정';
    $sql = " select * from lt_campaign where cp_id = '$cp_id' ";
    $cp = sql_fetch($sql);
...
$g5['title'] = "기획전/이벤트 " . $title_msg;
include_once('../admin.head.php');

$action_url = https_url('adm') . "/re/campaign.update.php";
$categories = array(
    'THEME' => '테마전',
    'MD' => 'MD`s Pick',
    'BRAND' => 'BRAND',
                    ...
                    <div class="tbl_frm01 tbl_wrap">
                        <table id="compaign-content-wrapper">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <th scope="row">프로모션</th>
                                    <td colspan="2">
                                    <label><input type="checkbox" name="cp_promotion" id="cp_promotion" value=1 <?php echo get_checked($cp['cp_promotion'], 1) ?>> </label>
                                    <label>(프로모션 기획전 체크)</label>
                                    <!-- <label><input type="radio" name="cp_use" value=1 <?php echo get_checked($cp['cp_use'], 1) ?>>공개</label>&nbsp;&nbsp; -->
                                    <select name="cp_promotion_select" id="cp_promotion_select" style="width : 200px">
                                    <option>쿠폰 선택</option>
                                        <? foreach ($cz_set as $cz) : ?>
                                            <option value="<?= $cz['cz_id'] ?>" <?= get_selected($cz['cp_promotion'], $cp['cp_id']) ?>><?= $cz['cz_subject'] ?></option>
                                        <? endforeach ?>
                                    </select>
                                    </td>
                                </tr>
                                                    ...
                                                    <td>
                                                        <div class="checkbox">
                                                            ...
                                                            if (empty($cp_link) && $w == "u")
                                                                $cp_link = "/event/view.php?cp_id=" . $cp['cp_id'];
                                                            ?>
                                ...
                                <tr>
                                    ...
                                    <td colspan="2">
                                        <input type="file" name="cp_image_1" id="cp_image2" title="<?= $banner ?> 이미지" class="frm_file " accept="image/*">
                                ...
                                <tr>
                                    ...
                                    <td colspan="2">
                                        <input type="file" name="cp_image_2" id="cp_image2" title="<?= $banner ?> 이미지" class="frm_file " accept="image/*">
                                ...
                                <?php if (empty($cp_item_set)) : ?>
                                    <tr>
                                        ...
                                        <td scope="row" colspen=2>
                                            ...
                                            선택 분류&nbsp;&nbsp;<input type="text" style="width: 60%;" name="cp_item_set_category[1]" id="cp_item_set_category_1" value="" readonly><button type="button" class="btn frm_input" target-data="coupon_category_modal" data-item-idx=1 onclick=openCpItemPopup(this)>분류선택</button><br>
                                            선택 개수&nbsp;&nbsp;<input type="text" style="width: 5%;" name="cp_item_set_number[1]" id="cp_item_set_number_1" value=""><br>
                                        </td>
                                    </tr>
                                <? else : ?>
                                    ...
                                    <?php foreach ($cp_item_set as $ii => $cp_item) : ?>
                                        <tr>
                                            ...
                                            <td scope="row" colspen=2>
                                                ...
                                                선택 분류&nbsp;&nbsp;<input type="text" style="width: 60%;" name="cp_item_set_category[<?= $ii ?>]" id="cp_item_set_category_<?= $ii ?>" value="<?= $cp_item['category'] ?>" readonly><button type="button" class="btn frm_input" target-data="coupon_category_modal" data-item-idx=<?= $ii ?> onclick=openCpItemPopup(this)>분류선택</button><br>
                                                <div class="numberSet">
                                                선택 개수&nbsp;&nbsp;<input type="text" style="width: 5%;" name="cp_item_set_number[<?= $ii ?>]" id="cp_item_set_number_[<?= $ii ?>]"value="<?= $cp_item['number'] ?>"><br>
                                                </div>
                                            </td>
                                        </tr>
                ...
                <div class="x_content">
                    <div class="form-group">
                        <div class="col-md-12 col-sm-12 col-xs-12 text-right">
                            ...
                            <a href="/adm/re/campaign.list.php"><button class="btn btn_02" type="button">목록</button></a>
...
<script>
    ...
    $("#cp_promotion").change(function(){ 
        if($("#cp_promotion").is(":checked")){
            $(".numberSet").css("display", "block");
        } else{
            $(".numberSet").css("display", "none");
        }
    });
...
<div class="modal fade" id="coupon_category_modal" tabindex="-1" role="dialog" aria-labelledby="coupon_category_modal">
    ...
    <script>
        ...
        $(".btn-add").on("click", function() {
            ...
            setHtml += '<button type="button" class="btn frm_input" target-data="coupon_category_modal" data-item-idx=' + nextIdx + ' onclick=openCpItemPopup(this)>분류선택</button><br>';
            setHtml +=  '<div class="numberSet">선택 개수&nbsp;&nbsp;<input type="text" style="width: 5%;" name="cp_item_set_number[' + nextIdx + ']" id="cp_item_set_number_' + nextIdx + '" value=""><div>';
            setHtml += '</td>';
            setHtml += '</tr>';
