...
foreach ($_POST["cp_image_del"] as $idx => $del) {
    if ($del == "1") {
        ...
        unlink($del_path);
        $sql_del = "UPDATE lt_event SET cp_image_{$idx} = NULL WHERE cp_id='{$cp_id}'";
        sql_query($sql_del);
    }
}
...
if ($w == "") {
    $sql = " insert lt_event set $sql_common ";
    sql_query($sql);
    $cp_id = sql_insert_id();
} else if ($w == "u") {
    $sql = " update lt_event set $sql_common where cp_id = '$cp_id' ";
    sql_query($sql);
} else if ($w == "d") {
    $sql = " delete from lt_event where cp_id = '$cp_id' ";
    sql_query($sql);
}

if ($w == "d") {
    goto_url('./event.list.php');
} else {
    goto_url("./event.update.form.php?w=u&amp;cp_id=$cp_id");
}
