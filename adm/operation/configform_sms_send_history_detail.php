<?php
include_once('./_common.php');

$sql  = "select * from lt_sms_sendhistory where sh_no = '{$sh_no}'; ";
$sh = sql_fetch($sql);
?>
<table>
  <colgroup>
  	<col class="grid_4">
  	<col>
  	<col class="grid_4">
  	<col>
  </colgroup>
  <tr>
    <th scope="row">발송타입</th>
    <td><?php echo $sh['sh_sendtype']; ?></td>
    <th scope="row">메시지구분</th>
    <td><?php echo ($sh['msg_type']=="sms")?"단문":"장문"; ?></td>
  </tr>
  <tr>
    <th scope="row">발송시간(등록시간)</th>
    <td><?php echo ($sh['send_time'] != "0000-00-00 00:00:00")?$sh['send_time']."(".$sh['sh_datetime'].")":$sh['sh_datetime']; ?></td>
    <th scope="row">발송건수</th>
    <td><?php 
    echo count(preg_split(",", $sh['dest_phone']));
            ?>건</td>
  </tr>
  <tr>
    <th scope="row">발신번호</th>
    <td><?php echo $sh['send_phone']; ?></td>
    <th scope="row">수신거부</th>
    <td><?php echo ($sh['donotcall_use'] == '1')?"제외":"포함"; ?></td>
  </tr>
  <tr>
    <th scope="row">수신번호</th>
    <td colspan="3"><?php echo $sh['dest_phone']; ?></td>
  </tr>
  <tr>
    <th scope="row">메시지제목</th>
    <td colspan="3"><?php echo $sh['msg_title']; ?></td>
  </tr>
  <tr>
    <th scope="row">발신내용</th>
    <td colspan="3"><?php echo $sh['msg_body']; ?></td>
  </tr>
  <tr>
    <th scope="row">처리 결과 코드</th>
    <td colspan="3"><?php 
    switch ( $sh['result_code']) {
            case '100':
                echo '사용자 에러';
                break;
            case '200':
                echo '성공';
                break;
            case '300':
                echo '파라미터 에러';
                break;
            case '400':
                echo '기타 에러';
                break;
            case '500':
                echo '미등록 발신번호 차단';
                break;
            default:
                echo '실패';
                break;
        }
    ?></td>
    <!-- th scope="row">CMID</th>
    <td><?php echo $sh['cmid']; ?></td -->
  </tr>
  <tr>
    <th scope="row">결과내용</th>
    <td colspan="3"><?php echo $sh['result_msg']; ?></td>
  </tr>
 </table>