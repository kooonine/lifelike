...
$sql = " select a.cp_id, c.cl_id, c.od_id, c.cl_datetime,a.cp_datetime,b.mb_id,b.mb_name,cp_start,cp_end,if(c.cl_id is null,'미사용','사용') as str_cl_id
...
$qstr = $qstr."&amp;cm_no=".$cm_no."&amp;coupon_issuance_type=".$coupon_issuance_type."&amp;po_datetime=".$po_datetime;
...
<div class="row">
  <div class="col-md-12 col-sm-12 col-xs-12">
  	<div class="x_panel">
        ...
        <table>
         ...
         <tr scope = 'row'>
            <th>쿠폰명</th>
            <td colspan="3">
              <?php echo $cmg['cm_subject']?>
            </td>
          </tr>
          <tr scope = 'row'>
            <th>쿠폰설명</th>
            <td colspan="3">
              <?php echo $cmg['cm_summary']?>
            </td>
          </tr>
          <tr scope = 'row'>
            <th>혜택구분</th>
            <td colspan="3">
              <?php 
                switch($cmg['cm_type']) {
                    case '0':
                        echo '할인금액 : '.number_format($cmg['cm_price']).'원';
                        break;
                    case '1':
                        echo '할인율 : '.$cmg['cm_price'].'%';
                        echo ', 절사단위 : '.number_format($cmg['cm_trunc']).'원단위';
                        echo ', 최소주문금액 : '.number_format($cmg['cm_minimum']).'원';
                        echo ', 최대금액 : '.number_format($cmg['cm_maximum']).'원';
                        break;
                }
                ?>
            </td>
          ...
          <tr scope = 'row'>
            <th>발급구분</th>
            <td colspan="3">
              <?php echo ($cmg['cm_target_type'] == '0')?"대상자지정":"조건부 자동" ?>
              <?php echo ($cmg['cm_target_type2'] != '')?"(".$cmg['cm_target_type2'].")":"" ?>
            </td>
          </tr>
          <tr scope = 'row'>
            <th>적용계산 기준</th>
            <td colspan="3">
            	<?php echo ($cmg['cm_use_price_type'] =='0')?"할인(쿠폰 제외) 적용 전 결제금액":"할인(쿠폰제외) 적용 후 결제금액"; ?>
            </td>
          </tr>
          <tr scope = 'row'>
            <th>사용기간</th>
            <td colspan="3">
               <?php 
                    if ($cmg['cm_end_time'] == 0) echo "기간 제한 없음";
                    else {
                        echo "발급일로부터 ".$cmg['cm_end_time']."일 이내";
                        /*
          ...
          <tr scope = 'row'>
            <th>적용범위</th>
            <td colspan="3">
              <?php echo ($cmg['cm_method'] =='0')?"상품쿠폰":"주문서쿠폰"; ?>
            </td>
          </tr>
          <tr scope = 'row'>
            <th>사용가능 기준금액</th>
    ...
    <div class="x_panel">
	  ...
	  <input type="hidden" name="cm_no" value="<?php echo $cm_no?>">
...
<input type="hidden" name="token" value="">
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        <tr class="<?php echo $bg; ?>">
            ...
            <td class="td_name sv_use"><div><?php echo $link1.$row['od_id'].$link2; ?></div></td>
