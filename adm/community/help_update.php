...
if ($w == 'd'){
    ...
    $result_url = './help_management.php?'.preg_replace('/^&amp;/', '?', $qstr);
    goto_url($result_url);
    exit;
}
else{
...
if($w != 'a' && $w != 'u' && $qaconfig['qa_req_email'] && !$qa_email)
    $msg[] = '이메일을 입력하세요.';
...
if ($qa_content == '') {
    $msg[] = '<strong>내용</strong>을 입력하세요.'.$_POST['qa_content'];
}
...
if($w == 'a' && $write['qa_email_recv']){
    
    $arr_change_data = array();
    /*{주문번호}        {제품명}        {문의유형}        {문의제목}        {문의내용}        {답변내용} */
    $it_name = "";
    if($write['it_id']){
        $it = sql_fetch("select * from lt_shop_item where it_id = '{$write['it_id']}' ");
        $it_name = $it['it_name'];
    }
    $arr_change_data['제품명'] = $it_name;
    $arr_change_data['주문번호'] = $write['od_id'];
    $arr_change_data['문의유형'] = $write['qa_category'];
    $arr_change_data['문의일'] =  substr($write['qa_datetime'],0,10);
    
    $arr_change_data['문의제목'] = $write['qa_subject'];
    $arr_change_data['문의내용'] = nl2br(conv_unescape_nl(stripslashes($write['qa_content'])));
    
    $content = nl2br(conv_unescape_nl(stripslashes($qa_content)));
    $arr_change_data['답변내용'] = $content;
    $arr_change_data['링크-문의확인'] = G5_BBS_URL."/qaview.php?qa_id=".$write['qa_id'];
    
    $arr_change_data['qa_id'] = $write['qa_id'];
    
    
    msg_autosend("게시판", "1:1문의 답변 통보", $write['mb_id'], $arr_change_data);
}
...
*/
if($w == 'a')
    $result_url = './help_detail.php?qa_id='.$qa_id.$qstr;
    else if($w == 'u' && $write['qa_type'])
        $result_url = './help_detail.php?qa_id='.$write['qa_parent'].$qstr;
    else
        $result_url = './help_management.php?'.preg_replace('/^&amp;/', '?', $qstr);
