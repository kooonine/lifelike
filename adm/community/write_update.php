...
if (isset($_POST['wr_content_mobile'])) {
    $wr_content_mobile = substr(trim($_POST['wr_content_mobile']),0,65536);
...
if (substr_count($wr_content_mobile, '&#') > 50) {
    alert('내용에 올바르지 않은 코드가 다수 포함되어 있습니다.');
    exit;
}
...
if ($w == '' || $w == 'r') {
    ...
    if ($w == 'r') {
        // 원글의 상태 변경 - 답변완료
        sql_query(" update $write_table set wr_9 = '답변완료' where wr_is_comment = '0' and wr_reply = '' and wr_num = '$wr_num' ");
    }
...
if (!($w == 'u' || $w == 'cu') && $config['cf_email_use'] && $board['bo_use_email']) {
    ...
    $wr_content_mobile = conv_content(conv_unescape_nl(stripslashes($wr_content_mobile)), $tmp_html);
...
if ($file_upload_msg) {
    alert($file_upload_msg, G5_ADMIN_URL.'/community/post_create.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.$qstr);
} else {
    if ($rtnURL) {
        goto_url($rtnURL.'?'.$qstr);
    } else {
        goto_url(G5_ADMIN_URL.'/community/board_management.php?bo_table='.$bo_table.'&amp;'.$qstr);
    }
}
?>
