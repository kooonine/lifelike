...
if($stx) {
    if (preg_match("/[a-zA-Z]/", $stx))
        $sql_search .= " and ( INSTR(LOWER(is_subject), LOWER('$stx')) > 0 or INSTR(LOWER(is_content), LOWER('$stx')) > 0 )";
        else
            $sql_search .= " and ( INSTR(is_subject, '$stx') > 0 or INSTR(is_content, '$stx') > 0) ";
}
  ...
  <div class="col-md-12 col-sm-12 col-xs-12">
	...
	<div id="tab_list_table">
      ...
      <label> 검색결과 <?php echo $total_count; ?>건</label>
        ...
        <option value="it_name,asc" <?php echo get_selected($sst.','.$sod, 'it_name,asc') ; ?>>상품명순</option>
