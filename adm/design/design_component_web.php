								...
								<? if ($row['main_id'] == "3") { ?>
									<td scope="col" class="text-center" rowspan="4">
										<table class="table table-bordered" style="margin-bottom: 0px;" style="height: 100px;">
											<tr class="bg-success">
												<td style="cursor:pointer;vertical-align: middle;" <? if ($main_id == "3") echo 'class="bg-primary"'; ?> onclick="location.href='./design_component_web.php?main_id=3';">메인 배너<br />영역 1</td>
											...
											<tr class="bg-success">
												<td style="cursor:pointer;vertical-align: middle;" <? if ($main_id == "4") echo 'class="bg-primary"'; ?> onclick="location.href='./design_component_web.php?main_id=4';">메인 배너<br />영역 2</td>
											</tr>
										</table>
									</td>
	...
	<div class="col-md-6 col-sm-6 col-xs-12">
		<div class="x_panel">
			<form name="frm" id="frm" method="post" enctype="multipart/form-data">
				...
				<input type="hidden" name="preview_main_id" value="<?= $view['main_id'] ?>">
					...
					<table class="table table-bordered">
						...
						<tbody>
							...
							<tr>
								...
								<td>
									<div class="radio col-md-12 col-sm-12 col-xs-12">
										<label><input type="radio" class="imgLinkYN" name="imgLinkYN0" data="0" value="N" <?= get_checked($main_view_data['imgLinkYN'], 'N') ?> <?= get_checked($main_view_data['imgLinkYN'], null) ?>>링크없음</label>&nbsp;&nbsp;
										<label><input type="radio" class="imgLinkYN" name="imgLinkYN0" data="0" value="Y" <?= get_checked($main_view_data['imgLinkYN'], 'Y') ?>>URL</label>
									</div>
									<input type="text" class="form-control <?= get_hidden($main_view_data['imgLinkYN'], 'N') ?> <?= get_hidden($main_view_data['imgLinkYN'], null) ?>" id="linkURL0" name="linkURL[]" value="<?= $main_view_data['linkURL']; ?>">
								</td>
							</tr>
				...
				<? } else if ($main_type1 == "product" || $main_type1 == "subproduct") { ?>
					...
					<div class="text-right">
						...
						<input type="hidden" value="<? if (!empty($main_view_data['it_id'])) echo implode(",", $main_view_data['it_id']); ?>" name="it_id_list" id="it_id_list" />
					</div>
...
<script>
	$(function() {
		$(document).ready(function($) {
			...
			$("#main_type1").change(function(event) {
				//alert($(this).val());
				location.href = './design_component_web.php?main_id=<?= $view['main_id'] ?>&chType1=' + $(this).val();
			});
		...
		$("#btnPreview").click(function(event) {
			...
			$("#frm").attr("action", "/index_preview.php");
			$("#frm").submit();
			return true;
		});
		...
		$("#btnSubmit").click(function(event) {
			...
			if (confirm("적용하시겠습니까?")) {
				$("#frm").attr("target", "");
				$("#frm").attr("action", "./design_component_web_update.php");
				$("#frm").submit();
			}
			return true;
		});
	...
	function frm_submit(f) {
		...
		if (confirm("적용하시겠습니까?")) {
			f.target = "";
			f.action = "./design_component_web_update.php";
			return true;
		}
		return false;
	}
