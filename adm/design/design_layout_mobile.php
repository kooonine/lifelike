	...
	<div class="col-md-12 col-sm-12 col-xs-12">
		<div class="x_panel">
			<table class="table table-bordered" style="height: 100%">
				...
				<tbody>
					<?php
					$sql = " select main_id, main_name, main_fixed, main_order, mobile_onoff, main_type1, main_type2, main_view_data, main_datetime $sql_common $sql_where order by main_order ";
					...
					while ($row=sql_fetch_array($result)) {
						...
						if($row['main_fixed'] == "Y") $bgType = 'bg-success';
						else $bgType = '';
						?>
						<tr class="<?php echo $bg; ?>">
							<td scope="col" class="text-center">
								<table class="table table-bordered" style="margin-bottom: 0px;"><tr>
									<td class="<?php echo $bgType; ?>"  style="cursor:pointer;vertical-align: middle;" onclick="location.href='./design_component_mobile.php?main_id=<?php echo $row['main_id'] ?>';">
										<?php echo $mainType1 ?>
									</td></tr></table>
								</td>

								<td class="text-center"  style="vertical-align: middle;"><?php echo $row['main_name'] ?></td>
								<td class="text-center"  style="vertical-align: middle;">
									<?php if($row['main_fixed'] == "Y") { echo '고정노출';
								} else if($row['mobile_onoff'] == "Y") { echo '<input type="button" id="btnMain'.$row['main_id'].'" main_id="'.$row['main_id'].'" class="btn btn-dark" value="ON" data="N" onclick="mainOnOffChange(this, \''.$row['main_id'].'\');"></input>';
							} else { echo '<input type="button" id="btnMain'.$row['main_id'].'" main_id="'.$row['main_id'].'" class="btn btn-secondary" value="OFF" data="Y" onclick="mainOnOffChange(this, \''.$row['main_id'].'\');"></input>';
						}
						?>
					</td>
					<td class="text-center">
						<input type="button" class="btn btn-secondary" value="관리하기"  onclick="location.href='./design_component_mobile.php?main_id=<?php echo $row['main_id'] ?>';"></input>
					</td>
				</tr>
			<?php } ?>
		...
		<div class="form-group">
			<div class="col-md-12 col-sm-12 col-xs-12 text-right">
				...
				<form name="frm" id="frm" method="post" >
					...
					<input type="button" class="btn btn-secondary" value="영역생성하기" onclick="newModal();"></input>
					<input type="button" class="btn btn-secondary" value="미리보기" id="btnPreview"></input>
					<input type="button" class="btn btn-success" value="적용하기" id="btnSubmit"></input>
				</form>
...
<script>
	$(function(){
		...
		$("#frm").attr("action","/index.php?device=mobile");
	...
	{
	//alert(main_id+","+mobile_onoff);
	var mobile_onoff = $("#"+ctl.id).attr("data");
...
function frm_submit(f)
{
	...
	if(confirm("적용하시겠습니까?"))
	{
		...
		$("#frm").attr("action","design_layout_mobile_update.php");
		$("#frm").attr("target","");
		$("#frm").submit();
		return false;
	}
