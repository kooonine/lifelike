...
$sql_common = " from lt_memoryform ";

$sql = "select * $sql_common WHERE mf_type = 0 order by mf_num ASC ";
$sql1 = "select * $sql_common WHERE mf_type = 1 order by mf_num ASC ";
    ...
    <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
            ...
            <div class="x_content">
                <div class="form-group">
                    <div class="col-md-6 col-sm-6 col-xs-6 text-right pull-right">
                        ...
                        <a href="./memory.update.form.php?w=pc" class="btn btn-success">배너등록</a>
                    </div>
                </div>
                <div class="tbl_head01 tbl_wrap">
                    <table>
                        ...
                        <tbody id="melist">
                            <input type="hidden" name ="mf_id_list" id ="mf_id_list" value = "">
                            <?php
                            for ($i = 0; $row = sql_fetch_array($pcResult); $i++) {
                                $status = $row['mf_use'] ? "사용" : "미사용";
                            ?>
                                <tr>
                                    ...
                                    <td class="td_chk mf_id_sort" data-value="<?php echo $row['mf_id'] ?>">
                                        <span class="glyphicon glyphicon-chevron-up" onclick="meSort(this, 'up')"></span>
                                        <span class="glyphicon glyphicon-chevron-down" onclick="meSort(this, 'down')"></span>
                                    </td>
                                    <td>
                                        <img src="<?php echo $row['mf_img']; ?>" class="img-thumbnail" id="imgimgFile" style="width: 150px; height: 150px;">
                                    </td>
                                    <td><?php echo $status; ?></td>
                                    <td class="td_mng td_mng_m">
                                        <a href="./memory.update.form.php?w=u&amp;mf_id=<?php echo $row['mf_id']; ?>" class="btn btn-success">수정</a>
                                        <a href="./memory.update.php?w=d&amp;mf_id=<?php echo $row['mf_id']; ?>" onclick="return delete_me(this);" class="btn btn-danger">삭제</a>
                                    </td>
                                </tr>
                            <?php
                            }
            ...
            <div class="x_content">
                ...
                <!-- 여기서 테이블 만들어 말아?  -->
                <tr>
                    <th scope="row">메모리폼 상품 </th>
                    <td scope="row" colspen=2>
                        선택 상품&nbsp;&nbsp;<input type="text" style="width: 60%;" name="cp_item_set_item[1]" id="cp_item_set_item_1" value="<?= $itemResult['mf_item'] ?>" readonly><button type="button" class="btn frm_input" target-data="coupon_product_modal" data-item-idx=1 onclick=openCpItemPopup(this)>상품선택</button><br>
...
<script>
    ...
    function itemSave() {
        if(confirm("저장하시겠습니까?")) {
            let mfItem = $("#cp_item_set_item_1").val();
            $.ajax({
                url: "./memory.update.php",
                method: "POST",
                data: {
    ...
    function meSort(elem, action) {

        const value = $(elem).parent().data("value");
        const $rows = $("#melist>tr>td.mf_id_sort");
        ...
        if (targetIdx >= 0 && targetIdx < $rows.length) {
            ...
            let values = [];
            $("#melist>tr>td.mf_id_sort").each(function(idx, elem) {
                values.push($(elem).data("value"));
            });
            $("#mf_id_list").val(values.join(','));
        }
    }
