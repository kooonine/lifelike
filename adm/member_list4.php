...
if ($mb_datetime != "") {
    ...
    if ($fr_mb_datetime && $to_mb_datetime) {
        $sql_search .= " and mb_intercept_date between '$fr_mb_datetime 00:00:00' and '$to_mb_datetime 23:59:59' ";
    }
}

$sql_search .= " and (mb_intercept_date <> '' or mb_block_write = '1' or  mb_block_shop = '1' or  mb_block_login = '1')" ;

if (!$sst) {
    $sst = "mb_intercept_date";
    $sod = "desc";
}
...
<input type="hidden" name="act_button" value="불량회원설정2">
...
<div class="tbl_head01 tbl_wrap">
    ...
    <thead>
    <tr>
        <th scope="col" id="mb_list_join"><?php echo subject_sort_link('mb_intercept_date', '', 'desc') ?>설정일</a></th>
        <th scope="col" id="mb_list_name"><?php echo subject_sort_link('mb_name') ?>이름</a></th>
        <th scope="col" id="mb_list_id"><?php echo subject_sort_link('mb_id') ?>아이디</a></th>
        <th scope="col" id="mb_list_auth">사유</th>
        <th scope="col" id="mb_list_auth">설정</th>
        <th scope="col" id="mb_list_mng"></th>
    </tr>
    </thead>
    <tbody>
    <?php
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        $s_mod = '<input type="button" class="btn btn_02 mb_intercept" value="변경" mb_id="'.$row['mb_id'].'" ">';
    ...
    <tr class="<?php echo $bg; ?>">
        <td headers="mb_list_join" class="td_date"><?php echo $intercept_date; ?></td>
        ...
        <td headers="mb_list_mobile" class="td_tel"><?php echo get_text($row['mb_7']); ?></td>
        <td headers="mb_list_sex" ><?php
            if(get_text($row['mb_block_login']) == "1") echo "로그인차단";
            if(get_text($row['mb_block_shop']) == "1") echo " 구매차단";
            if(get_text($row['mb_block_write']) == "1") echo " 글차단";
            ?></td>
...
<div class="modal-content">
  ...
  <div class="modal-footer">
    ...
    <input type="submit" value="불량회원설정" onclick="document.pressed=this.value" class="btn btn-success">
  </div>
</div>
<!-- Modal content-->
</div>
...
$(function(){
    ...
    $(".mb_intercept").on("click", function() {
        var $this = $(this);
