...
//if ($stx) 
{
    ...
    $search_query .='&amp;w='.$w.'&amp;mb_id='.$mb_id.'&amp;mode='.$mode;
...
if(!isset($rtnURL) || $rtnURL == "") $rtnURL = G5_ADMIN_URL."/member_form.php";
...
?>
    ...
    <div class="x_title">
      ...
      <br /><br />
        ...
        <form name="fboardlist" id="fboardlist" action="<?php echo G5_ADMIN_URL?>/community/post_management_list_update.php" onsubmit="return fboardlist_submit(this);" method="post">
      ...
      <div class="tbl_head01 tbl_wrap" style="margin-bottom: 50px">
        ...
        <tr>
          <th colspan="9" style="text-align: right;">
            <div style="float: left;">
              <button class="btn btn_03" type="button" id="btn_view_deleted1" >삭제된 글 보기</button>
            </div>
        ...
        <tr>
          <td class="td_chk">
                ...
                <input type="hidden" name="wr_id[]" value="<?php echo $list[$idx][$i]['wr_id'] ?>" id="wr_id_<?php echo $k ?>">
                <input type="hidden" name="board_table[]" value="<?php echo $list[$idx][$i]['bo_table'] ?>" id="bo_table_<?php echo $k ?>">
          ...
          <td class="td_left" style="text-align:left; padding-left:<?php echo $list[$idx][$i]['reply'] ? (strlen($list[$idx][$i]['wr_reply'])*10) : '0'; ?>px">
          	<?php echo ($list[$idx][$i]['wr_8'] == "스팸")?"<span class='label label-danger'>SPAM</span>":"";?>
          	
          	<a href="#" onclick='fview("v","<?php echo $list[$idx][$i]['bo_table'] ?>","<?php echo $list[$idx][$i]['wr_id'] ?>","<?php echo $k ?>");'>
          	<?php echo ($list[$idx][$i]['wr_reply'] != "")?"<i class='fa fa-long-arrow-right'></i>[답변]":"";?>
            &nbsp;
          ...
          <td class="td_id">
            <?php if($list[$idx][$i]['wr_9'] == '답변전') echo '<a href="#" onclick=\'fview("r","'.$list[$idx][$i]['bo_table'].'","'.$list[$idx][$i]['wr_id'].'");\'><button class="btn btn_02" type="button">답변하기</button></a>'; ?>
          </td>
        </tr>
        <?php } ?>
    	<?php } ?>
        ...
        <tfoot>
          <tr>
            <th colspan="9" style="text-align: right;">
              <div style="float: left;">
                <button class="btn btn_03" type="button" id="btn_view_deleted2" >삭제된 글 보기</button>
              </div>
...
<div class="modal-content">
  ...
  <div class="modal-body">
    ...
    <form name="fboardlist2" id="fboardlist2" action="<?php echo G5_ADMIN_URL?>/community/post_management_list_update.php" onsubmit="return fboardlist_submit2(this);" method="post">
        ...
        <tbody>
        <?php 
        $sql = " select a.*, b.bo_subject from lt_write_delete as a, {$g5['board_table']} as b where a.bo_table = b.bo_table and wr_is_comment = '0' and mb_id = '{$mb_id}' order by a.wr_del_datetime desc ";
        if($test) echo $sql.'<br/>';
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            ...
            <td colspan="2">
              ...
              <input type="checkbox" name="chk_point_to" value="wirter" id="chk_point_to_wirter" checked="checked" disabled="disabled">
              게시물 작성자</label> &nbsp;
              
              <label for="chk_point_to_comment">
              <input type="checkbox" name="chk_point_to" value="comment" id="chk_point_to_comment" disabled="disabled">
              댓글 작성자</label> &nbsp;
            </td>
        </tr>
        <tr>
...
function fview(w, bo_table, wr_id, k)
{
	...
	$.post(
        "<?php echo G5_ADMIN_URL?>/community/ajax.post_get.php",
        { w: w
        , bo_table: bo_table
        , wr_id:wr_id
        , rtnURL : '<?php echo $rtnURL?>' 
