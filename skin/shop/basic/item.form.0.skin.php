...
?>
<style>
	#sit_sel_option input[type="text"] {
		height: 19px;
		margin-top: 6px;
	...
	#btn-size-selector button.btn.green {
		font-weight: bold;
	}
				...
				<div class="frt_descript">
					<div class="head">
						<div class="comm">
							...
							if ($view_detail_items['view_wish']) {
								...
								echo "<button type=\"button\" class=\"pick ico " . (($rowwish['wishis'] != '0') ? 'on' : '') . "\" onclick=\"item_wish(document.fitem, '" . $it_id . "');\" it_id=\"" . $it_id . "\"><span class=\"blind\">찜</span></button>";								//echo "</a>";
							}
					...
					<div class="order_list">
							...
							<li>
								<div class="info_service">
									...
									if ($sc_price <= 0) {
										...
										$sc_price = "배송비 : " . number_format($sc_price) . "원";
									}

									if (!empty($sc_min_price)) {
										$sc_price .= " (" . number_format($sc_min_price) . "원 이상 주문시 무료배송)";
									}
									?>
									<ul class="info_list">
										<li class="cell"><span class="icon-pack icon-delevery"></span><?= $sc_price ?></li>
									</ul>
								</div>
							</li>
						</ul>
						<div class="order_list" id="list-item-options">
							<ul>
								...
								<? if ($subits[0]['its_free_laundry'] || $subits[0]['its_laundry_use'] || $subits[0]['its_laundrykeep_use'] || $subits[0]['its_repair_use']) { ?>
									<li style="border-bottom:1px solid #d1d1d1; padding-bottom:20px;">
										<span class="item full <? if ($subits_count > 1) echo "info_add" ?>">케어 서비스 <span style="font-size:12px;">(제공 서비스)</span></span>
										<div class="info_service">
											<p class="info_title"><?= $subits[0]['its_item'] ?></p>
											<ul class="info_list">
												<?
											...
											<div class="info_service">
												<?
														for ($i = 1; $i < count($subits); $i++) {
															$its = $subits[$i];
															?>
													<p class="info_title"><?= $its['its_item'] ?></p>
													<ul class="info_list">
														<?
								...
								<? } ?>
								<li>
									<select id="select-size-selector" onchange="add_sel_option_by_select(this)" style="user-select: none;">
										<option value="">필수 옵션을 선택해주세요.</option>
										<?
										if (count($subits) > 1) { ?>
											<option value='S'>사이즈 S</option>
											<option value='Q'>사이즈 Q</option>
											<option value='K'>사이즈 K</option>
										<?
										} else {
											...
											foreach ($subits as $sits) {
												...
												for ($ii = 0; $io = sql_fetch_array($db_io); $ii++) {
													if (!isset($si_options[$io['io_id']])) $si_options[$io['io_id']] = $io['io_id'];
												}
											}

											if (!empty($si_options)) {
												foreach ($si_options as $so) echo "<option value='{$so}'>" . str_replace('_', ' ', $so) . "</option>";
											}
										}
										?>
									</select>
								...
								for ($i = 0; $i < count($subits); $i++) {
									$its = $subits[$i];
									$is_pillow = (preg_match("/베개/", $its['its_item']) === 1);
									?>
									<li style="display: none;">
										<p class="info_title" its_no="<?= $its['its_no'] ?>"><?= $its['its_item'] ?> <span class="price"><em class="point"><?= display_price($its['its_final_price']) ?> 원</em></span></p>
										<input type="hidden" name="its_final_price[]" value="<?= $its['its_final_price']; ?>" its_no="<?= $its['its_no'] ?>" />
											...
											<li>
												...
												<strong class="result">
													<?
														...
														$io_select = '<select id="it_option_' . $i . '" name="sel_it_option[]" class="cart_it_option btn_select" its_no="' . $its['its_no'] . '" data-pillow="' . $is_pillow . '">' . PHP_EOL;
						...
						<div class="btn_group two">
							<? if ($is_orderable) { ?>
								<button type="button" class="btn big white" onclick="document.pressed=this.value;fitem_submit();" value="장바구니"><span>장바구니</span></button>
								<button type="button" class="btn big green" onclick="document.pressed=this.value;fitem_submit();" value="바로구매"><span>바로구매</span></button>
							<? } ?>
							...
							<? } else if (!$is_orderable) { ?>
								<button type="button" class="btn big green" style="width: 100%;"><span>재고없음</span></button>
							<? } ?>
...
<script>
	...
	function add_sel_option_mobile(type, id, option, price, stock, it_price, its_no, supply_ids) {
		...
		const item_option_count = $("select[name='sel_it_option[]']").length;
		const item_name_price = $("p.info_title[its_no='" + its_no + "'] > span").text().trim();
		const item_name = $("p.info_title[its_no='" + its_no + "']").text().replace(item_name_price, '').trim();

		if ($(".sit_opt_list[its_no='" + its_no + "']").size() >= 1) {
			$(".sit_opt_list[its_no='" + its_no + "']").each(function(soi, soe) {
				const oid = $(soe).find("input[name='io_id[" + item_code + "][]']").val();
				if (oid === id) {
					$(soe).find("button.count_plus").click();
					price_calculate();
					isExists = true;
				}
			});
		...
		let item_name_title = item_name + "(" + id + ") : " + opt_prc;
		if (item_option_count <= 1) {
			item_name_title = $("div.head > p.title").text();
		}
		...
		// opt += "<div class=\"item\" style='width: 100%'>" + item_name_title + "</div>";
		opt += "<div class=\"item\" style='width: 100%'>" + item_name + " " + id + " (+" + number_format(price) + "원)</div>";


		opt += "<div class=\"count_control\">";
		...
		opt += "<button type=\"button\" class=\"count_plus\"><span class=\"blind\">증가</span></button></div>";
		opt += "<div id='sit-opt-remove'>" + number_format(it_price * 1 + price * 1) + " 원<button type='button' class='btn-sit-opt-remove' data-its_no=" + its_no + " onclick='removeSelectedOptions(this)'><span>×</span></button></div></li>";
	...
	function add_sel_option_by_size(elem, size) {
		...
		$(optionSelect).each(function(idx, select) {
			...
			$(select).children().each(function(cidx, option) {
				if (option.value) {
					if (option.value.indexOf("_" + size) >= 0) $(option).attr("selected", "selected");
				}
			});
			...
			if (is_pillow && (size == 'K' || size == 'Q')) {
				const btnCountUp = $("li.sit_opt_list[its_no='" + its_no + "']").find("button.count_plus");
				btnCountUp.click();
			}
		});

		$("#btn-size-selector").find("button.btn.green").removeClass("green").addClass("white");
		$(elem).removeClass("white").addClass("green");
	}
	...
	function add_sel_option_by_select(elem) {
		...
		if ($(".info_option").length > 1) $(".sit_opt_list").remove();
