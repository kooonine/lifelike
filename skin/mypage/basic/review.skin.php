...
$sql_list = "select * from lt_write_review where mb_id = '{$member['mb_id']}' and wr_is_comment = 0 and wr_datetime >= DATE_ADD(now(), INTERVAL -".$month." MONTH) ";
$sql_board = "select * from lt_board where bo_table='{$type}'";
$sql_count = "select count(*) as cnt from lt_write_review where mb_id = '{$member['mb_id']}'  and wr_is_comment = 0 and wr_datetime >= DATE_ADD(now(), INTERVAL -".$month." MONTH) ";
		...
		while ($list=sql_fetch_array($result)) {
			...
			<li>
				<div class="ListBox">
					<div class="cont">
						<? if($list['wr_file'] == 1){
							$sql2 = " select bf_file from lt_board_file where bo_table='review' and wr_id= {$list['wr_id']} ";
							$row2 = sql_fetch($sql2);
							$sum_img_url = G5_DATA_URL.'/file/review/'.$row2['bf_file'];
							?>
							<div class="photo">
								<a href="<?=G5_BBS_URL.'/board.php?bo_table=review&wr_id='.$list['wr_id']; ?>"><img src="<?=$sum_img_url;?>" alt="" /></a>
							</div>
						<? }?>
						...
						<p class="title bold">
							<?=$list['wr_subject']?>
							<? if($bo_newYN == 'Y'){ ?><span class="new">N</span><? } ?>
						</p>
						<p class="subtitle"><?=$list['wr_3']?></p>
