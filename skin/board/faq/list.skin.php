...
<? if ($rss_href || $write_href) { ?>
	<ul class="<?=isset($view) ? 'view_is_list btn_top' : 'btn_top top';?>">
		<? if ($rss_href) { ?><li><a href="<?=$rss_href ?>" class="btn_b01"><i class="fa fa-rss" aria-hidden="true"></i><span class="sound_only">RSS</span></a></li><? } ?>
		<? if ($admin_href) { ?><li><a href="<?=$admin_href ?>" class="btn_admin"><i class="fa fa-user-circle" aria-hidden="true"></i><span class="sound_only">관리자</span></a></li><? } ?>
		<? if ($write_href) { ?><li><a href="<?=$write_href ?>" class="btn_b02"><i class="fa fa-pencil" aria-hidden="true"></i> 글쓰기</a></li><? } ?>
	</ul>
<? } ?>
<!-- 게시판 목록 시작 -->
<div id="bo_list">
	...
	<div id="bo_list_total">
		<span>전체 <?=number_format($total_count) ?>건</span>
		<?=$page ?> 페이지
	</div>
		...
		<div class="list_01">
			...
			<ul>
				...
				for ($i=0; $i<count($list); $i++) {
					?>
					<li class="<? if ($list[$i]['is_notice']) echo "bo_notice"; ?>">
						<? if ($is_checkbox) { ?>
							<div class="bo_chk">
								...
								<div class="bo_subject">
									...
									<a href="<?=$list[$i]['href'] ?>" class="bo_subject">
										<?=$list[$i]['icon_reply']; ?>
										<? if ($list[$i]['is_notice']) { ?><strong class="notice_icon"><i class="fa fa-volume-up" aria-hidden="true"></i>공지</strong><? } ?>
										<?=$list[$i]['subject'] ?>
										<?
						// if ($list[$i]['file']['count']) { echo '<'.$list[$i]['file']['count'].'>'; }
								...
								<div class="bo_info">
									...
									<span class="bo_date"><? if ($list[$i]['comment_cnt']) { ?><span class="sound_only">댓글</span><i class="fa fa-commenting-o" aria-hidden="true"></i><?=$list[$i]['comment_cnt']; ?><span class="sound_only">개</span><? } ?> <i class="fa fa-clock-o" aria-hidden="true"></i> <?=$list[$i]['datetime2'] ?></span>
						...
						<? if ($list_href || $is_checkbox || $write_href) { ?>
							<div class="bo_fx">
								<ul class="btn_bo_adm">
									<? if ($list_href) { ?>
										<li><a href="<?=$list_href ?>" class="btn_b01 btn"> 목록</a></li>
									<? } ?>
									<? if ($is_checkbox) { ?>
				...
				<fieldset id="bo_sch">
					...
					<form name="fsearch" method="get">
						...
						<input name="stx" value="<?=stripslashes($stx) ?>" placeholder="검색어(필수)" required id="stx" class="sch_input" size="15" maxlength="20">
