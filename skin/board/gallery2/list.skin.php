		...
		<div class="grid ">
			<? if($board['bo_use_userform'] == "1") {?>
				<div class="step-nav">
					<ul class="type2 onoff tab_btn">
						<li name="onOffBar" data='on'><a href="#">진행중</a></li>
			...
			<div class="tab">
				<ul class="type2 tab_btn">
					<li <? if($sst == "wr_datetime") {?>class="on"<? } ?>><a href="<?=G5_BBS_URL?>/board.php?bo_table=<?=$board['bo_table'] ?>&sst=wr_datetime&sod=desc"><span>등록순</span></a></li>
					<li <? if($sst == "wr_good") {?>class="on"<? } ?>><a href="<?=G5_BBS_URL?>/board.php?bo_table=<?=$board['bo_table'] ?>&sst=wr_good&sod=desc"><span>추천순</span></a></li>
				</ul>
			</div>
								...
								<div class="card_list_Box">
									<div class="cont">
										...
										<div class="type-wrap2">
											<ul class="type-tag2">
												<?
												$typeIcon = explode(',', $list[$i]['wr_6']);
												foreach ($typeIcon as $key => $value) {
													if($value){
														?>
										...
										<div class="title bold ellipsis">
											<a href="<?=$list[$i]['href'] ?>"><?=$list[$i]['wr_subject']?></a>
											<? if($bo_newYN == 'Y'){ ?>
												<span class="new">N</span>
											<? } ?>
										</div>
										<div class="text"><?php echo ($board['bo_use_view'] == '1' || $board['bo_use_view_summary'] == '1')?cut_str(get_text($list[$i]['wr_3']), 40):'' ?></div>
										<div class="date"><?php echo ($board['bo_use_view'] == '1' || $board['bo_use_view_datetime'] == '1')?$list[$i]['wr_datetime']:'' ?></div>
									</div>
									<? if($board['bo_use_userform'] != "1") {?>
										<div class="user-area">
											<?
											...
											<div class="user-like">
												<? if($board['bo_use_good']) {?>
													...
													?>
													<button type="button" class="pick ico <?=$pickYN['bg_flag']?'on':''?>" onclick="location.href='<?=$good_href.'&amp;'.$qstr ?>'"><span class="blind">찜</span><?=$list[$i]['wr_good']?></button>
												<? } ?>
