...
<div id="container">
	<div id="lnb" class="header_bar">
		<h1 class="title"><span>체험단신청</span></h1>
...
<?php
    $sql_board = " select * from lt_write_experience where wr_id='$wr_id'";
...
?>
		<div class="inp_wrap">
			<div class="title count3"><label for="f1">신청자 명</label></div>
			<div class="inp_ele count6">
				<div class="input"><input type="text" placeholder="" id="f1" value="<?php echo $member['mb_name']?>"></div>
			</div>
		</div>
<?php
        } 
...
?>
		<div class="inp_wrap">
			<div class="title count3"><label for="f2">이메일</label></div>
			<div class="inp_ele count6">
				<div class="input"><input type="text" placeholder="" id="f2" value="<?php echo $member['mb_email']?>"></div>
			</div>
		</div>
<?php
        }
...
?>
		<div class="inp_wrap">
			...
			<div class="inp_ele count6">
				<div class="input"><input type="tel" placeholder="" id="f3" value="<?php echo $member['mb_hp']?>"></div>
			</div>
		</div>
<?php
        }
...
?>
		...
		<div class="inp_wrap">
			<div class="inp_ele count3">
				<button type="button" class="btn small gray_line" onclick="win_zip('fviewcomment', 'f4', 'f4_2', 'f4_3','f4_4');">우편번호</button>
			</div>
			<div class="inp_ele count6">
				<label for="f4" class="blind" >우편번호</label>
				<div class="input"><input type="tel" placeholder="" id="f4"  name= "f4" title="우편번호" value="<?php echo $member['mb_zip1'].$member['mb_zip2']?>" disabled></div>
			</div>
		</div>
		<div class="inp_wrap">
			<div class="inp_ele">
				<div class="input"><input type="text" placeholder="" id="f4_2" name= "f4_2" title="상세주소" value="<?php echo $member['mb_addr1']?>" ></div>
			</div>
		</div>
		<div class="inp_wrap">
			<div class="inp_ele">
				<div class="input"><input type="text" placeholder="" id="f4_3" name= "f4_3" title="상세주소" value="<?php echo $member['mb_addr2']?>" ></div>
			</div>
		</div>
		<input type="hidden" id = "f4_4"name="f4_4" value="<?php echo $member['mb_addr_jibeon']; ?>">
<?php
        }?>
        <div id="itembox"></div>
...
$(document).ready(function(){
	$("#btnUser_privacy").click(function(){
        var url = '<?php echo G5_MOBILE_URL?>/common/terms_agreement.php?id=chk_01&type=user_privacy';
...
function fviewcomment_submit(f)
{
	...
	contentJson.name = $('#f1').val();
	contentJson.email = $('#f2').val();
	contentJson.phone = $('#f3').val();
	
	contentJson.age = $('#year').val()+'-'+$('#month').val()+'-'+$('#day').val();
	contentJson.sex = $('#mb_sex').val();

	var addressJson = new Object();
	addressJson.addr1= $('#f4').val();
	addressJson.addr2= $('#f4_2').val();
	addressJson.addr3= $('#f4_3').val();
	addressJson.addr_jibeon = $('#f4_4').val();
