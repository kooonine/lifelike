...
header += '<h1 class="title"><span>리뷰 작성</span></h1>';
...
<div class="content mypage sub">
    ...
    <input type="hidden" name="is_type" value="<?php echo ($mode=='txt')?"0":"1"; ?>">
    
    <input type="hidden" name="is_subject" value="<?php echo stripslashes($ct['it_name']); ?> / 옵션 : <?php echo get_text($ct['ct_option']); ?>" id="is_subject" >
	<!-- 컨텐츠 시작 -->
	<div class="grid">
    	...
    	<div class="order_cont">
    		<div class="body">
    			...
    			?>
    				...
    				<div class="info">
    					<strong><?php echo stripslashes($ct['it_name'])?></strong>
    					<p>옵션 : <?php echo get_text($ct['ct_option']); ?></p>
    					<p>주문일 : <?php echo substr($ct['ct_time'],0,10); ?></p>
    				</div>
    			</div>
    		</div>
    	</div>
    	...
    	<div class="review_star">
    		...
    		<div class="star-point">
                <div class="star">
                	<input type="hidden" name="is_score" id="is_score" value="<?php echo $is_score?>">
                    <ul class="select_star">
                        <li class="<?php echo ($is_score>=1)?'on':'';?>"><button type="button" onclick="$('#is_score').val('1');"><span class="blind">1점</span></button></li>
                        <li class="<?php echo ($is_score>=2)?'on':'';?>"><button type="button" onclick="$('#is_score').val('2');"><span class="blind">2점</span></button></li>
                        <li class="<?php echo ($is_score>=3)?'on':'';?>"><button type="button" onclick="$('#is_score').val('3');"><span class="blind">3점</span></button></li>
                        <li class="<?php echo ($is_score>=4)?'on':'';?>"><button type="button" onclick="$('#is_score').val('4');"><span class="blind">4점</span></button></li>
                        <li class="<?php echo ($is_score>=5)?'on':'';?>"><button type="button" onclick="$('#is_score').val('5');"><span class="blind">5점</span></button></li>
                    </ul>
                </div>
            </div>
    		<div class="select_group">
                <input type="hidden" name="is_age" id="is_age" value="<?php echo $is_age?>">
    			<ul class="onoff">
    				<li class="<?php echo ($is_age==20)?'on':'';?>"><a href="#" onclick="$('#is_age').val('20');">20대</a></li>
    				<li class="<?php echo ($is_age==30)?'on':'';?>"><a href="#" onclick="$('#is_age').val('30');">30대</a></li>
    				<li class="<?php echo ($is_age==40)?'on':'';?>"><a href="#" onclick="$('#is_age').val('40');">40대</a></li>
    				<li class="<?php echo ($is_age==50)?'on':'';?>"><a href="#" onclick="$('#is_age').val('50');">50대 이상</a></li>
    			</ul>
    		</div>
    	</div>
    	...
    	<div class="border_top">
    		...
    		<div class="inp_wrap">
    			<span class="byte">0/200</span>
    		</div>
    	</div>
    
    	<div class="btn_comm review" <?php echo ($mode=='txt')?"hidden":""; ?>>
    		...
    		<?php for ($i=0; $i<5; $i++) { ?>
    		<input type="file" id="bf_file<?php echo $i+1 ?>" name="bf_file[]" hidden idx="<?php echo $i+1 ?>" act="<?php echo ($w == 'u' && $file[$i])?"in":"" ?>">
    		<input type="checkbox" id="bf_file_del<?php echo $i+1 ?>" name="bf_file_del[<?php echo $i?>]" value="1" hidden>
    		<?php } ?>
    	</div>
    	<p class="ico_import red point_red" <?php echo ($mode=='txt')?"hidden":""; ?>>최대 사진 5매, 동영상 10mb 이하의 파일만 첨부 가능합니다.</p>
    	<div class="file_list" <?php echo ($mode=='txt')?"hidden":""; ?>>
    		<div class="swiper-container">
    			<ul class="swiper-wrapper" id="file_list">
        			<?php 
        			$j=1;
        			for ($i=0; $i<5; $i++) { ?>
        			<?php if($w == 'u' && $file[$i] && $file[$i]['thumb']) { 
    			    $src = G5_DATA_URL.'/file/itemuse/'.$file[$i]['thumb'];
    			    ?>
    				<li class="swiper-slide" style="margin-right: 10px;">
    					<button type="button" class="btn_delete" id="file_delete" idx="<?php echo $j ?>"><span class="blind">삭제</span></button>
    ...
    <div class="grid foot">
    	<div class="btn_group two">
    		<button type="button" onclick="self.close();history.back();" class="btn big border gray"><span>취소</span></button>
...
function fitemuse_submit(f)
{
	...
	if($("textarea[name='is_content']") != undefined){
		...
		if(rbyte < 40){
			alert("상품에 대한 평가는 한글 기준 20자 이상 작성해 주세요.");
			return false;
		}
	}
