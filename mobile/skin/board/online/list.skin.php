...
<div class="content magazine">
	<!-- 컨텐츠 시작 -->
	<?
	$bannersql = " select * from lt_board where bo_use = 1 and bo_table='online'";
			...
			<div class="swiper-container">
				<div class="swiper-wrapper">
					<? for($i=0; $i<$main_view_data['bannerCount']; $i++) {
						...
						$muse = $img_data['muse'];
						if($muse == 1){
							$img_file = G5_DATA_PATH.'/banner/online/'.$img_data['mimgFile'];
						}else {
						...
						if ($img_data['imgFile'] && file_exists($img_file)) {
							if($muse == 1){
								$img_url = G5_DATA_URL.'/banner/online/'.$img_data['mimgFile'];
							} else {
				...
				for ($i=0; $i<count($list); $i++) {
					...
					<li>
						<a href="<?=$list[$i]['href'] ?>">
							<div class="cont">
								<? if($list[$i]['wr_file'] == 1){
									$sql2 = " select bf_file from lt_board_file where bo_table='online' and wr_id= {$list[$i]['wr_id']} ";
									$row2 = sql_fetch($sql2);
					...
					<div class="btn_fix"><a href="<?=G5_BBS_URL?>/write.php?bo_table=online"><button type="button" class="btn_fix_write"><span class="blind">글쓰기</span></button></a></div>
				</div>
				<!-- 컨텐츠 종료 -->
			</div>
			<!-- 게시판 목록 끝 -->
