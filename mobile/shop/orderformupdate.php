...
while($row=sql_fetch_array($company_result)){
    ...
    if($first) {
        ...
        $sql = "update {$g5['g5_shop_order_table']}
                    set company_code='$c_company_code'
                        ,od_receipt_price = od_receipt_price - (select ifnull(sum((ct_price + io_price) * ct_qty),0) from lt_shop_cart where od_id = '$tmp_cart_id' and ct_select = '1' and company_code != '$c_company_code')
                        ,od_cart_price = od_cart_price  - (select ifnull(sum((ct_price + io_price) * ct_qty),0) from lt_shop_cart where od_id = '$tmp_cart_id' and ct_select = '1' and company_code != '$c_company_code')
                        ,main_od_id = '$od_id'
                  where od_id = '$od_id'";
    ...
    $sql = " select   a.ct_id, a.od_type, a.od_id, a.mb_id
              ...
              , a.its_sap_code, a.its_order_no, a.its_no
              , a.io_sapcode_color_gz,a.io_order_no,a.io_color_name,a.io_hoching,a.io_sap_price
       ...
       from lt_shop_cart as a
            left outer join lt_shop_item_sub as b
