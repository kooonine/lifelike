...
header += '<h1 class="title"><span><?=$title?></span></h1>';
...
<div class="content mypage sub">
    ...
    <div class="grid">
        ...
        <div class="order_cont chk_order">
            ...
            $sql = " select ct_id, it_id, it_name, cp_price, ct_send_cost, it_sc_type
                            ,ct_option, ct_qty, ct_price, ct_point, ct_status, io_type, io_price, ct_rental_price
                        from {$g5['g5_shop_cart_table']} as a
                        where od_id = '$od_id' and a.ct_status = '배송완료'
                        order by ct_id ";
            ...
            <div class="body">
                <div class="cont">
					...
					<li>
                        ...
                        ?>
                        <p>옵션 : <span class="point_black"><?php echo get_text($row['ct_option']) ?></span></p>
						<p>수량 :
							<span class="sel_box" style="width:50px;">
     					...
     					?>
    					<p class="price">
                            <span>리스 금액 : </span><?php echo number_format($sell_price); ?>원
                            <input type="hidden" name="stotal[<?php echo $i ?>]" value="0" id="stotal_<?php echo $i ?>">
    					</p>
    					<?php } else {?>
    					<p class="price">
                            <span>제품금액 : </span><?php echo number_format($sell_price); ?>원
    ...
    <div class="grid">
		...
		<div class="inp_wrap">
			...
			<div class="inp_ele count6">
                <div class="input"><input type="text" name="od_b_hp" id="od_b_hp" required class="frm_input required" maxlength="20" placeholder="휴대전화 번호 입력" value="<?php echo $od['od_b_hp']?>"></div>
			</div>
		</div>
    ...
    <div class="grid">
        <div class="title_bar">
            <h3 class="g_title_01">반품 예정 금액</h3>
        </div>
		<div class="order_title">
			...
			<strong class="result">
				<em class="big point_red" id="cancel_price"><?php echo number_format($od['od_receipt_price']-$od['od_refund_price']); ?> 원</em>
                <span>(1,000 원)</span>
            </strong>
        </div>
        <div class="order_title">
			...
			<strong class="result">
				<em><?=number_format($od['od_receipt_price'] - $od['od_refund_price']); ?> 원</em>
            </strong>            
        </div>
...
function cancel_price_chk(){
	...
	if(cancel_select == "색상및사이즈변경" || cancel_select == "다른상품잘못주문"){
		//배송비 유료 환불조건
		tot_cancel_price = tot_cancel_price - cancel_send_costs;
