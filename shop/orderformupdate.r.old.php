...
$rt_rental_price = $row['od_price']; //월 렌탈료
$cart_count = $row['cart_count']; //제품수
$od_misu = $row['od_misu']; //렌탈금액 * 렌탈개월수 = 총 납부할 금액
...
// 190818 렌탈 1회차 출금시점 변경 : 구매시 -> 출고시 - balance@panpacific.co.kr
/*
...
if ($tno && $default['de_card_test']) {
    ...
    //최소 계약등록시 첫달 렌탈요금 결제, 테스트 결제 시 환불처리
...
msg_autosend('렌탈', '계약 신청', $member['mb_id'], $arr_change_data);
