...
if($act == "buy"){
	...
	//else
	//    goto_url(G5_BBS_URL.'/login.php?url='.urlencode(G5_SHOP_URL.'/orderform.php?od_type='.$od_type));
} else if ($act == "alldelete") {
	// 모두 삭제이면
	$sql = "delete from ".$g5['g5_shop_cart_table']." where od_id = '".$tmp_cart_id."' ";
	sql_query($sql);
	goto_url(G5_SHOP_URL.'/cart2.php?od_type='.$send_od_type);
	...
	for($i=0; $i<$count; $i++) {
		...
		if($it['it_buy_min_qty'] || $it['it_buy_max_qty']) {
			...
			if($it['it_buy_max_qty'] > 0) {
				...
				if(($sum_qty + $row4['ct_sum']) > $it['it_buy_max_qty']){
					alert($it['it_name'].'의 선택옵션 개수 총합 '.number_format($it['it_buy_max_qty']).'개 이하로 주문해 주십시오.', './cart2.php');
				}
			}
		}
		...
		/*
		$opt_list = array();
		$sql = " select * from lt_shop_item_option where it_id = '".$it_id."' and io_use = 1 order by io_no asc ";
		$result = sql_query($sql);
		$lst_count = 0;
		for($k=0; $row=sql_fetch_array($result); $k++) {
			...
			$opt_list[$row['io_type']][$row['io_id']]['its_no'] = $row['its_no'];
		...
		// 장바구니에 Insert
		$comma = '';
		$sql = " INSERT INTO ".$g5['g5_shop_cart_table']." ( od_id, od_type, mb_id, it_id, it_name, it_sc_type, it_sc_method, it_sc_price, it_sc_minimum, it_sc_qty, ct_status, ct_price, ct_point, ct_point_use, ct_stock_use, ct_option, ct_qty, ct_notax, io_id, io_type, io_price, ct_time, ct_ip, ct_send_cost, ct_direct, ct_select, ct_select_time, its_no, ct_rental_price, ct_item_rental_month, company_code ) VALUES ";
		...
		for($k=0; $k<$opt_count; $k++) {
			...
			$sql .= $comma."( '".$tmp_cart_id."', '".$od_type."', '".$member['mb_id']."', '".$it['it_id']."', '".addslashes($it['it_name'])."', '".$it['it_sc_type']."', '".$it['it_sc_method']."', '".$it['it_sc_price']."', '".$it['it_sc_minimum']."', '".$it['it_sc_qty']."', '쇼핑', '".$subrow['its_final_price']."', '".$point."', '0', '0', '".$io_value."', '".$ct_qty."', '".$it['it_notax']."', '".$io_id."', '".$io_type."', '".$io_price."', '".G5_TIME_YMDHIS."', '".$remote_addr."', '".$ct_send_cost."', '".$sw_direct."', '".$ct_select."', '".$ct_select_time."', '".$its_no."', '".$subrow['its_final_rental_price']."', '".$it_item_rental_month."', '".$it['ca_id3']."'
							)";
			$comma = ' , ';
			$ct_count++;
		}
...
if ($sw_direct){
	//if ($is_member)
	//{
	goto_url(G5_SHOP_URL."/orderform.php?sw_direct=".$sw_direct."&od_type=".$send_od_type);
	//}
	//else
	//{
	//    goto_url(G5_BBS_URL."/login.php?url=".urlencode(G5_SHOP_URL."/orderform.php?sw_direct=".$sw_direct."&od_type=".$od_type));
	//}
} else {
	if (G5_IS_MOBILE ) {
				...
				<section class="popup_container">
					<div class="grid">
						...
						<div class="btn_group two">
							<button type="button" class="btn big border gray" onclick="history.back();"><span>쇼핑 계속</span></button>
							<button type="button" class="btn big green" onclick="location.href=\''.G5_SHOP_URL.'/cart2.php?od_type='.$send_od_type.'\'"><span>장바구니 확인</span></button>
						</div>
