    ...
    */
    ...
    if( '0000' == $xpay->Response_Code() ) {
        ...
        for ($i=0; $i<$count; $i++)
        {
            ...
            sql_query(" update {$g5['g5_shop_cart_table']} set ct_status = '주문취소', ct_status_claim = '주문취소' where ct_id = '$ct_id' ");
            
            $sql = " insert into lt_shop_order_history
                        (od_id, ct_id, it_name, is_important, sh_memo, sh_time, sh_ip, sh_mb_name, sh_mb_id)
                     values
                        ('$od_id', '$ct_id', '$it_name', 0, '[주문취소] ".$mod_memo."', '".G5_TIME_YMDHIS."', '$REMOTE_ADDR', '{$member['mb_name']}', '{$member['mb_id']}'); ";
            sql_query($sql);
        }
