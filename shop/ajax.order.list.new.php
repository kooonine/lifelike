...
$perpage = 10;
if ($add_page > 1) $fr = ($add_page - 1) * $perpage . ",";
            ...
            <td class="order_id" style="width: 220px;">
                ...
                <? if(!empty($od_status_button_set[$cart_item['ct_status']])) { 
                    ...
                    <div style="margin-top: 12px;">
                        <button type="button" class="btn-member btn-sm btn-gray btn-round btn-order-action" data-action="<?= $action ?>" data-odid=<?= $order['od_id'] ?> data-ctid=<?= $cart_item['ct_id'] ?> data-invoice='<?= $order['od_invoice'] ? $order['od_invoice'] : $cart_item['ct_invoice'] ?>' data-invoice-co='<?= $order['od_delivery_company'] ? $order['od_delivery_company'] : $cart_item['ct_delivery_company'] ?>'><?= $label ?></button>
                    </div>
                <? endforeach ?>
                <? } ?>
            </td>
            ...
            <td style="padding: 0 14px; border: unset;">
                <div style="background-color: #f2f2f2; display: flex; justify-content: space-evenly;">
                    <? if ($cart_item['ct_status'] != '주문' && $cart_item['ct_status'] != '결제완료' && $cart_item['ct_status'] != '상품준비중' && $cart_item['ct_status'] != '주문취소') :  ?>
                        <a href="/common/tracking.php?invc_co=<?= $cart_item['ct_delivery_company'] ?>&invc_no=<?= $cart_item['ct_invoice'] ?>&view_popup=1" target="_blank">
