...
foreach ($coupons as $coupon) {
    ...
    // 발급여부
    if (is_coupon_downloaded($member['mb_id'], $cp['cz_id'])){
        $mgs_is_coupon .=  "쿠폰 : ".$cp['cz_subject'] . "\n";
        continue;
    }
    // 포인트 쿠폰은 회원포인트 체크
    if ($cp['cz_type'] && ($member['mb_point'] - $cp['cz_point']) < 0)
    ...
    for ($ci = 1; $ci <= $coupon['count']; $ci++) {
        ...
        $sql = " INSERT INTO {$g5['g5_shop_coupon_table']}
            ( cp_id, cp_subject, cp_desc, cp_method, cp_target, mb_id, cz_id, cp_start, cp_end, cp_type, cp_price, cp_trunc, cp_minimum, cp_maximum, cp_datetime )
        VALUES
            ( '$cp_id', '{$cp['cz_subject']}', '{$cp['cz_desc']}', '{$cp['cp_method']}', '{$cp['cp_target']}', '{$member['mb_id']}', '{$coupon['id']}', '$cp_start', '$cp_end', '{$cp['cp_type']}', '{$cp['cp_price']}', '{$cp['cp_trunc']}', '{$cp['cp_minimum']}', '{$cp['cp_maximum']}', '" . G5_TIME_YMDHIS . "' ) ";

        if (isset($coupon['force']) && $coupon['force'] == true) {
            $sql = " INSERT INTO {$g5['g5_shop_coupon_table']}
            ( cp_id, cp_subject, cp_desc, cp_method, cp_target, cz_id, cp_start, cp_end, cp_type, cp_price, cp_trunc, cp_minimum, cp_maximum, cp_datetime )
        VALUES
            ( '$cp_id', '{$cp['cz_subject']}', '{$cp['cz_desc']}', '{$cp['cp_method']}', '{$cp['cp_target']}', '{$coupon['id']}', '$cp_start', '$cp_end', '{$cp['cp_type']}', '{$cp['cp_price']}', '{$cp['cp_trunc']}', '{$cp['cp_minimum']}', '{$cp['cp_maximum']}', '" . G5_TIME_YMDHIS . "' ) ";
        }
...
if($mgs_is_coupon != ''){
    die(json_encode(array('error' => $mgs_is_coupon .'이미 다운로드 했습니다.')));
}else{
    die(json_encode(array('error' => '쿠폰이 발급됐습니다.')));
}
