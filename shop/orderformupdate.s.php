...
// 주문정보 입력 오류시 결제 취소
if(!$result) {
    die('<p>고객님의 주문 정보를 처리하는 중 오류가 발생해서 주문이 완료되지 않았습니다.</p>');
}
...
if(!$result) {
    // 주문삭제
    sql_query(" delete from {$g5['g5_shop_order_table']} where od_id = '$od_id' ");
    sql_query(" update {$g5['g5_shop_cart_table']} set od_id = '$tmp_cart_id', ct_status = '쇼핑', od_sub_id = '' where od_id = '$od_id' ");
...
msg_autosend('수선', '수선 신청', $member['mb_id'], $arr_change_data);
