...
if(($od_settle_case != '후불' && $od_settle_case != '무통장' && $od_settle_case != 'KAKAOPAY') && $default['de_pg_service'] == 'lg' && !$_POST['LGD_PAYKEY'])
    alert('결제등록 요청 후 주문해 주십시오.');

// 장바구니가 비어있는가?
$od_id = get_session('ss_order_id');

if (get_cart_count($od_id) == 0)// 장바구니에 담기
    alert('결제상품이 비어 있습니다.\\n\\n이미 주문하셨거나 결제상품이 없는 경우입니다.', G5_SHOP_URL.'/orderinquiry.php');
...
$tot_ct_price = $od['od_cart_price'];
$od_send_cost = $od['od_send_cost']; //배송비
$od_send_cost2 = $od['od_send_cost2']; //추가배송비

$i_price = $i_price + $i_send_cost;
$order_price = $od['od_misu'];
...
$sql = "update {$g5['g5_shop_cart_table']}
           set ct_receipt_price = '$od_receipt_price',
...
//수선중으로 변경 => 삼진으로 수선중 데이터 전송
SM_ADD_REPAIR_REQUEST($od_id);

if($od['ct_status_claim'] == "고객반려") {
    SM_ADD_REPAIR_REQUEST($od_id);
} else {
    ...
    msg_autosend('수선', '수선 결제 완료', $member['mb_id'], $arr_change_data);
}
// SMS BEGIN --------------------------------------------------------
