		...
		<div class="grid cont">
							...
							<thead>
								<tr>
									...
									<? if ($od['od_type'] == "O" || $od['od_type'] == "R") { ?>
										...
										<th>수량</th>
										<th><?= $od['od_type'] == "R" ? '이용요금(월)' : '제품 금액' ?></th>
										<th><?= $od['od_type'] == "R" ? '전체금액' : '주문 금액' ?></th>
									<? } ?>
									<th>상태</th>
									<? if ($od['od_type'] == "O" || $od['od_type'] == "R") { ?>
										<th>리뷰</th>
				...
				<? if ($od['od_type'] == "L" || $od['od_type'] == "K" || $od['od_type'] == "S") { ?>
					...
					<table class="TBasic3">
						...
						<tr>
							...
							<td><?= nl2br($od['cust_memo']) ?></td>
						</tr>
						<tr>
							<th class="tleft">첨부사진</th>
							<td>
								<ul class="list">
									<?
								...
								<?= count($cust_file) == 0 ? "첨부된 사진이 없습니다." : ""; ?>
							</td>
						</tr>
					</table>
				<? } ?>
			...
			<div class="grid">
				<table class="">
					...
					<tr>
						<td class="bold" style="padding-right:5px; height:35px;">결제상세정보</td>
						<td class="bold" style="padding-left:5px; height:35px;">배송지정보</td>
					</tr>
					<tr>
						<td valign="top" style="padding-right:5px;">
							<table class="TBasic4">
								...
								<? if ($od['od_type'] == "O") { ?>
									...
									<tr>
										<th class="tleft">결제 금액</th>
										<td class="bold"><?= number_format(($od['od_cart_price'] + $od['od_send_cost']) - ($od['od_receipt_point'] + $od['od_cart_coupon'] + $od['od_coupon'])); ?> 원</td>
									</tr>

									<tr>
										<th class="tleft" style="padding-top:10px; padding-bottom:10px;">
											주문 금액
											<ul class="disc" style="margin-top:5px; font-weight:normal;">
												<li><?= $od_type_name ?> 금액</li>
												<li>배송비</li>
											</ul>
										</th>
										<td style="padding-top:10px; padding-bottom:10px;">
											<span class="bold"><?= number_format($od['od_cart_price'] + $od['od_send_cost']); ?> 원</span>
											<ul class="disc" style="margin-top:5px;">
												<li><?= number_format($od['od_cart_price']); ?> 원</li>
												<li><?= number_format($od['od_send_cost']); ?> 원</li>
											</ul>
										</td>
									</tr>
									<? if ($od['od_receipt_point'] + $od['od_cart_coupon'] + $od['od_coupon'] > 0) { ?>
										<tr>
											<th class="tleft" style="padding-top:10px; padding-bottom:10px;">
												할인금액 합계
												<ul class="disc" style="margin-top:5px; font-weight:normal;">
											...
											<td class="fred" style="padding-top:10px; padding-bottom:10px;">
												<span class="bold">- <?= number_format($od['od_receipt_point'] + $od['od_cart_coupon'] + $od['od_coupon']); ?> 원</span>
												<ul class="disc" style="margin-top:5px;">
													<? if ($od['od_coupon'] > 0) { ?>
														<li>- <?= number_format($od['od_coupon']); ?> 원</li>
													<? } ?>
													<? if ($od['od_receipt_point'] > 0) { ?>
														<li>- <?= display_point($od['od_receipt_point']); ?></li>
													<? } ?>
													<? if ($od['od_cart_coupon'] > 0) { ?>
														<li><?= ($od['od_cart_coupon']) ? '-' . number_format($od['od_cart_coupon']) : ''; ?> 원</li>
													<? } ?>
												</ul>
											</td>
										</tr>
								...
								<? if ($od['od_type'] == "R") { ?>
									...
									<tr>
										...
										<td>매월 <?= $od['rt_billday']; ?>일</td>
									</tr>
									<tr>
										<th class="tleft">납부횟수</th>
										<td>
											<?php if ($od['rt_payment_count'] > 0) : ?>
									...
									<? if ($od['od_receipt_point'] > 0) { ?>
										<tr>
											<th class="tleft">적립금 할인</th>
											<td class="fred">- <?= display_point($od['od_receipt_point']); ?></td>
										</tr>
									<? } ?>
									<? if ($od['od_cart_coupon'] > 0) { ?>
										<tr>
											<th class="tleft">제품 정보 할인</th>
									...
									<? if ($od['od_coupon'] > 0) { ?>
										<tr>
											...
											<td class="fred">- <?= number_format($od['od_coupon']); ?> 원</td>
										</tr>
									<? } ?>
								<? } else { ?>
									...
									<? if ($od['od_cancel_price'] > 0) { ?>
										<tr>
											...
											<td class="bold"><?= number_format($od['od_cancel_price']); ?> 원</td>
										</tr>
									<? } ?>
									<? if ($od['od_refund_price'] > 0) { ?>
										<tr>
											<th class="tleft">환불 금액</th>
											<td class="bold"><?= number_format($od['od_refund_price']); ?> 원</td>
										</tr>
									<? } ?>
									<? if ($od['od_send_cost2'] > 0) { ?>
										<tr>
											<th class="tleft">반품 배송비</th>
											<td class="bold"><?= number_format($od['od_send_cost2']); ?> 원</td>
										</tr>
									<? } ?>
								<? } ?>
							</table>
