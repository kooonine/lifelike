...
switch($od['od_type']) {
	...
	if($od['od_status_claim'] == "펭귄반려") $od_status_step .= '<li '.(($od['od_status'] == "세탁중"||$od['od_status'] == "세탁반려"||$od['od_status'] == "보관중"||$od['od_status'] == "보관완료"||$od['od_status'] == "배송중"||$od['od_status'] == "배송완료")?'class="on"':'').'><span>세탁반려</span></li>';
	else $od_status_step .= '<li '.(($od['od_status'] == "세탁중"||$od['od_status'] == "세탁반려"||$od['od_status'] == "보관중"||$od['od_status'] == "보관완료"||$od['od_status'] == "배송중"||$od['od_status'] == "배송완료")?'class="on"':'').'><span>세탁 중</span></li>';
...
switch($od_status) {
	...
	$btn_act .= '<button class="btn gray_line small" od_id="'.$od['od_id'].'" uid="'.$uid.'"><span>해지취소</span></button>';
	$btn_act .= '<button class="btn gray_line small" od_id="'.$od['od_id'].'"><span>수거지변경</span></button>';
	$btn_act .= '<button class="btn gray_line small" od_id="'.$od['od_id'].'"><span>위약금결제</span></button>';
...
<div id="container">
	...
	<div class="content mypage sub">
		...
		<div class="grid cont">
							...
							<? if($od['od_type'] == "O"){ ?>
									...
									<div class="info">
										...
										<p>
											...
											<? if($row['ct_status']=="주문취소"){?>
												<span style="color:#FF0000;"><?=$row['ct_status']; ?></span>
											<? } else if($row['ct_status']=="반품요청"){?>
												<span style="color:#FF0000;"><?=$row['ct_status']; ?></span>
											<? } else if($row['ct_status']=="반품완료"){?>
												<span style="color:#FF0000;"><?=$row['ct_status']; ?></span>
												<?} else {?>
													<span class="point_black"><?=$row['ct_status']; ?></span>
												<? } ?>
											</p>

											<p><span class="txt">제품 금액</span><span class="point_black"><?=number_format($opt_price); ?></span></p>
								...
								<? } else if($od['od_type'] == "R") { ?>
									...
									<div class="cont right_cont">
										...
										<div class="info">
											...
											<p><span class="txt">상태</span><span class="point_black"><?=$row['ct_status']; ?></span></p>
										</div>
										<div class="pay_item">
								...
								<? } else if($od['od_type'] == "L") { ?>
									...
									<div class="cont right_cont">
										...
										<div class="pay_item">
											결제금액<span class="amount"><strong><?=display_price($row['ct_receipt_price']); ?></strong></span>
										</div>
									</div>
								...
								<? } else if($od['od_type'] == "S") { ?>
									...
									<div class="cont right_cont">
										...
										<div class="pay_item">
											결제금액<span class="amount"><strong><?=($od['od_cart_price'])?display_price($tot_price)." 원":"후불"; ?></strong></span>
										</div>
									</div>
								<? } ?>
								<?
								$splitstr = '<div class="btn_comm count3"></div>';
							}
							?>
			...
			<div class="grid">
				<div class="title_bar">
					<h2 class="g_title_01">결제 상세</h2>
					<span style="float:right;">주문일시 : <?=$od['od_time']; ?></span>
				</div>
				...
				<div class="divide_two box stick">
						...
						<div class="order_list result_right">
							<ul>
								...
								<li>
									...
									<strong class="result"><?=$od['rt_payment_count']; ?> 회</strong>
								</li>
								...
								<? if ($od['od_refund_price'] > 0) { ?>
									<li>
										...
										<strong class="result"><?=number_format($od['od_refund_price']); ?></strong>
									</li>
								<? } ?>
							</ul>
						</div>
						...
						<div class="order_list result_right">
							<ul>
								...
								<? if ($od['od_cancel_price'] > 0) { ?>
									<li>
										<span class="item">취소 적립금/금액</span>
					...
					<div class="box">
						<div class="order_title white">
							...
							<strong class="result">
								<em class="big bold"><?=number_format($tot_price-$od['od_refund_price']-$od['od_receipt_point']); ?>원</em>
							</strong>
						</div>
						<? if($od['od_type'] != "R") { ?>
						<div class="order_list result_right">
							<li>
								...
								<strong class="result"><?=number_format($od['od_cart_price']+$od['od_send_cost']+$od['od_send_cost2']); ?>원</strong>
							</li>
							<li>
						...
						<ul>
							...
							<? if ($od['od_invoice'] && $od['od_delivery_company']){ ?>
								...
								<li>
									...
									<strong class="result"><?=$od['od_invoice_time']; ?></strong>
								</li>
							<? } ?>
						</ul>
					</div>
		...
		$(function(){
			$(".btn_invoice2").click(function(){
				...
				if(href.indexOf("<?=G5_URL?>") >= 0){
					$.post(href, { od_id: '<?=$od_id?>' }, function(data) {
			...
			$(document).on("click", "#orderinquiry_btn button", function(){
				...
				switch(mode){
					...
					case "수거지변경":
					case "배송지변경":
					$.post("./orderinquiry.deliverychangeform.php", { od_id: od_id, uid:uid },function(data) {
						$("#dvOrderinquiryPopup").html(data);
					});
					break;
					...
					case "해지신청":
					$.post("./orderinquiry.contractout.php", { od_id: od_id, uid:uid }, function(data){
						$("#dvOrderinquiryPopup").html(data);
					});
					break;
