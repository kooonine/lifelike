...
if ($step == 2) {
    ...
    if ($partable == false && $db_order_cart->num_rows > count($ct_id)) {
        ...
        $contents = include_once(G5_VIEW_PATH . "/member.order.return.confirm.php");
    }
} else {
    if (!empty($od_id)) {
        ...
        $contents = include_once(G5_VIEW_PATH . "/member.order.return.php");
    } else {
